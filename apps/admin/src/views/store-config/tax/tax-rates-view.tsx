        ...
        <article className="grid gap-3 rounded-xl border bg-card p-4">
            ...
            <div className="flex flex-wrap items-center gap-5 text-sm">
                <label className="flex items-center gap-2">
                    <Switch
                        checked={item.compound}
                        onCheckedChange={(checked) => update.mutate({ compound: checked === true })}
                    />
                    {tax("compound")}
                </label>
                <label className="flex items-center gap-2">
                    <Switch
                        checked={item.applies_to_shipping}
                        onCheckedChange={(checked) => update.mutate({ applies_to_shipping: checked === true })}
                    />
                    {tax("shipping")}
                </label>
        ...
        <div className="grid gap-5">
            ...
            <div className="grid gap-2 rounded-xl border bg-card p-4 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_130px_100px_auto] md:items-end">
                <label className="grid gap-1 font-medium text-xs">
                    ...
                    <select
                        ...
                        value={classId ?? ""}
                        onChange={(event) => setClassId(event.target.value ? Number(event.target.value) : null)}
                    >
                        <option value="">—</option>
                ...
                <Button
                    ...
                    onClick={() =>
                        classId &&
                        create.mutate(
                            { tax_class_id: classId, label: label.trim(), rate: Number(rate), country: country.trim() || null },
                            {
                                onSuccess: () => {
                                    setLabel("");
                                    setRate("");
            ...
            {rates.isError ? (
                <div className="flex items-center justify-between rounded-lg border p-3 text-sm">
                    ...
                    <Button size="sm" variant="outline" onClick={() => void rates.refetch()}>
                        {t("retry")}
                    </Button>
                </div>
            ) : null}
            <div className="grid gap-3">
                {(rates.data ?? []).map((item) => (
                    <TaxRateRow key={item.id} item={item} />
                ))}
            </div>
            {rates.data?.length === 0 ? (
                <p className="rounded-lg border p-8 text-center text-muted-foreground text-sm">{tax("noRates")}</p>
            ) : null}
        </div>
    );
}
