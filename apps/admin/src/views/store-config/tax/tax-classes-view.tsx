...
function TaxClassRow({ item }: { item: TaxClass }) {
    ...
    return (
        <div className="grid gap-2 rounded-lg border bg-card p-3 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] md:items-end">
            <label className="grid gap-1 text-xs font-medium"><span>{tax("name")}</span><Input value={name} onChange={(event) => setName(event.target.value)} /></label>
            <label className="grid gap-1 text-xs font-medium"><span>{tax("slug")}</span><Input value={slug} onChange={(event) => setSlug(event.target.value)} dir="ltr" /></label>
            <div className="flex gap-2">
                <Button type="button" size="sm" disabled={!name.trim() || !slug.trim() || update.isPending} onClick={() => update.mutate({ name: name.trim(), slug: slug.trim() })}>{update.isPending ? t("saving") : t("save")}</Button>
    ...
    return (
        <div className="grid gap-5">
            ...
            <div className="grid gap-2 rounded-xl border bg-card p-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] sm:items-end">
                <label className="grid gap-1 text-xs font-medium"><span>{tax("name")}</span><Input value={name} onChange={(event) => setName(event.target.value)} /></label>
                <label className="grid gap-1 text-xs font-medium"><span>{tax("slug")}</span><Input value={slug} onChange={(event) => setSlug(event.target.value)} dir="ltr" /></label>
                <Button type="button" disabled={!name.trim() || !slug.trim() || create.isPending} onClick={() => create.mutate({ name: name.trim(), slug: slug.trim() }, { onSuccess: () => { setName(""); setSlug(""); } })}>{create.isPending ? t("saving") : t("newClass")}</Button>
            </div>
            {classes.isPending ? <p className="text-muted-foreground text-sm">{t("loading")}</p> : null}
            {classes.isError ? <div className="flex items-center justify-between rounded-lg border p-3 text-sm"><span>{t("loadError")}</span><Button size="sm" variant="outline" onClick={() => void classes.refetch()}>{t("retry")}</Button></div> : null}
            <div className="grid gap-2">{(classes.data ?? []).map((item) => <TaxClassRow key={item.id} item={item} />)}</div>
            {classes.data?.length === 0 ? <p className="rounded-lg border p-8 text-center text-muted-foreground text-sm">{tax("noClasses")}</p> : null}
        </div>
    );
}
