    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="grid gap-3 sm:grid-cols-3">
                ...
                <MetricCard icon={Database} label={fa ? "دامنه‌های متصل" : "Connected domains"} value={domainCount} />
                                ...
                                >
                                    <div className="flex items-start justify-between gap-3">
                                        <div className="flex size-9 items-center justify-center rounded-lg border bg-muted/50">
                                            {item.mode === "settings" ? <Settings2 className="size-4" /> : <Database className="size-4" />}
                                        </div>
                                    ...
                                    <div className="mt-4 flex items-center gap-2">
                                        ...
                                        <Badge variant="secondary" className="text-[10px]">
                                            {item.mode === "settings" ? (fa ? "تنظیمات" : "Settings") : fa ? "دامنه اصلی" : "Canonical domain"}
                                        </Badge>
                                    </div>
                                        ...
                                        <div className="min-w-0">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <Badge variant={item.source === "rollback" ? "outline" : "secondary"} className="text-[10px]">
                                                    {item.source === "rollback" ? (fa ? "بازگردانی" : "Rollback") : fa ? "ویرایش" : "Update"}
                                                </Badge>
                                            </div>
                                            <p className="mt-0.5 truncate text-xs text-muted-foreground">
                                                {item.changed_keys.length > 0
                                                    ? item.changed_keys.join("، ")
                                                    : fa ? "نسخه پایه" : "Baseline revision"}
                                            </p>
                                        </div>
                                    </div>
