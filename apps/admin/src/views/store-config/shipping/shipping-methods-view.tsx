        ...
        <div className="grid gap-5">
            ...
            {methods.isError ? (
                <div className="flex items-center justify-between rounded-lg border p-3 text-sm">
                    ...
                    <Button size="sm" variant="outline" onClick={() => void methods.refetch()}>
                        {t("retry")}
                    </Button>
                </div>
            ) : null}
            ...
            {methods.data?.length === 0 ? (
                <p className="rounded-lg border p-8 text-center text-muted-foreground text-sm">{shipping("noMethods")}</p>
            ) : null}
        </div>
    );
}
