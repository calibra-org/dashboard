...
import { BarChart3, RefreshCw, ShieldCheck, Sparkles } from "#/icons";
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
                ...
                <Metric
                    ...
                    value={
                        data.rfm_score === null ? "—" : `${formatNumber(data.rfm_score, locale)} / ${formatNumber(15, locale)}`
                    }
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
                ...
                <Metric
                    ...
                    value={
                        data.historical_contribution_ltv_minor === null
                            ? t("common.unavailable")
                            : formatMoney(data.historical_contribution_ltv_minor, locale)
                    }
                    note={data.prediction_meta.contribution_ltv?.status ?? t("common.unavailable")}
                />
            ...
            <div className="grid gap-3 lg:grid-cols-[1fr_1fr_1.25fr]">
                    ...
                    <div className="mt-3 flex items-end gap-5">
                        <div>
                            <div className="font-semibold text-xl tabular-nums">
                                {formatNumber(Number(support.open_tickets ?? 0), locale)}
                            </div>
                            <div className="text-muted-foreground text-xs">{localizedLabel(locale, "باز", "Open")}</div>
                        </div>
                        <div>
                            <div className="font-semibold text-xl tabular-nums">
                                {formatNumber(Number(support.tickets_90d ?? 0), locale)}
                            </div>
                            <div className="text-muted-foreground text-xs">
                                {localizedLabel(locale, "۹۰ روز اخیر", "Last 90 days")}
                            </div>
                        </div>
                    </div>
                </div>
                <div className="rounded-lg border p-3">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2 text-xs">
                        <Badge variant={consent.email_opt_in ? "secondary" : "outline"}>
                            Email {consent.email_opt_in ? "✓" : "—"}
                        </Badge>
                        ...
                        {data.nba_candidates.map((candidate, index) => (
                            ...
                            >
                                ...
                                <Badge variant="outline">
                                    {candidate.eligibility === "blocked_by_consent"
                                        ? t("detail.blockedByConsent")
                                        : t("detail.candidateOnly")}
                                </Badge>
                            </div>
                        ))}
                    </div>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 border-t pt-3 text-muted-foreground text-xs">
                <span>
                    {t("common.updatedAt")}: {formatDateTime(data.calculated_at, locale)}
                </span>
                <span>
                    {t("common.engineVersion")}: <span className="font-mono text-foreground">{data.engine_version}</span>
                </span>
                <span>
                    {t("detail.qualityStatus")}:{" "}
                    {data.quality_status === "ready" ? localizedLabel(locale, "آماده", "Ready") : t("common.insufficientData")}
                </span>
            </div>
