        ...
        <div className="flex flex-col gap-5">
            ...
            <Card>
                ...
                <CardContent className="p-0">
                    {cohorts.isPending ? <div className="space-y-2 p-4">{Array.from({ length: 6 }, (_, index) => <Skeleton key={index} className="h-10" />)}</div> : null}
                    {cohorts.isError ? <div className="p-6 text-danger text-sm">{cohorts.error.message}</div> : null}
                    {cohorts.data && cohorts.data.length === 0 ? <div className="p-8 text-center text-muted-foreground text-sm">{t("cohorts.empty")}</div> : null}
                    {cohorts.data && cohorts.data.length > 0 ? (
                        <div className="overflow-x-auto">
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        <TableHead>{t("cohorts.cohort")} <HelperTooltip>{t("cohorts.cohortHelp")}</HelperTooltip></TableHead>
                                        <TableHead>{t("cohorts.lifecycle")}</TableHead>
                                        <TableHead className="text-end">{t("cohorts.customers")}</TableHead>
                                        <TableHead className="text-end">{t("cohorts.revenue")} <HelperTooltip>{t("cohorts.revenueHelp")}</HelperTooltip></TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {cohorts.data.map((row) => (
                                        <TableRow key={`${row.cohort}-${row.lifecycle_state}`}>
                                            ...
                                            <TableCell className="text-end tabular-nums">{formatNumber(row.customers, locale)}</TableCell>
                                            <TableCell className="text-end font-medium tabular-nums">{formatMoney(row.revenue_ltv_minor, locale)}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
