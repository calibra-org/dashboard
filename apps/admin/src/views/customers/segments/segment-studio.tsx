...
function operatorLabel(t: ReturnType<typeof useTranslations>, operator: Exclude<SegmentOperator, "in">) {
    const keys = { eq: "operatorEq", neq: "operatorNeq", gt: "operatorGt", gte: "operatorGte", lt: "operatorLt", lte: "operatorLte" } as const;
        ...
        <div className="flex flex-col gap-5">
            ...
            <div className="grid gap-4 xl:grid-cols-[20rem_minmax(0,1fr)]">
                ...
                <div className="flex min-w-0 flex-col gap-4">
                    {!selected ? (
                        <Card>
                            <CardContent className="p-8 text-center text-muted-foreground text-sm">{t("segments.empty")}</CardContent>
                        </Card>
                    ) : (
                        <>
                            <Card>
                                        ...
                                        <div>
                                            ...
                                            <div className="mt-2 flex flex-wrap gap-2">
                                                <Badge variant="outline">
                                                    {definitionQuery.data?.kind === "saved_view" ? t("segments.savedView") : t("segments.dynamic")}
                                                </Badge>
                                                {definitionQuery.data?.member_count !== null && definitionQuery.data?.member_count !== undefined ? (
                                                    <Badge variant="secondary">
                                                        {formatNumber(definitionQuery.data.member_count, locale)} {t("common.customers")}
                                                    </Badge>
                                                ) : null}
                                            </div>
                                        </div>
                                ...
                                <CardContent className="flex flex-col gap-4">
                                    <div className="grid gap-3 md:grid-cols-3">
                                        <Field label={t("segments.kind")} help={t("segments.kindHelp")}>
                                            <Select value={kind} onValueChange={(value) => setKind(value as typeof kind)}>
                                                <SelectTrigger><SelectValue>{(value) => kindLabel(value as typeof kind, locale)}</SelectValue></SelectTrigger>
                                                <SelectContent>
                                                    {(["rule_based", "rfm", "lifecycle", "cohort"] as const).map((value) => (
                                                        <SelectItem key={value} value={value}>{kindLabel(value, locale)}</SelectItem>
                                                    ))}
                                                </SelectContent>
                                            </Select>
                                        </Field>
                                        <Field label={t("segments.refreshPolicy")} help={t("segments.refreshPolicyHelp")}>
                                            <Select value={refreshPolicy} onValueChange={(value) => setRefreshPolicy(value as typeof refreshPolicy)}>
                                                <SelectTrigger>
                                                    <SelectValue>{(value) => value === "event_driven" ? t("common.eventDriven") : t("common.manual")}</SelectValue>
                                                </SelectTrigger>
                                                <SelectContent>
                                        ...
                                        <Field label={t("segments.definition")} help={t("segments.definitionHelp")}>
                                            <Select value={logic} onValueChange={(value) => setLogic(value as typeof logic)}>
                                                <SelectTrigger>
                                                    <SelectValue>{(value) => value === "or" ? t("segments.logicOr") : t("segments.logicAnd")}</SelectValue>
                                                </SelectTrigger>
                                                <SelectContent>
                                    ...
                                    <div className="flex flex-col gap-2">
                                        {conditions.map((condition, index) => (
                                            ...
                                            >
                                                <Select value={condition.feature} onValueChange={(value) => updateCondition(index, { feature: value as SegmentFeature })}>
                                                    ...
                                                    <SelectContent>
                                                        {FEATURES.map((feature) => <SelectItem key={feature} value={feature}>{featureLabel(t, feature)}</SelectItem>)}
                                                    </SelectContent>
                                                </Select>
                                                <Select value={condition.operator} onValueChange={(value) => updateCondition(index, { operator: value as SegmentOperator })}>
                                                    ...
                                                    <SelectContent>
                                                        {OPERATORS.map((operator) => <SelectItem key={operator} value={operator}>{operatorLabel(t, operator)}</SelectItem>)}
                                                    </SelectContent>
                                                </Select>
                                        ...
                                        <Button
                                            ...
                                            onClick={() => setConditions((current) => [...current, { feature: "lifecycle.state", operator: "eq", value: "active_repeat" }])}
                                        >
                                            <Plus className="size-4" aria-hidden="true" />
                                        ...
                                        {preview.data ? (
                                            <div className="space-y-3">
                                                ...
                                                <div className="flex flex-wrap gap-1.5">
                                                    {preview.data.data.sample_customer_ids.map((id) => <Badge key={id} variant="outline">#{formatNumber(id, locale)}</Badge>)}
                                                </div>
                                            </div>
                                        ) : (
                                    ...
                                    <CardContent className="space-y-2">
                                        {definitionQuery.data?.last_evaluated_at ? (
                                            <p className="text-muted-foreground text-xs">
                                                {t("segments.evaluatedAt")}: {formatDateTime(definitionQuery.data.last_evaluated_at, locale)}
                                            </p>
                                        ) : null}
...
function ConditionValueEditor({
    ...
    if (options) {
        ...
        return (
            <Select
                value={serialized}
                onValueChange={(value) => onChange(condition.feature.startsWith("consent.") ? value === "true" : value)}
            >
                <SelectTrigger><SelectValue>{(value) => renderOption(String(value))}</SelectValue></SelectTrigger>
                <SelectContent>
                    {options.map((option) => <SelectItem key={option} value={option}>{renderOption(option)}</SelectItem>)}
                </SelectContent>
            </Select>
        );
    }
