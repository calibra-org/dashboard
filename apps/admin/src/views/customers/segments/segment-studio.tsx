...
const FEATURES: SegmentFeature[] = [
    ...
    "economics.historical_contribution_ltv_minor",
    "consent.email",
    "consent.sms",
];
...
function featureLabel(t: ReturnType<typeof useTranslations>, feature: SegmentFeature) {
    const keys: Record<SegmentFeature, string> = {
        ...
        "economics.historical_contribution_ltv_minor": "featureContribution",
        "consent.email": "featureEmailConsent",
        "consent.sms": "featureSmsConsent",
    };
    return t(`segments.${keys[feature]}` as never);
}
...
function operatorLabel(t: ReturnType<typeof useTranslations>, operator: Exclude<SegmentOperator, "in">) {
    ...
    return t(`segments.${keys[operator]}` as never);
}

function kindLabel(kind: "rule_based" | "rfm" | "cohort" | "lifecycle", locale: Locale) {
    if (kind === "rfm") return "RFM";
    if (locale === "fa") {
        if (kind === "rule_based") return "قانون‌محور";
        if (kind === "lifecycle") return "چرخه‌عمر";
        return "کوهورت";
    }
    if (kind === "rule_based") return "Rule-based";
...
export function SegmentStudio() {
    ...
    const selected = useMemo(
        () => segments.data?.find((segment) => segment.id === selectedId) ?? null,
    ...
    useEffect(() => {
        ...
        setConditions(
            data.definition.conditions.length > 0
                ? data.definition.conditions
                : [{ feature: "lifecycle.state", operator: "eq", value: "active_repeat" }],
        );
            ...
            <div className="grid gap-4 xl:grid-cols-[20rem_minmax(0,1fr)]">
                <Card className="h-fit">
                    <CardHeader className="pb-3">
                        <CardTitle className="text-base">{t("segments.select")}</CardTitle>
                    </CardHeader>
                    <CardContent className="flex flex-col gap-4">
                        ...
                        {segments.data?.length === 0 ? (
                            <p className="text-muted-foreground text-sm">{t("segments.empty")}</p>
                        ) : null}
                                ...
                                <button
                                    ...
                                    className={
                                        selectedId === segment.id
                                            ? "rounded-lg border border-primary/30 bg-primary/5 px-3 py-2 text-start text-sm"
                                            : "rounded-lg border border-transparent px-3 py-2 text-start text-sm hover:bg-muted"
                                    }
                                >
                                    <span className="block font-medium">{segment.name}</span>
                                    <span className="mt-0.5 block text-muted-foreground text-xs">
                                        #{formatNumber(segment.id, locale)}
                                    </span>
                                </button>
                            ))}
                        </div>
                        ...
                        <>
                                    ...
                                    <div className="flex flex-wrap items-start justify-between gap-3">
                                        <div>
                                            ...
                                            <div className="mt-2 flex flex-wrap gap-2">
                                                <Badge variant="outline">
                                                    {definitionQuery.data?.kind === "saved_view"
                                                        ? t("segments.savedView")
                                                        : t("segments.dynamic")}
                                                </Badge>
                                                {definitionQuery.data?.member_count !== null &&
                                                definitionQuery.data?.member_count !== undefined ? (
                                                    <Badge variant="secondary">
                                                        {formatNumber(definitionQuery.data.member_count, locale)}{" "}
                                                        {t("common.customers")}
                                                    </Badge>
                                                ) : null}
                                            </div>
                                        ...
                                        <div className="flex flex-wrap items-center gap-2">
                                            <Button
                                                ...
                                                disabled={!definitionQuery.data?.definition || preview.isPending}
                                            >
                                                <RefreshCw
                                            ...
                                            <Button
                                                ...
                                                disabled={!definitionQuery.data?.definition || evaluate.isPending}
                                            >
                                                <Users className="size-4" aria-hidden="true" />
                                                {t("common.evaluate")}
                                            </Button>
                                            <Button
                                                onClick={persistDefinition}
                                                disabled={saveDefinition.isPending || conditions.length === 0}
                                            >
                                    ...
                                    <div className="grid gap-3 md:grid-cols-3">
                                        <Field label={t("segments.kind")} help={t("segments.kindHelp")}>
                                            <Select value={kind} onValueChange={(value) => setKind(value as typeof kind)}>
                                                <SelectTrigger>
                                                    <SelectValue>
                                                        {(value) => kindLabel(value as typeof kind, locale)}
                                                    </SelectValue>
                                                </SelectTrigger>
                                                <SelectContent>
                                                    {(["rule_based", "rfm", "lifecycle", "cohort"] as const).map((value) => (
                                        ...
                                        <Field label={t("segments.refreshPolicy")} help={t("segments.refreshPolicyHelp")}>
                                            <Select
                                                value={refreshPolicy}
                                                onValueChange={(value) => setRefreshPolicy(value as typeof refreshPolicy)}
                                            >
                                                <SelectTrigger>
                                                    <SelectValue>
                                                        {(value) =>
                                                            value === "event_driven"
                                                                ? t("common.eventDriven")
                                                                : t("common.manual")
                                                        }
                                                    </SelectValue>
                                                </SelectTrigger>
                                        ...
                                        <Field label={t("segments.definition")} help={t("segments.definitionHelp")}>
                                            <Select value={logic} onValueChange={(value) => setLogic(value as typeof logic)}>
                                                <SelectTrigger>
                                                    <SelectValue>
                                                        {(value) =>
                                                            value === "or" ? t("segments.logicOr") : t("segments.logicAnd")
                                                        }
                                                    </SelectValue>
                                                </SelectTrigger>
                                                <SelectContent>
                                        ...
                                        {conditions.map((condition, index) => (
                                            ...
                                            >
                                                <Select
                                                    ...
                                                    onValueChange={(value) =>
                                                        updateCondition(index, { feature: value as SegmentFeature })
                                                    }
                                                >
                                                    <SelectTrigger>
                                                        <SelectValue>
                                                            {(value) => featureLabel(t, value as SegmentFeature)}
                                                        </SelectValue>
                                                    </SelectTrigger>
                                                    <SelectContent>
                                                ...
                                                <Select
                                                    ...
                                                    onValueChange={(value) =>
                                                        updateCondition(index, { operator: value as SegmentOperator })
                                                    }
                                                >
                                                    <SelectTrigger>
                                                        <SelectValue>
                                                            {(value) => operatorLabel(t, value as Exclude<SegmentOperator, "in">)}
                                                        </SelectValue>
                                                    </SelectTrigger>
                                                    <SelectContent>
                            ...
                            <div className="grid gap-4 lg:grid-cols-2">
                                <Card>
                                    <CardHeader className="pb-3">
                                        <CardTitle className="flex items-center gap-2 text-base">
                                            {t("segments.previewResult")}
                                            <HelperTooltip>{t("segments.previewHelp")}</HelperTooltip>
                                        </CardTitle>
                                    </CardHeader>
                                    <CardContent>
                                        {preview.data ? (
                                            <div className="space-y-3">
                                                <p className="font-semibold text-3xl tabular-nums">
                                                    {formatNumber(preview.data.data.count, locale)}
                                                </p>
                                                <div className="flex flex-wrap gap-1.5">
                                                    {preview.data.data.sample_customer_ids.map((id) => (
                                ...
                                <Card>
                                    ...
                                    <CardContent className="space-y-2">
                                        {definitionQuery.data?.last_evaluated_at ? (
                                            <p className="text-muted-foreground text-xs">
                                                {t("segments.evaluatedAt")}:{" "}
                                                {formatDateTime(definitionQuery.data.last_evaluated_at, locale)}
                                            </p>
                                        ) : null}
...
function ConditionValueEditor({
    ...
    const yes = locale === "fa" ? "بله" : "Yes";
    const no = locale === "fa" ? "خیر" : "No";

    if (options) {
        const serialized = String(condition.value);
        const renderOption = (value: string) => {
            if (condition.feature === "lifecycle.state") return t(`lifecycleStates.${value}` as never);
            if (condition.feature === "risk.band") return t(`riskBands.${value}` as never);
            if (condition.feature === "value.band") return t(`valueBands.${value}` as never);
        ...
        return (
            <Select
                ...
                onValueChange={(value) => {
                    ...
                    onChange(condition.feature.startsWith("consent.") ? serializedValue === "true" : serializedValue);
                }}
            >
                <SelectTrigger>
                    <SelectValue>{(value) => renderOption(String(value))}</SelectValue>
                </SelectTrigger>
                <SelectContent>
                    {options.map((option) => (
    ...
    return (
        <Input
            ...
            onChange={(event) =>
                onChange(isNumericFeature(condition.feature) ? Number(event.target.value || 0) : event.target.value)
            }
        />
    );
}
