...
export interface CustomerIntelligence {
    ...
    signals: {
        ...
        consent?: { email_opt_in?: boolean; sms_opt_in?: boolean; personalization?: boolean };
        economics?: {
...
export function useRefreshCustomerIntelligence(customerId: number) {
    ...
    return useMutation({
        mutationFn: () =>
            apiMutate<{ data: CustomerIntelligence }>("POST", `customer-intelligence/customers/${customerId}/refresh`, {
                locale,
            }),
        onSuccess: () => {
...
export function usePreviewCustomerSegment(segmentId: number) {
    ...
    return useMutation({
        mutationFn: () =>
            apiMutate<{ data: { count: number; sample_customer_ids: number[] } }>(
                "POST",
                `customer-segments/${segmentId}/preview`,
                {
                    locale,
                },
            ),
...
export function useEvaluateCustomerSegment(segmentId: number) {
    ...
    return useMutation({
        mutationFn: () =>
            apiMutate<{ data: { member_count: number; evaluated_at: string } }>(
                "POST",
...
export function useCustomerSegmentMembers(segmentId: number | null, page = 1, limit = 25) {
    const locale = useLocale() as Locale;
    return useQuery<{ data: SegmentMember[]; meta: { page: number; limit: number; total: number; last_page: number } }, Error>({
