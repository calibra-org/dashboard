...
const groups: NavGroup[] = [
    ...
    { titleKey: "analytics", items: [
        ...
        { href: "/planning", localizedLabel: { fa: "برنامه‌ریزی تقاضا و موجودی", en: "Demand & Inventory Planning" }, icon: ChartNoAxesCombined },
    ] },
...
function isActive(pathname: string, href: string): boolean {
    if (href === "/products") { if (pathname === "/products" || pathname === "/products/new") return true; return /^\/products\/\d+(?:\/|$)/.test(pathname); }
...
export function Sidebar({ userId }: { userId: number }) {
    const navT = useTranslations("Nav"); const ticketsT = useTranslations("Tickets"); const siteT = useTranslations("Site"); const locale = useLocale(); const pathname = usePathname();
    const factorActive = pathname === "/factor" || pathname.startsWith("/factor/"); const contentActive = pathname === "/content" || pathname.startsWith("/content/"); const seoActive = pathname === "/seo" || pathname.startsWith("/seo/"); const ticketActive = pathname === "/tickets" || pathname.startsWith("/tickets/");
    const ticketUnread = useTicketRealtime(userId); const [factorOpen, setFactorOpen] = useState(factorActive); const [contentOpen, setContentOpen] = useState(contentActive); const [seoOpen, setSeoOpen] = useState(seoActive); const [ticketOpen, setTicketOpen] = useState(ticketActive);
    ...
    const itemLink = (item: NavItem, compact = false) => {
        const Icon = item.icon; const active = isActive(pathname, item.href); const label = item.localizedLabel ? item.localizedLabel[locale === "fa" ? "fa" : "en"] : navT(item.labelKey as Parameters<typeof navT>[0]);
        return <Link key={item.href} href={item.href as never} className={cn("flex items-center rounded-md transition-colors", compact ? "gap-2.5 px-2.5 py-2 text-xs" : "gap-3 px-3 py-2", active ? "bg-sidebar-accent font-medium text-sidebar-accent-foreground" : "text-sidebar-foreground/75 hover:bg-sidebar-accent/60 hover:text-sidebar-accent-foreground")}><Icon className={compact ? "size-3.5 shrink-0" : "size-4 shrink-0"} aria-hidden="true" /><span>{label}</span></Link>;
    };
    const collapsible = (id: string, active: boolean, open: boolean, setOpen: (value: boolean) => void, icon: ComponentType<SVGProps<SVGSVGElement>>, title: string, items: NavItem[], sections?: Record<number, string>, badge = 0) => {
        const Icon = icon;
        return <div className="mt-1"><button type="button" className={cn("flex w-full items-center gap-3 rounded-md px-3 py-2 text-start transition-colors", active ? "bg-sidebar-accent/80 font-medium text-sidebar-accent-foreground" : "text-sidebar-foreground/75 hover:bg-sidebar-accent/60 hover:text-sidebar-accent-foreground")} aria-expanded={open} aria-controls={id} onClick={() => setOpen(!open)}><Icon className="size-4 shrink-0" aria-hidden="true" /><span className="flex-1">{title}</span>{badge > 0 ? <span className="min-w-5 rounded-full bg-primary px-1.5 py-0.5 text-center font-semibold text-[10px] text-primary-foreground tabular-nums"><span className="sr-only">{ticketsT("unread", { count: badge })}</span><span aria-hidden="true">{badge > 99 ? "99+" : badge}</span></span> : null}<ChevronDown className={cn("size-3.5 transition-transform", open && "rotate-180")} aria-hidden="true" /></button>{open ? <div id={id} className="ms-5 mt-1 flex flex-col gap-0.5 border-sidebar-border border-s ps-2">{items.map((item, index) => <div key={item.href}>{sections?.[index] ? <div className="px-2.5 pt-2 pb-1 font-medium text-[0.6rem] text-sidebar-foreground/40 uppercase tracking-wider">{sections[index]}</div> : null}{itemLink(item, true)}</div>)}</div> : null}</div>;
    };
    return <aside className="hidden w-64 shrink-0 flex-col gap-1 border-sidebar-border border-e bg-sidebar text-sidebar-foreground md:flex"><div className="flex h-14 items-center gap-2 border-sidebar-border border-b px-5"><div className="grid size-7 place-items-center rounded-md bg-sidebar-primary font-bold text-sidebar-primary-foreground text-sm"><Box className="size-4" aria-hidden="true" /></div><span className="font-semibold text-sm tracking-tight">{siteT("name")}</span></div><nav className="flex flex-1 flex-col gap-5 overflow-y-auto px-3 py-4 text-sm">{groups.map((group) => <div key={group.titleKey} className="flex flex-col gap-1"><div className="px-3 pb-1 font-medium text-[0.65rem] text-sidebar-foreground/50 uppercase tracking-wider">{navT(group.titleKey)}</div>{group.items.map((item) => itemLink(item))}{group.titleKey === "sales" ? <div className="mt-1">{collapsible("factor-sidebar-items", factorActive, factorOpen, setFactorOpen, ReceiptText, navT("factor"), factorItems)}{collapsible("content-sidebar-items", contentActive, contentOpen, setContentOpen, PenLine, navT("content"), contentItems)}{collapsible("seo-sidebar-items", seoActive, seoOpen, setSeoOpen, Search, navT("seo"), seoItems, { 0: navT("seoSectionControl"), 2: navT("seoSectionCatalog"), 6: navT("seoSectionContent"), 10: navT("seoSectionMonitoring"), 14: navT("seoSectionSystem") })}{collapsible("ticket-sidebar-items", ticketActive, ticketOpen, setTicketOpen, MessageSquare, navT("tickets"), ticketItems, undefined, ticketUnread)}</div> : null}</div>)}</nav></aside>;
}
