...
export function GrowthPortfolioWorkspace() {
    ...
    const loading = overview.isLoading || plans.isLoading || opportunities.isLoading || runs.isLoading || rebalances.isLoading;
    const errored = overview.isError || plans.isError || opportunities.isError || runs.isError || rebalances.isError;
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="grid gap-6 xl:grid-cols-[1.7fr_1fr]">
                    <div>
                        ...
                        <p className="mt-2 max-w-3xl text-muted-foreground text-sm leading-7">
                            فرصت و outcome همچنان از Decision Intelligence فاز ۱۰ می‌آید. این لایه فقط snapshot، constraint،
                            portfolio decision، دلیل عدم انتخاب و evidence بازتوازن را نگه می‌دارد.
                        </p>
                    </div>
                    <div className="rounded-2xl border bg-background/70 p-4">
                        ...
                        <div className="mt-5 grid grid-cols-2 gap-3">
                            <div className="rounded-xl bg-muted/40 p-3">
                                ...
                                <div className="mt-1 font-semibold text-2xl">{money(overview.data?.plans)}</div>
                            </div>
                            <div className="rounded-xl bg-muted/40 p-3">
                                <div className="text-muted-foreground text-xs">Rebalance</div>
                                <div className="mt-1 font-semibold text-2xl">{money(overview.data?.rebalances)}</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {errored ? (
                <Card className="border-destructive/40 p-5 text-destructive">دریافت داده‌های سبد رشد ناموفق بود.</Card>
            ) : null}
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
                <Card className="p-5">
                    ...
                    <div className="mt-2 font-semibold text-3xl tabular-nums">{money(latest?.expected_value_p50_minor)}</div>
                </Card>
                <Card className="p-5">
                ...
                <Card className="p-5">
                    ...
                    <div className="mt-2 font-semibold text-2xl tabular-nums">{money(latestUtilization.cash)}</div>
                </Card>
                <Card className="p-5">
                    <div className="text-muted-foreground text-sm">بیشترین ریسک منتخب</div>
                    <div className="mt-2 font-semibold text-2xl tabular-nums">{percent(latestUtilization.max_risk)}</div>
                </Card>
                <Card className="p-5">
                ...
                <Card className="space-y-4 p-5">
                    ...
                    <div>
                        ...
                        <Textarea
                            ...
                            placeholder="بیشینه‌سازی contribution با حفظ نقدینگی و ریسک"
                        />
                    </div>
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        <div>
                            ...
                            <Input
                                inputMode="numeric"
                                value={cashBudget}
                                onChange={(event) => setCashBudget(event.target.value)}
                            />
                        </div>
                        <div>
                            ...
                            <Input
                                inputMode="decimal"
                                value={hoursBudget}
                                onChange={(event) => setHoursBudget(event.target.value)}
                            />
                        </div>
                        <div>
                        ...
                        <div>
                            <Label>حداکثر اقدام منتخب</Label>
                            <Input
                                inputMode="numeric"
                                value={maxSelected}
                                onChange={(event) => setMaxSelected(event.target.value)}
                            />
                        </div>
                        <div>
                            <Label>حداقل confidence</Label>
                            <Input
                                inputMode="decimal"
                                value={minConfidence}
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            ...
                            <p className="text-muted-foreground text-sm">
                                برای مدیریت candidate و اجرای optimizer یک Plan را انتخاب کن.
                            </p>
                        </div>
                        {selectedPlan ? (
                            <Button
                                disabled={create.isPending || !candidates.data?.length}
                                onClick={() => create.mutate({ path: `plans/${selectedPlan.public_id}/run`, body: {} })}
                            >
                                Optimize
                            </Button>
                        ) : null}
                    ...
                    <div className="mt-4 grid gap-3 md:grid-cols-2">
                        {plans.data?.map((plan) => (
                            <button
                                key={plan.public_id}
                                type="button"
                                onClick={() => setPlanId(plan.public_id)}
                                className={`rounded-2xl border p-4 text-start transition ${selectedPlan?.public_id === plan.public_id ? "border-primary bg-primary/5" : "hover:bg-muted/40"}`}
                            >
            ...
            <div className="grid gap-4 xl:grid-cols-[1fr_1.4fr]">
                <Card className="space-y-4 p-5">
                    <div>
                        <h2 className="font-semibold text-lg">Candidate management</h2>
                        <p className="text-muted-foreground text-sm">منبع فقط opportunity/recommendation باز فاز ۱۰ است.</p>
                    </div>
                    <div>
                        ...
                        >
                            <option value="">انتخاب فرصت…</option>
                            {opportunities.data?.map((item) => (
                                <option key={item.id} value={item.id}>
                                    {item.title_fa} · {item.domain}
                                </option>
                            ))}
                        </select>
                    </div>
                    <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <Input
                                inputMode="numeric"
                                value={candidateCash}
                                onChange={(event) => setCandidateCash(event.target.value)}
                            />
                        </div>
                        <div>
                            ...
                            <Input
                                ...
                                onChange={(event) => setCandidateHours(event.target.value)}
                            />
                        </div>
                        <div>
                            ...
                            <Input
                                ...
                                onChange={(event) => setCandidateConfidence(event.target.value)}
                            />
                        </div>
                        <div>
                            <Label>Risk</Label>
                            <Input
                                inputMode="decimal"
                                value={candidateRisk}
                                onChange={(event) => setCandidateRisk(event.target.value)}
                            />
                        </div>
                    </div>
                    <div>
                        <Label>Dependency case IDs</Label>
                        <Input
                            value={dependencies}
                    ...
                    <div>
                        <Label>Exclusive case IDs</Label>
                        <Input
                            value={exclusiveWith}
                            onChange={(event) => setExclusiveWith(event.target.value)}
                            placeholder="22, 31"
                        />
                    </div>
                    <Button
                        className="w-full"
                        disabled={!selectedPlan || !selectedOpportunity || create.isPending}
                        onClick={() =>
                            selectedPlan &&
                            selectedOpportunity &&
                            create.mutate({
                                path: `plans/${selectedPlan.public_id}/candidates`,
                                body: {
                                    intelligence_case_id: selectedOpportunity.id,
                                    expected_incremental_contribution_minor: Number(candidateValue || 0),
                                    confidence: Math.min(1, Math.max(0, Number(candidateConfidence || 0))),
                                    required_cash_minor: Math.max(0, Number(candidateCash || 0)),
                                    team_hours: Math.max(0, Number(candidateHours || 0)),
                                    warehouse_capacity: 0,
                                    supplier_capacity: 0,
                                    risk: Math.min(1, Math.max(0, Number(candidateRisk || 0))),
                                    reversibility: 0.7,
                                    time_to_value: 0.7,
                                    customer_impact: 0.7,
                                    strategic_alignment: 0.7,
                                    dependencies: numberList(dependencies),
                                    exclusive_with: numberList(exclusiveWith),
                ...
                <Card className="p-5">
                    <div>
                        <h2 className="font-semibold text-lg">Candidate ledger</h2>
                        <p className="text-muted-foreground text-sm">Snapshot هر candidate به version منبع Phase 10 قفل می‌شود.</p>
                    </div>
                    <div className="mt-4 overflow-x-auto">
                        <table className="w-full min-w-[760px] text-sm">
                            <thead className="text-muted-foreground">
                                <tr className="border-b">
                                    <th className="px-3 py-3 text-start">فرصت</th>
                                    <th className="px-3 py-3 text-start">Value</th>
                                    <th className="px-3 py-3 text-start">Confidence</th>
                                    <th className="px-3 py-3 text-start">Risk</th>
                                    <th className="px-3 py-3 text-start">Cash</th>
                                    ...
                                    <tr key={item.id} className="border-b last:border-0">
                                        <td className="px-3 py-3">
                                            ...
                                            <div className="text-muted-foreground text-xs">
                                                #{item.intelligence_case_id} · {item.domain}
                                            </div>
                                        </td>
                                        <td className="px-3 py-3 tabular-nums">
                                            {money(item.expected_incremental_contribution_minor)}
                                        </td>
                                        <td className="px-3 py-3">{percent(item.confidence)}</td>
                                        <td className="px-3 py-3">{percent(item.risk)}</td>
                                        <td className="px-3 py-3 tabular-nums">{money(item.required_cash_minor)}</td>
                                        <td className="px-3 py-3 text-end">
                                            <Button
                                                variant="outline"
                                                disabled={!selectedPlan || remove.isPending}
                                                onClick={() =>
                                                    selectedPlan &&
                                                    remove.mutate({
                                                        path: `plans/${selectedPlan.public_id}/candidates/${item.id}`,
                                                        body: {},
                                                    })
                                                }
                                            >
                        ...
                        {candidates.data?.length === 0 ? (
                            <div className="p-8 text-center text-muted-foreground text-sm">
                                برای این Plan هنوز candidate ثبت نشده است.
                            </div>
                        ) : null}
                    </div>
                </Card>
            ...
            <Card className="p-5">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <p className="text-muted-foreground text-sm">
                            P10/P50/P90، constraint snapshot و تصمیم هر candidate قابل drill-down است.
                        </p>
                    </div>
                </div>
                <div className="mt-4 overflow-x-auto">
                    <table className="w-full min-w-[800px] text-sm">
                        <thead className="text-muted-foreground">
                            <tr className="border-b">
                                <th className="px-3 py-3 text-start">Plan</th>
                                <th className="px-3 py-3 text-start">Status</th>
                                <th className="px-3 py-3 text-start">P10</th>
                                <th className="px-3 py-3 text-start">P50</th>
                                <th className="px-3 py-3 text-start">P90</th>
                        ...
                        <tbody>
                            {runs.data?.map((run) => (
                                <tr
                                    key={run.public_id}
                                    onClick={() => setRunId(run.public_id)}
                                    className={`cursor-pointer border-b last:border-0 ${selectedRunId === run.public_id ? "bg-primary/5" : "hover:bg-muted/30"}`}
                                >
            ...
            {runDetail.data ? (
                <div className="grid gap-4 xl:grid-cols-[1.55fr_1fr]">
                    <Card className="p-5">
                        <div>
                            <h2 className="font-semibold text-lg">Run drill-down</h2>
                            <p className="text-muted-foreground text-sm">
                                Selected / Deferred / Infeasible با دلیل و binding constraint.
                            </p>
                        </div>
                        <div className="mt-4 space-y-2">
                            {runDetail.data.items.map((item) => (
                                <div key={item.id} className="rounded-2xl border p-4">
                                    ...
                                    <div className="mt-3 flex flex-wrap gap-2 text-xs">
                                        <span className="rounded-lg bg-muted/50 px-2 py-1">
                                            Value {money(item.expected_weighted_value_minor)}
                                        </span>
                                        <span className="rounded-lg bg-muted/50 px-2 py-1">Risk {percent(item.risk)}</span>
                                        {parsedConstraints(item.binding_constraints).map((constraint) => (
                                            <span key={constraint} className="rounded-lg border px-2 py-1">
                                                {constraint}
                                            </span>
                                        ))}
                                    </div>
                    ...
                    <Card className="space-y-4 p-5">
                        <div>
                            <h2 className="font-semibold text-lg">Realized outcome</h2>
                            <p className="text-muted-foreground text-sm">
                                Evidence فقط از outcomeهای Phase 10 متعلق به همین Run پذیرفته می‌شود.
                            </p>
                        </div>
                        <div>
                        ...
                        <div>
                            <Label>Attribution confidence</Label>
                            <Input
                                inputMode="decimal"
                                value={attributionConfidence}
                                onChange={(event) => setAttributionConfidence(event.target.value)}
                            />
                        </div>
                        <div>
                            <Label>Measurement window</Label>
                            <Input value={measurementWindow} onChange={(event) => setMeasurementWindow(event.target.value)} />
                        </div>
                        <div>
                            <Label>Phase 10 outcome IDs</Label>
                            <Input
                                value={sourceOutcomeIds}
                                onChange={(event) => setSourceOutcomeIds(event.target.value)}
                                placeholder="101, 102"
                            />
                        </div>
                        <Button
                            ...
                            onClick={() =>
                                create.mutate({
                                    path: `runs/${runDetail.data.public_id}/outcomes`,
                                    body: {
                                        realized_value_minor: Number(realizedValue || 0),
                                        attribution_confidence: Math.min(1, Math.max(0, Number(attributionConfidence || 0))),
                                        measurement_window: measurementWindow.trim() || undefined,
                                        source_outcome_ids: numberList(sourceOutcomeIds),
                                    },
                                })
                            }
                        >
                        ...
                        <div className="space-y-2">
                            {runDetail.data.outcomes.map((outcome) => (
                                <div key={outcome.id} className="rounded-xl bg-muted/40 p-3 text-sm">
                                    <div className="flex justify-between gap-3">
                                        <span>{money(outcome.realized_value_minor)}</span>
                                        <span>
                                            {outcome.realization_ratio == null ? "—" : percent(outcome.realization_ratio)}
                                        </span>
                                    </div>
                                    <div className="mt-1 text-muted-foreground text-xs">
                                        {outcome.measurement_window ?? "بدون بازه"} · confidence{" "}
            ...
            <div className="grid gap-4 xl:grid-cols-[1fr_1.4fr]">
                <Card className="space-y-4 p-5">
                    <div>
                        <h2 className="font-semibold text-lg">Dynamic Rebalance</h2>
                        <p className="text-muted-foreground text-sm">
                            تغییر شرایط Portfolio را refresh می‌کند؛ action فعال پرریسک بدون approval حذف نمی‌شود.
                        </p>
                    </div>
                    <div>
                        <Label>Trigger</Label>
                    ...
                    <div>
                        <Label>شرح/شواهد trigger</Label>
                        <Textarea
                            value={triggerNote}
                            onChange={(event) => setTriggerNote(event.target.value)}
                            placeholder="مثلاً تأمین‌کننده اصلی SKUهای کمپین را تا ۷۲ ساعت آینده تحویل نمی‌دهد."
                        />
                    </div>
                    <div>
                        <Label>Cash override (اختیاری)</Label>
                        <Input
                            inputMode="numeric"
                            value={overrideCash}
                            onChange={(event) => setOverrideCash(event.target.value)}
                            placeholder="خالی = بودجه Plan"
                        />
                    </div>
                    <div>
                        <Label>Active case IDs (اختیاری)</Label>
                        <Input
                            value={activeCaseIds}
                            onChange={(event) => setActiveCaseIds(event.target.value)}
                            placeholder="علاوه بر actionهای in_progress فاز ۱۰"
                        />
                    </div>
                    <Button
                        className="w-full"
                        disabled={!selectedPlan || create.isPending}
                        onClick={() =>
                            selectedPlan &&
                            create.mutate({
                                path: `plans/${selectedPlan.public_id}/rebalance`,
                                body: {
                                    trigger_kind: triggerKind,
                                    trigger_snapshot: { note: triggerNote.trim() },
                                    constraint_overrides: overrideCash.trim()
                                        ? { cash_budget_minor: Math.max(0, Number(overrideCash)) }
                                        : {},
                                    active_case_ids: numberList(activeCaseIds),
                                },
                            })
                        }
                    >
                ...
                <Card className="p-5">
                    <div>
                        <h2 className="font-semibold text-lg">Rebalance & Approval Ledger</h2>
                        <p className="text-muted-foreground text-sm">Approval reference مستقیماً از Governance OS می‌آید.</p>
                    </div>
                    <div className="mt-4 space-y-2">
                        {rebalances.data?.map((event) => {
                            const protectedIds = parsedArray(event.protected_active_case_ids);
                            return (
                                <div key={event.public_id} className="rounded-2xl border p-4">
                                    <div className="flex flex-wrap items-start justify-between gap-3">
                                        <div>
                                            <div className="font-medium">{event.plan_name}</div>
                                            <div className="mt-1 text-muted-foreground text-xs">
                                                {triggerLabels[event.trigger_kind] ?? event.trigger_kind} ·{" "}
                                                {new Date(event.detected_at).toLocaleString("fa-IR")}
                                            </div>
                                        </div>
                                        <span className="rounded-full border px-2.5 py-1 text-xs">{event.status}</span>
                                    </div>
                                    {protectedIds.length ? (
                                        <div className="mt-3 rounded-xl border border-primary/20 bg-primary/5 p-3 text-sm">
                                            اقدام‌های فعال پرریسک محافظت‌شده: {protectedIds.map((id) => `#${id}`).join("، ")}
                                            <div className="mt-1 font-mono text-muted-foreground text-xs">
                                                {event.approval_reference ?? "approval pending"}
                                            </div>
                                        </div>
                                    ) : null}
                                    {event.status === "approval_required" ? (
                                        <Button
                                            ...
                                            disabled={create.isPending}
                                            onClick={() =>
                                                create.mutate({ path: `rebalances/${event.public_id}/apply`, body: {} })
                                            }
                                        >
                                            اعمال پس از Approval
                                        </Button>
                                    ) : null}
                                </div>
                            );
