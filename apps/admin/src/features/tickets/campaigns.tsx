...
import { campaignStatusLabel, campaignTemplateStatusLabel, SupportMetric, SupportPageHeader, supportChannelLabel } from "./ui";
                        ...
                        <div className="min-w-0">
                            ...
                            <div className="mt-2 flex flex-wrap gap-1.5">
                                ...
                                <Badge variant="outline" className={templateClass(campaign.template_status)}>
                                    {locale === "en" ? "Template" : "قالب"}:{" "}
                                    {campaignTemplateStatusLabel(campaign.template_status, locale)}
                                </Badge>
                                <Badge variant="outline">{supportChannelLabel(campaign.channel, locale)}</Badge>
                            </div>
                        </div>
                        <span className="text-muted-foreground text-xs" dir="ltr">
                            v{campaign.version.toLocaleString(numberLocale)}
                        </span>
                    </div>
                </CardHeader>
                <CardContent className="space-y-4 p-4 sm:p-5">
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                        <div className="rounded-xl border p-3">
                            ...
                            <div className="mt-1 font-semibold text-lg tabular-nums">
                                {summary.total.toLocaleString(numberLocale)}
                            </div>
                        </div>
                        <div className="rounded-xl border p-3">
                            <div className="text-[0.65rem] text-muted-foreground">{locale === "en" ? "Sent" : "ارسال‌شده"}</div>
                            <div className="mt-1 font-semibold text-lg tabular-nums">
                                {summary.sent.toLocaleString(numberLocale)}
                            </div>
                        </div>
                        <div className="rounded-xl border p-3">
                            <div className="text-[0.65rem] text-muted-foreground">
                            ...
                            <div className="mt-1 font-semibold text-lg text-success tabular-nums">
                        ...
                        <div className="rounded-xl border p-3">
                            ...
                            <div className="mt-1 font-semibold text-danger text-lg tabular-nums">
                    ...
                    <div className="grid gap-2 text-xs sm:grid-cols-2">
                        <div className="flex items-center justify-between gap-3 rounded-lg border px-3 py-2">
                            <span className="text-muted-foreground">{locale === "en" ? "Queued" : "در صف"}</span>
                            <span>{summary.queued.toLocaleString(numberLocale)}</span>
                        </div>
                        <div className="flex items-center justify-between gap-3 rounded-lg border px-3 py-2">
                            <span className="text-muted-foreground">{locale === "en" ? "Pending" : "در انتظار"}</span>
                            <span>{summary.pending.toLocaleString(numberLocale)}</span>
                        </div>
                        <div className="flex items-center justify-between gap-3 rounded-lg border px-3 py-2">
                            <span className="text-muted-foreground">{locale === "en" ? "Skipped" : "ردشده از صف"}</span>
                            <span>{summary.skipped.toLocaleString(numberLocale)}</span>
                        </div>
                        <div className="flex items-center justify-between gap-3 rounded-lg border px-3 py-2">
                            <span className="text-muted-foreground">{locale === "en" ? "Opted out" : "لغو دریافت"}</span>
                            <span>{summary.opted_out.toLocaleString(numberLocale)}</span>
                        </div>
                    </div>
                    <div className="grid gap-2 text-xs sm:grid-cols-2">
                        <div className="flex items-center justify-between gap-3 rounded-lg border px-3 py-2">
            ...
            <div className="grid gap-4 lg:grid-cols-2">
                    ...
                    <CardContent>
                        <div className="flex flex-wrap gap-2">
                            <Button
                                variant="outline"
                                onClick={() =>
                                    void review.mutateAsync({ expected_version: campaign.version, decision: "approved" })
                                }
                                disabled={review.isPending || campaign.template_status === "approved"}
                            >
                                <CheckCircle2 className="size-4 text-success" aria-hidden="true" />
                                {locale === "en" ? "Approve" : "تأیید قالب"}
                            </Button>
                            <Button
                                variant="outline"
                                onClick={() =>
                                    void review.mutateAsync({ expected_version: campaign.version, decision: "rejected" })
                                }
                                disabled={review.isPending || campaign.template_status === "rejected"}
                            >
                                <XCircle className="size-4 text-danger" aria-hidden="true" />
                                {locale === "en" ? "Reject" : "رد قالب"}
                            </Button>
                        </div>
                        <p className="mt-3 text-muted-foreground text-xs leading-5">
                            {locale === "en"
                                ? "Scheduling is fail-closed: the template must be approved and the selected channel must already be verified connected."
                                : "زمان‌بندی Fail-Closed است: قالب باید تأیید شده و کانال انتخابی از قبل اتصال تأییدشده داشته باشد."}
                        </p>
                        {review.isError ? (
                ...
                <Card className="shadow-sm">
                    ...
                    <CardContent className="flex flex-wrap gap-2">
                        <Button
                            onClick={() =>
                                void transition.mutateAsync({ expected_version: campaign.version, status: "scheduled" })
                            }
                        ...
                        <Button
                            variant="outline"
                            onClick={() =>
                                void transition.mutateAsync({ expected_version: campaign.version, status: "cancelled" })
                            }
                        ...
                        {transition.isError ? (
                            <p className="w-full text-danger text-xs">
                                {locale === "en"
                                    ? "Transition rejected. Check schedule, approval, channel connection and version."
                                    : "تغییر وضعیت رد شد؛ زمان‌بندی، تأیید قالب، اتصال کانال و نسخه کمپین را بررسی کنید."}
                            </p>
                        ) : null}
                    </CardContent>
                </Card>
                ...
                <CardContent>
                    <form onSubmit={add} className="space-y-3">
                        <Textarea
                            ...
                            placeholder={
                                locale === "en"
                                    ? "One email, phone or recipient key per line"
                        ...
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            <span className="text-muted-foreground text-xs">
                                {locale === "en"
                                    ? "Duplicates are deduplicated server-side and opt-outs remain explicit."
                                    : "مقادیر تکراری در سرور حذف می‌شوند و لغو دریافت به‌صورت صریح حفظ می‌شود."}
                            </span>
                            <Button
                                type="submit"
                                size="sm"
...
export function TicketCampaignsPage() {
    ...
    const distribution = useMemo(() => {
        ...
        for (const campaign of rows)
            map.set(campaign.channel, (map.get(campaign.channel) ?? 0) + (campaign.recipient_summary?.total ?? 0));
        ...
        <div className="flex flex-col gap-5">
                ...
                {campaigns.isLoading ? (
                    ...
                    <>
                        ...
                        <SupportMetric
                            ...
                            hint={
                                totals.sent > 0
                                    ? `${deliveredRate.toFixed(1)}% ${locale === "en" ? "of sent" : "از ارسال‌شده"}`
                                    : undefined
                            }
                            icon={CheckCircle2}
                            tone="success"
            ...
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(22rem,0.85fr)]">
                <div className="min-w-0 space-y-4">
                    <Card className="shadow-sm">
                        <CardHeader>
                            <CardTitle className="text-base">{locale === "en" ? "Recent campaigns" : "کمپین‌های اخیر"}</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-2">
                            {campaigns.isLoading ? (
                                ...
                                rows.map((campaign) => {
                                    ...
                                    const pct =
                                        (summary?.total ?? 0) > 0
                                            ? Math.round(((summary?.delivered ?? 0) / (summary?.total ?? 1)) * 100)
                                            : 0;
                                    return (
                                        <button
                                            key={campaign.id}
                                            ...
                                            <div className="flex items-start justify-between gap-3">
                                                <div className="min-w-0">
                                                    ...
                                                    <div className="mt-1 flex flex-wrap items-center gap-2 text-[0.66rem] text-muted-foreground">
                                                        ...
                                                        <span>·</span>
                                                        <span>
                                                            {pct.toLocaleString(numberLocale)}٪{" "}
                                                            {locale === "en" ? "delivered" : "تحویل"}
                                                        </span>
                                                    </div>
                                                </div>
                                                <Badge variant="outline" className={statusClass(campaign.status)}>
                ...
                <aside className="space-y-4 xl:sticky xl:top-4">
                    <Card className="shadow-sm">
                        <CardHeader>
                            <CardTitle className="text-base">
                                {locale === "en" ? "Delivery evidence" : "نمای تحویل واقعی"}
                            </CardTitle>
                            <p className="text-muted-foreground text-xs">
                                {locale === "en"
                                    ? "Aggregated from persisted campaign recipient ledgers."
                                    : "تجمیع‌شده از دفتر واقعی مخاطبان کمپین‌ها."}
                            </p>
                        </CardHeader>
                        <CardContent className="space-y-3">
                            {[
                                ...
                                <div key={String(label)}>
                                    ...
                                    <Progress
                                        value={Number(base) > 0 ? (Number(value) / Number(base)) * 100 : 0}
                                        className="h-1.5"
                                    />
                                </div>
                            ))}
                    ...
                    <Card className="shadow-sm">
                        <CardHeader>
                            <CardTitle className="text-base">
                                {locale === "en" ? "Audience by channel" : "توزیع مخاطب بر اساس کانال"}
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-3">
                            {distribution.length === 0 ? (
                                <p className="py-5 text-center text-muted-foreground text-xs">
                                    {locale === "en" ? "No audience ledger yet." : "هنوز دفتر مخاطبی ثبت نشده است."}
                                </p>
                            ) : (
                                distribution.map(([item, total]) => (
                                    <div key={item}>
                                        <div className="mb-1.5 flex items-center justify-between gap-3 text-xs">
                                            <span>{supportChannelLabel(item, locale)}</span>
                    ...
                    <Card className="shadow-sm">
                        ...
                        <CardContent>
                            <form onSubmit={submit} className="space-y-4">
                                ...
                                <label className="block space-y-1.5 text-xs">
                                    <span className="font-medium">{locale === "en" ? "Message" : "متن پیام"}</span>
                                    <Textarea
                                        name="template_body"
                                        required
                                        maxLength={20_000}
                                        ...
                                        placeholder={
                                            locale === "en" ? "Write the outbound message…" : "متن پیام گروهی را بنویسید…"
                                        }
                                    />
                                </label>
                        ...
                        <CardContent className="flex gap-3 p-4">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-muted-foreground text-xs leading-5">
                                    {locale === "en"
                                        ? "A draft may store a future time, but it becomes scheduled only after explicit approval and verified-channel checks pass."
                                        : "پیش‌نویس می‌تواند زمان آینده داشته باشد، اما فقط پس از عبور از تأیید قالب و بررسی اتصال معتبر، زمان‌بندی‌شده می‌شود."}
                                </p>
                            </div>
                        </CardContent>
                    </Card>
