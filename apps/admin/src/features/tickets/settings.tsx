    ...
    return (
        <div className="rounded-xl border p-3 transition-colors hover:bg-muted/15">
            <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                    ...
                    <div className="mt-1 text-[0.7rem] text-muted-foreground">
                        {locale === "en" ? "Execution priority" : "اولویت اجرا"}:{" "}
                        {rule.priority.toLocaleString(locale === "fa" ? "fa-IR" : "en-US")} ·{" "}
                        {locale === "en" ? "version" : "نسخه"} {rule.version.toLocaleString(locale === "fa" ? "fa-IR" : "en-US")}
                    </div>
                </div>
                <Switch
            ...
            <details className="mt-3 rounded-lg border bg-muted/10">
                <summary className="cursor-pointer px-3 py-2 text-[0.68rem] text-muted-foreground">
                    {locale === "en" ? "View technical conditions & actions" : "مشاهده شرط‌ها و اقدامات فنی"}
                </summary>
                <div className="grid gap-2 border-t p-2 sm:grid-cols-2">
                    <pre className="overflow-x-auto rounded-lg bg-muted p-2 text-[0.65rem] leading-5" dir="ltr">
            ...
            <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-2 text-[0.7rem] text-muted-foreground">
                        ...
                        <span>
                            {locale === "en" ? "version" : "نسخه"}{" "}
                            {rule.version.toLocaleString(locale === "fa" ? "fa-IR" : "en-US")}
                        </span>
                    </div>
                </div>
                <Switch
    ...
    return (
        <div className="flex flex-col gap-5">
            <SupportPageHeader
                ...
                subtitle={
                    locale === "en"
                        ? "Manage SLA defaults, workflow states, routing, automation, operator capacity and integration posture from persisted support configuration."
                        : "SLA، وضعیت‌های گردش کار، مسیریابی، اتوماسیون، ظرفیت کارشناسان و وضعیت اتصال‌ها را از تنظیمات واقعی پشتیبانی مدیریت کنید."
                }
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <SupportMetric
                    label={locale === "en" ? "Workflow states" : "وضعیت‌های گردش کار"}
                    value={(workflowStatuses.data?.length ?? 0).toLocaleString(numberLocale)}
                    icon={Network}
                    tone="info"
                />
                <SupportMetric
                    label={locale === "en" ? "Active automations" : "اتوماسیون‌های فعال"}
                    value={activeAutomations.toLocaleString(numberLocale)}
                    icon={Bot}
                    tone="primary"
                />
                <SupportMetric
                    label={locale === "en" ? "Connected channels" : "کانال‌های متصل"}
                    value={connected.toLocaleString(numberLocale)}
                    hint={`${configured.toLocaleString(numberLocale)} ${locale === "en" ? "configured/enabled" : "پیکربندی‌شده/فعال"}`}
                    icon={Radio}
                    tone="success"
                />
                <SupportMetric
                    label={locale === "en" ? "Fresh agent presence" : "کارشناسان با حضور تازه"}
                    value={freshPresence.toLocaleString(numberLocale)}
                    icon={Users}
                    tone="neutral"
                />
            </div>
                ...
                <Card className="shadow-sm">
                    ...
                    <CardContent className="space-y-4">
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        {(["available", "busy", "away", "offline"] as AgentPresenceState[]).map((value) => (
                                            <SelectItem key={value} value={value}>
                                                {presenceStateLabel(value, locale)}
                                            </SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            <div className="rounded-xl border p-3 text-center">
                                <div className="font-semibold text-lg tabular-nums">
                                    {freshPresence.toLocaleString(numberLocale)}
                                </div>
                            ...
                            <div className="rounded-xl border p-3 text-center">
                                <div className="font-semibold text-lg tabular-nums">
                                    {(presence.data ?? [])
                                        .filter((item) => item.effective_state === "available")
                            ...
                            <div className="rounded-xl border p-3 text-center">
                                <div className="font-semibold text-lg tabular-nums">
                                    {(presence.data ?? [])
                                        .reduce((sum, item) => sum + item.active_count, 0)
                        ...
                        <p className="text-[0.7rem] text-muted-foreground leading-5">
                            {locale === "en"
                                ? "Presence uses a real heartbeat TTL; stale operators are not displayed as online."
                            ...
                            (workflowStatuses.data ?? []).map((status) => (
                                <div key={status.id} className="rounded-xl border p-3">
                                    <div className="flex items-center justify-between gap-2">
                                        ...
                                        <Badge variant="outline">{workflowSemanticLabel(status.semantic_group, locale)}</Badge>
                                    </div>
                                    ...
                                    <div className="mt-2 flex flex-wrap gap-1.5">
                                        {status.is_terminal ? (
                                            <Badge variant="outline">{locale === "en" ? "Terminal" : "نهایی"}</Badge>
                                        ) : null}
                                        {status.is_customer_waiting ? (
                                            <Badge variant="outline">
                                                {locale === "en" ? "Customer waiting" : "انتظار مشتری"}
                                            </Badge>
                                        ) : null}
                                        {!status.is_enabled ? (
                                            <Badge variant="outline">{locale === "en" ? "Disabled" : "غیرفعال"}</Badge>
                                        ) : null}
                                    </div>
                                </div>
                            ))
                    ...
                    <form onSubmit={submitWorkflowStatus} className="rounded-xl border bg-muted/20 p-4">
                        ...
                        <div className="grid gap-3 sm:grid-cols-2">
                            ...
                            <Select name="semantic_group" defaultValue="active">
                                ...
                                <SelectContent>
                                    {(["active", "waiting", "resolved", "closed"] as const).map((value) => (
                                        <SelectItem key={value} value={value}>
                                            {workflowSemanticLabel(value, locale)}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                            <div className="flex flex-wrap items-center gap-4 text-xs">
                                <label className="inline-flex items-center gap-2">
                                    ...
                                    {locale === "en" ? "Terminal" : "نهایی"}
                                </label>
                                <label className="inline-flex items-center gap-2">
                                    <input type="checkbox" name="is_customer_waiting" />
                                    {locale === "en" ? "Customer waiting" : "انتظار مشتری"}
                                </label>
                            </div>
                        </div>
            ...
            <div className="grid gap-4 xl:grid-cols-2">
                <Card className="shadow-sm">
                    <CardHeader>
                        ...
                        <p className="text-muted-foreground text-xs">
                            {locale === "en"
                                ? "Rules are evaluated by priority; technical JSON stays behind the advanced editor."
                                : "قوانین بر اساس اولویت اجرا می‌شوند؛ JSON فنی داخل ویرایشگر پیشرفته قرار دارد."}
                        </p>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        ...
                        <details className="rounded-xl border border-dashed">
                            <summary className="cursor-pointer px-4 py-3 font-medium text-xs">
                                {locale === "en" ? "Advanced: add routing rule" : "پیشرفته: افزودن قانون مسیریابی"}
                            </summary>
                ...
                <Card className="shadow-sm">
                    <CardHeader>
                        ...
                        <p className="text-muted-foreground text-xs">
                            {locale === "en"
                                ? "Enabled automations execute on persisted support events only."
                                : "اتوماسیون‌های فعال فقط روی رویدادهای واقعی و ثبت‌شده پشتیبانی اجرا می‌شوند."}
                        </p>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        ...
                        <details className="rounded-xl border border-dashed">
                            <summary className="cursor-pointer px-4 py-3 font-medium text-xs">
                                {locale === "en" ? "Advanced: add automation" : "پیشرفته: افزودن اتوماسیون"}
                            </summary>
                                    ...
                                    <Select name="trigger" defaultValue="ticket_created">
                                        ...
                                        <SelectContent>
                                            {(
                                                ...
                                                <SelectItem key={value} value={value}>
                                                    {automationTriggerLabel(value, locale)}
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div className="rounded-xl border p-3">
                                ...
                                <div className="text-[0.7rem] text-muted-foreground">
                                    {locale === "en" ? "configured or enabled" : "پیکربندی‌شده یا فعال"}
                                </div>
                            </div>
                            <div className="rounded-xl border p-3">
                                <div className="font-semibold text-xl tabular-nums">{connected.toLocaleString(numberLocale)}</div>
                                <div className="text-[0.7rem] text-muted-foreground">
                                    {locale === "en" ? "provider-verified" : "اتصال تأییدشده"}
                                </div>
                            </div>
                        </div>
                ...
                <Card className="shadow-sm">
                    ...
                    <CardContent className="space-y-3">
                        {[
                            ...
                            locale === "en"
                                ? "Provider credentials are environment references and are never returned as plaintext."
                                : "اعتبارنامه ارائه‌دهنده با env ref نگه‌داری می‌شود و plaintext به مرورگر برنمی‌گردد.",
                            locale === "en"
                                ? "Sensitive mutations use version guards, authorization, rate limits and audit events."
                                : "تغییرات حساس با version guard، مجوز، rate limit و audit event انجام می‌شود.",
                            locale === "en"
                                ? "Connected status requires provider evidence; configuration alone is not treated as healthy."
                                : "وضعیت Connected به شواهد ارائه‌دهنده نیاز دارد و صرف پیکربندی اتصال سالم محسوب نمی‌شود.",
                        ].map((item) => (
