                        ...
                        <div>
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                <Badge variant="outline" tone="success">
                                    Autonomy L{overview.runtime.autonomy_level}
                                </Badge>
                                <Badge
                                    variant="outline"
                                    tone={overview.policies.some((item) => item.status === "frozen") ? "warning" : "success"}
                                >
                                    {overview.policies.some((item) => item.status === "frozen")
                                        ? copy.freezePresent
                                        : copy.governanceReady}
                                </Badge>
                            </div>
...
function OverviewTab({ overview, fa }: { overview: PricingBrainOverview; fa: boolean }) {
    ...
    const awaitingApproval = overview.policies.filter((item) =>
        ["review", "approved", "scheduled"].includes(item.latest_version?.state ?? ""),
    ).length;
    return (
        <div className="flex flex-col gap-5">
            <section
                className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"
                aria-label={fa ? "نمای کلیدی قیمت‌گذاری" : "Pricing KPIs"}
            >
                <MetricCard
                    ...
                    detail={
                        fa
                            ? `${overview.catalog.priced_products} از ${overview.catalog.products} محصول`
                            : `${overview.catalog.priced_products} of ${overview.catalog.products} products`
                    }
                    icon={<Package className="size-4" aria-hidden="true" />}
                    help={
                        fa
                            ? "از regular_price کاتالوگ tenant فعلی محاسبه می‌شود."
                            : "Calculated from regular_price in the current tenant catalog."
                    }
                ...
                <MetricCard
                    ...
                    detail={
                        fa
                            ? `${overview.economics.covered_products} محصول با Cost Evidence`
                            : `${overview.economics.covered_products} products with cost evidence`
                    }
                    icon={<Wallet className="size-4" aria-hidden="true" />}
                    help={
                        fa
                            ? "پوشش واقعی Phase 12 Economics؛ داده مفقود با unavailable نمایش داده می‌شود."
                            : "Real Phase 12 Economics coverage; missing data remains unavailable."
                    }
                ...
                <MetricCard
                    ...
                    help={
                        fa
                            ? "از domain فعلی Coupons و Discounter مشترک خوانده می‌شود."
                            : "Read from the existing Coupons domain and shared Discounter."
                    }
                />
                <MetricCard
                    ...
                    help={
                        fa
                            ? "فقط نسخه active و unfrozen می‌تواند Guardrail checkout باشد."
                            : "Only active, unfrozen versions can guard checkout."
                    }
                />
            </section>
                ...
                <Card className="overflow-hidden">
                    ...
                    <CardContent className="pt-5">
                        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                            <FlowStep
                                index="01"
                                title={fa ? "Proposal" : "Proposal"}
                                detail={
                                    fa
                                        ? `${overview.proposals.length} پیشنهاد ثبت‌شده`
                                        : `${overview.proposals.length} recorded proposals`
                                }
                            />
                            <FlowStep
                                index="02"
                                title={fa ? "Approval" : "Approval"}
                                detail={fa ? `${awaitingApproval} تصمیم باز` : `${awaitingApproval} open decisions`}
                            />
                            <FlowStep
                                index="03"
                                title={fa ? "Guardrail" : "Guardrail"}
                                detail={fa ? `${activePolicies} نسخه فعال` : `${activePolicies} active versions`}
                            />
                            <FlowStep
                                index="04"
                                title="Checkout"
                                detail={fa ? "revalidation + snapshot اتمیک" : "atomic revalidation + snapshot"}
                            />
                        </div>
            ...
            <CardContent className="pt-5">
                <form action={formAction} className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    <Field
                        ...
                        required
                        help={fa ? "قیمت مرجع در integer minor units." : "Reference price in integer minor units."}
                    />
                    <Field
                        name="candidate_price"
                        label={fa ? "قیمت پیشنهادی" : "Candidate price"}
                        required
                        help={
                            fa
                                ? "Candidate قبل از تخفیف‌های Discounter؛ صفر یا بیشتر."
                                : "Candidate before Discounter promotions; zero or greater."
                        }
                    />
                    <Field
                        name="quantity"
                        ...
                        defaultValue="1"
                        help={fa ? "برای درآمد و سود سناریو." : "Used for scenario revenue and profit."}
                    />
                    <Field
                        name="promotion_discount"
                        label={fa ? "Allocation پروموشن" : "Promotion allocation"}
                        defaultValue="0"
                        help={
                            fa
                                ? "مجموع تخفیف سناریویی line در minor units. در checkout این مقدار از Discounter canonical می‌آید."
                                : "Scenario line discount in minor units. Checkout receives this exact value from the canonical Discounter."
                        }
                    />
                    <Field
                        name="product_id"
                        ...
                        help={
                            fa
                                ? "برای بازیابی COGS از Phase 12 وقتی COGS دستی خالی است."
                                : "Used to resolve Phase 12 COGS when manual COGS is empty."
                        }
                    />
                    <Field
                        name="variation_id"
                        label="Variation ID"
                        help={fa ? "برای Cost Evidence سطح واریانت." : "For variation-level cost evidence."}
                    />
                    <Field
                        name="floor_price"
                        ...
                        help={
                            fa
                                ? "در checkout روی درآمد line بعد از Promotion کنترل می‌شود."
                                : "Checked against line revenue after promotions at checkout."
                        }
                    />
                    <Field
                        name="cogs"
                        label={fa ? "COGS دستی" : "Manual COGS"}
                        help={
                            fa
                                ? "اختیاری؛ در غیر این صورت Snapshot و Cost Layer بررسی می‌شوند."
                                : "Optional; otherwise realized snapshots and cost layers are checked."
                        }
                    />
                    <Field
                        ...
                        help={
                            fa
                                ? "Margin بعد از Promotion محاسبه می‌شود؛ بدون COGS، Guardrail با missing_economics fail-closed می‌شود."
                                : "Margin is calculated after promotions; without COGS the guardrail fails closed with missing_economics."
                        }
                    />
                    <Field
                        name="maximum_discount_percent"
                        label={fa ? "حداکثر تخفیف ترکیبی (%)" : "Maximum combined discount (%)"}
                        help={
                            fa
                                ? "افت base/sale و allocation پروموشن با هم نسبت به قیمت مرجع سنجیده می‌شوند."
                                : "Base/sale reduction and promotion allocation are measured together against the reference price."
                        }
                    />
                        ...
                        <Button type="submit" className="w-full" disabled={pending}>
                            {pending
                                ? fa
                                    ...
                                    : "Simulating…"
                                : fa
                                  ? "شبیه‌سازی و کنترل Guardrail"
                                  : "Simulate and validate guardrails"}
                        </Button>
                    </div>
                </form>
                {state.error ? (
                    <div className="mt-4 rounded-lg border border-danger/30 bg-danger/5 p-3 text-danger text-sm" role="alert">
                        {fa ? state.error : "Simulation failed. Check inputs, permissions, and Economics availability."}
                    </div>
                ) : null}
    ...
    return (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <EvidenceCard
                ...
                status={
                    overview.economics.status === "available" ? (fa ? "متصل" : "Available") : fa ? "بدون پوشش" : "Unavailable"
                }
                tone={overview.economics.status === "available" ? "success" : "warning"}
                reason={
                    overview.economics.latest_cost_evidence_at
                        ? `${fa ? "آخرین شواهد هزینه" : "Latest cost evidence"}: ${overview.economics.latest_cost_evidence_at}`
                        : fa
                          ? "Cost Layer قابل استفاده ثبت نشده است."
                          : "No usable Cost Layer is recorded."
                }
            />
            <EvidenceCard
                title={fa ? "کشش قیمت" : "Price elasticity"}
                status={fa ? "شواهد ناکافی" : "Insufficient evidence"}
                tone="warning"
                reason={overview.evidence.elasticity.reason}
            />
            <EvidenceCard
                title={fa ? "آزمایش causal" : "Causal experimentation"}
                status={fa ? "غیرفعال" : "Unavailable"}
                tone="warning"
                reason={overview.evidence.experimentation.reason}
            />
            <Card className="md:col-span-2 xl:col-span-3">
                <CardContent className="grid gap-3 p-5 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <RuntimeRow
                        label={fa ? "Serving promotion" : "Serving promotion"}
                        value={overview.runtime.promotion_engine}
                    />
                    <RuntimeRow
                        label={fa ? "Causal claims" : "Causal claims"}
                        value={fa ? "بدون شواهد: ممنوع" : "Forbidden without evidence"}
                    />
                </CardContent>
            </Card>
        </div>
...
function RuntimeRow({ label, value }: { label: string; value: string }) {
    return (
        <div className="flex items-center justify-between gap-3 rounded-lg border px-3 py-2">
            ...
            <span className="font-medium text-xs sm:text-sm" dir="auto">
                {value}
            </span>
        </div>
    );
        ...
        <div className="mt-5 rounded-xl border bg-muted/20 p-4" aria-live="polite">
            <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex gap-2">
                    <Badge variant="outline" tone={decision.accepted ? "success" : "danger"}>
                        {decision.accepted ? (fa ? "قابل قبول" : "Accepted") : fa ? "رد شده" : "Rejected"}
                    </Badge>
                    <Badge variant="outline" tone={data.economics.value === null ? "warning" : "info"}>
                        {economicsLabel(data.economics.source, fa)}
                    </Badge>
                </div>
            </div>
            <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
                ...
                <ResultMetric
                    label={fa ? "درآمد خالص Candidate" : "Candidate net revenue"}
                    value={decision.candidateNetRevenue}
                />
                <ResultMetric label={fa ? "تخفیف ترکیبی" : "Combined discount"} value={`${decision.discountPercent}%`} />
                <ResultMetric
                    label="Margin"
                    value={decision.marginPercent === null ? (fa ? "ناموجود" : "Unavailable") : `${decision.marginPercent}%`}
                />
            ...
            {decision.violations.length > 0 ? (
                <div className="mt-4 flex flex-col gap-2">
                    {decision.violations.map((item) => (
                        <div key={item.code} className="rounded-lg border border-danger/20 bg-danger/5 px-3 py-2 text-sm">
                            <div className="font-medium text-danger" dir="ltr">
                                {item.code}
                            </div>
                ...
                <div className="mt-4 rounded-lg border border-success/20 bg-success/5 px-3 py-2 text-sm text-success">
                    {fa
                        ? "Candidate پس از لحاظ Allocation پروموشن از Guardrailها عبور کرد."
                        : "The candidate passed all guardrails after the promotion allocation was applied."}
                </div>
            )}
        </div>
    );
...
function ResultMetric({ label, value }: { label: string; value: string | number }) {
    return (
        <div className="rounded-lg border bg-background px-3 py-3">
            ...
            <div className="mt-1 font-semibold tabular-nums" dir="ltr">
...
function economicsLabel(
    source: NonNullable<import("./actions").PricingSimulationState["data"]>["economics"]["source"],
    fa: boolean,
) {
...
function workspaceCopy(fa: boolean) {
    return fa
        ? {
              ...
              description:
                  "مرکز تصمیم، حاکمیت و شبیه‌سازی قیمت روی Price Resolver، Discounter و Economics موجود Calibra؛ با Policy نسخه‌پذیر، Approval مستقل، Freeze/Rollback و ردپای Checkout، بدون موتور قیمت موازی.",
        ...
        : {
              ...
              description:
                  "Decision, governance, and simulation on top of Calibra's existing Price Resolver, Discounter, and Economics, with versioned policies, independent approval, freeze/rollback, and checkout trace—without a parallel pricing engine.",
