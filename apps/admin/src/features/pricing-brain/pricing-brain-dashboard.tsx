        ...
        <div className="flex flex-col gap-6" dir="rtl">
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <p className="mt-2 max-w-3xl text-muted-foreground text-sm leading-7">
                                تصمیم‌یار اقتصادی قیمت و تخفیف روی Price Resolver، Discounter و Economics موجود Calibra؛ بدون موتور
                                موازی و بدون ادعای causal یا Margin ساختگی.
                            </p>
                        </div>
                    </div>
                    <div className="flex gap-2">
                        <Button asChild variant="outline" size="sm">
                            <Link href="/economics">مرکز اقتصاد</Link>
                        </Button>
                        <Button asChild variant="outline" size="sm">
                            <Link href="/coupons">پروموشن‌ها</Link>
                        </Button>
                    </div>
                </div>
            </header>
            ...
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4" aria-label="نمای کلیدی قیمت‌گذاری">
                <MetricCard
                    ...
                    detail={`${overview.catalog.priced_products} از ${overview.catalog.products} محصول`}
                    icon={<Package className="size-4" aria-hidden="true" />}
                    help="از regular_price کاتالوگ tenant فعلی محاسبه می‌شود."
                    progress={overview.catalog.pricing_coverage_percent}
                />
                <MetricCard
                    title="پوشش COGS"
                    value={`${overview.economics.coverage_percent}%`}
                    detail={`${overview.economics.covered_products} محصول با Cost Layer`}
                    icon={<Wallet className="size-4" aria-hidden="true" />}
                    help="پوشش واقعی Phase 12 Economics؛ نبود داده با صفر یا unavailable نمایش داده می‌شود."
                    progress={overview.economics.coverage_percent}
                />
                <MetricCard
                    title="کوپن فعال"
                    value={`${overview.promotions.active_coupons}`}
                    detail={`از ${overview.promotions.coupons} کوپن`}
                    icon={<BadgePercent className="size-4" aria-hidden="true" />}
                    help="از domain فعلی Coupons و Discounter مشترک خوانده می‌شود."
                />
                <MetricCard
                    title="فروش ویژه"
                    value={`${overview.catalog.sale_products}`}
                    detail="محصول دارای sale_price"
                    icon={<TrendingUp className="size-4" aria-hidden="true" />}
                    help="فعال‌شدن زمانی sale همچنان توسط Price Resolver canonical تعیین می‌شود."
                />
            </section>
            ...
            <div className="grid gap-4 xl:grid-cols-[1.45fr_0.55fr]">
                    ...
                    <CardHeader className="border-b bg-muted/20">
                        <CardTitle className="flex items-center gap-2 text-base">
                            ...
                            <HelperTooltip side="bottom">
                                Simulation و production candidate validation از یک هسته deterministic استفاده می‌کنند. این صفحه هیچ
                                قیمت محصولی را فعال یا ذخیره نمی‌کند.
                            </HelperTooltip>
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="pt-5">
                        <form action={formAction} className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                            ...
                            <Field
                                ...
                                label="قیمت پیشنهادی"
                                required
                                help="Candidate برای ارزیابی؛ صفر یا بیشتر."
                            />
                            <Field name="quantity" label="تعداد" defaultValue="1" help="برای درآمد و سود سناریو." />
                            <Field
                                name="product_id"
                                label="شناسه محصول"
                                help="اختیاری؛ برای بازیابی COGS از Phase 12 وقتی COGS دستی وارد نشده است."
                            />
                            <Field
                                name="variation_id"
                                label="شناسه واریانت"
                                help="اگر Cost Evidence مربوط به واریانت است وارد شود."
                            />
                            <Field name="floor_price" label="کف قیمت" help="Candidate پایین‌تر از این مقدار رد می‌شود." />
                            <Field
                                name="cogs"
                                label="COGS دستی"
                                help="اختیاری؛ اگر خالی باشد ابتدا Cost Snapshot و سپس Cost Layer واقعی بررسی می‌شود."
                            />
                            <Field
                                name="minimum_margin_percent"
                                label="حداقل Margin (%)"
                                help="۰ تا ۱۰۰. بدون COGS، این Guardrail با missing_economics رد می‌شود."
                            />
                            <Field
                                name="maximum_discount_percent"
                                label="حداکثر تخفیف (%)"
                                help="حداکثر افت مجاز نسبت به قیمت مرجع، بین ۰ تا ۱۰۰."
                            />
                            <div className="flex items-end md:col-span-2 xl:col-span-3">
                                <Button type="submit" className="w-full" disabled={pending}>
                                    {pending ? "در حال شبیه‌سازی…" : "شبیه‌سازی و کنترل Guardrail"}
                                </Button>
                            </div>
                        </form>
                        {state.error ? (
                            <div
                                className="mt-4 rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-destructive text-sm"
                                role="alert"
                            >
                                {state.error}
                            </div>
                ...
                <div className="flex flex-col gap-4">
                    <EvidenceCard
                        title="اقتصاد Phase 12"
                        status={overview.economics.status === "available" ? "متصل" : "بدون پوشش"}
                        tone={overview.economics.status === "available" ? "success" : "warning"}
                        reason={
                            overview.economics.latest_cost_evidence_at
                                ? `آخرین شواهد هزینه: ${overview.economics.latest_cost_evidence_at}`
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2 text-base">
                                ...
                                مسیر مرجع
                                <HelperTooltip>Phase 18 این authorityها را مصرف می‌کند و جایگزینشان نمی‌شود.</HelperTooltip>
                            </CardTitle>
                        </CardHeader>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex gap-2">
                    <Badge variant="outline" tone={decision.accepted ? "success" : "danger"}>
                        {decision.accepted ? "قابل قبول" : "رد شده"}
                    </Badge>
                    <Badge variant="outline" tone={data.economics.value === null ? "warning" : "info"}>
                        {economicsLabel(data.economics.source)}
                    </Badge>
                </div>
            </div>
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <ResultMetric label="قیمت مؤثر" value={decision.effectivePrice} />
                <ResultMetric label="تخفیف" value={`${decision.discountPercent}%`} />
                <ResultMetric label="درآمد ناخالص" value={decision.grossRevenue} />
                <ResultMetric label="Margin" value={decision.marginPercent === null ? "ناموجود" : `${decision.marginPercent}%`} />
            </div>
            {decision.violations.length > 0 ? (
                <div className="mt-4 flex flex-col gap-2">
                    {decision.violations.map((item) => (
                        ...
                        >
                            <div className="font-medium">{violationLabel(item.code)}</div>
                            <div className="mt-1 text-muted-foreground text-xs">
                                {item.message} · مقدار فعلی: {item.actual ?? "ناموجود"} · حد: {item.required}
                            </div>
                        </div>
                    ))}
                </div>
