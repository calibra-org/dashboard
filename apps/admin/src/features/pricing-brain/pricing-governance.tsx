...
function PolicyWorkspace({ policies, locale, fa }: { policies: PricingPolicySummary[]; locale: string; fa: boolean }) {
    ...
    const review = policies.filter((policy) => ["review", "approved", "scheduled"].includes(policy.latest_version?.state ?? "")).length;
            ...
            <section className="flex flex-col gap-3" aria-label={fa ? "سیاست‌های قیمت" : "Pricing policies"}>
                {policies.length === 0 ? (
                    <EmptyGovernance
                        ...
                        detail={fa ? "اولین policy را به‌صورت Draft بسازید؛ تا زمان Approval و Activation هیچ اثری روی checkout ندارد." : "Create the first policy as a draft. It has no checkout effect until approval and activation."}
                    />
                ) : (
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        ...
                        <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-muted-foreground text-xs">
                            ...
                            <span>{fa ? "نسخه" : "Version"}: {version?.version ?? "—"}</span>
                            <span>{fa ? "هدف" : "Objective"}: {policy.objective ?? "—"}</span>
                            <span>{scopeLabel(version, fa)}</span>
                        </div>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {version ? <GuardrailBadges version={version} fa={fa} /> : null}
                    </div>
                </div>
            </CardHeader>
            <CardContent className="grid gap-5 pt-5 xl:grid-cols-[1.35fr_0.65fr]">
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <div className="rounded-xl border bg-muted/15 p-4 text-xs leading-6">
                        ...
                        {policy.freeze_reason ? <div className="mt-3 rounded-lg border border-danger/30 bg-danger/5 p-2 text-danger">{policy.freeze_reason}</div> : null}
                    </div>
                </div>
            </CardContent>
        </Card>
                ...
                <CardContent className="flex flex-col gap-3 pt-5">
                    {proposals.length === 0 ? (
                        <EmptyGovernance
                            ...
                            detail={fa ? "پیشنهاد قیمت صرفاً evidence و candidate است و قیمت کاتالوگ را تغییر نمی‌دهد." : "A pricing proposal is evidence plus a candidate; it never mutates catalog pricing by itself."}
                        />
                    ) : (
                        proposals.map((proposal) => <ProposalCard key={proposal.id} proposal={proposal} fa={fa} />)
                    )}
                </CardContent>
            </Card>
        </div>
...
function CreateProposalForm({ policies, locale, fa }: { policies: PricingPolicySummary[]; locale: string; fa: boolean }) {
    ...
    return (
        <Card className="overflow-hidden">
            ...
            <CardContent className="pt-5">
                {policies.length === 0 ? (
                    <EmptyGovernance title={fa ? "ابتدا یک Policy بسازید" : "Create a policy first"} detail={fa ? "هر Proposal باید به یک Policy حاکمیتی متصل باشد." : "Every proposal must be linked to a governance policy."} />
                ) : (
                    <form action={action} className="grid gap-4 sm:grid-cols-2">
                        ...
                        <div className="sm:col-span-2">
                            <Button type="submit" className="w-full" disabled={pending}>
                                {pending ? (fa ? "در حال ثبت…" : "Saving…") : fa ? "ثبت Proposal بدون تغییر قیمت" : "Record proposal without repricing"}
                            </Button>
                        </div>
                    </form>
                )}
...
function ProposalCard({ proposal, fa }: { proposal: PricingProposal; fa: boolean }) {
    const delta = proposal.reference_price_minor === 0 ? 0 : Math.round(((proposal.candidate_price_minor - proposal.reference_price_minor) / proposal.reference_price_minor) * 10_000) / 100;
    return (
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <div className="flex items-center gap-2 font-medium text-sm">
                        ...
                        <Badge variant="outline" tone="info">{proposal.status}</Badge>
                    </div>
                    <div className="mt-2 text-muted-foreground text-xs">
                        Policy #{proposal.policy_id} · Product #{proposal.product_id}{proposal.variation_id ? ` / Variation #${proposal.variation_id}` : ""}
                    </div>
                </div>
                <div className="text-end">
                    <div className="font-semibold tabular-nums" dir="ltr">{proposal.candidate_price_minor.toLocaleString()} {proposal.currency}</div>
                    <div className={`mt-1 text-xs ${delta > 0 ? "text-success" : delta < 0 ? "text-warning" : "text-muted-foreground"}`} dir="ltr">
                        {delta > 0 ? "+" : ""}{delta}%
                    </div>
                </div>
            </div>
...
function LifecycleBadge({ state, fa }: { state: PricingPolicyVersion["state"]; fa: boolean }) {
    const tone = state === "active" ? "success" : state === "review" || state === "approved" || state === "scheduled" ? "warning" : state === "rolled_back" || state === "stopped" ? "danger" : "info";
    return (
...
function GuardrailBadges({ version, fa }: { version: PricingPolicyVersion; fa: boolean }) {
    ...
    return (
        <>
            {floor !== null ? <Badge variant="outline">{fa ? "کف" : "Floor"}: {floor.toLocaleString()}</Badge> : null}
            {margin !== null ? <Badge variant="outline">Margin ≥ {margin}%</Badge> : null}
            {discount !== null ? <Badge variant="outline">{fa ? "تخفیف" : "Discount"} ≤ {discount}%</Badge> : null}
        </>
    );
}
...
function MutationFeedback({ state }: { state: { ok: boolean; error: string | null; message: string | null } }) {
    ...
    return (
        <div className={`mt-3 rounded-lg border p-3 text-xs ${state.error ? "border-danger/30 bg-danger/5 text-danger" : "border-success/30 bg-success/5 text-success"}`} role={state.error ? "alert" : "status"}>
...
function lifecycleActions(state: PricingPolicyVersion["state"], fa: boolean) {
    if (state === "draft") return [{ value: "submit", label: fa ? "ارسال برای Review" : "Submit for review", tone: "default" as const }];
