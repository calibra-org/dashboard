...
const percent = (value: number | null | undefined) => (value === null || value === undefined ? "—" : `${fa(value * 100, 1)}٪`);
...
function SectionTitle({ title, description, side }: { title: string; description: string; side?: ReactNode }) {
    return (
        <div className="flex flex-col gap-3 border-border/70 border-b pb-4 md:flex-row md:items-start md:justify-between">
            <div>
                ...
                <div className="grid gap-5 p-6 xl:grid-cols-[1.3fr_.7fr]">
                    <div>
                        ...
                        <h2 className="mt-4 font-semibold text-2xl">
                            کش سریع فقط وقتی ارزش دارد که قابل‌اعتماد، قابل‌ابطال و قابل‌مشاهده باشد
                        </h2>
                        <p className="mt-3 max-w-4xl text-muted-foreground text-sm leading-7">
                            lite cash موتور کش دوم نمی‌سازد. همین cache فعلی Calibra را با policy، purge plan، warm job،
                            optimization profile، evidence و guardrail مدیریت می‌کند. مسیرهای مالی، سفارش، موجودی و داده‌های خصوصی
                            عمداً از cache policy عمومی خارج‌اند.
                        </p>
                    </div>
                    <div className="grid grid-cols-2 gap-3 text-sm">
                        ...
                        <StatusTile
                            label="L1"
                            value={topology.data?.l1_enabled ? "فعال" : "خاموش"}
                            good={topology.data?.l1_enabled}
                        />
                        <StatusTile
                            label="L2"
                            value={topology.data?.l2_enabled ? "فعال" : "غیرفعال"}
                            good={topology.data?.l2_enabled}
                        />
                        <StatusTile
                            label="Bus"
                            value={topology.data?.bus_enabled ? "همگام" : "غیرفعال"}
                            good={topology.data?.bus_enabled}
                        />
                    </div>
                </div>
    ...
    return (
        <div className="space-y-5">
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
                <Metric
                    label="Policy"
                    value={fa(value?.counts.policies)}
                    hint={`${fa(value?.counts.enabled_policies)} policy فعال`}
                />
            ...
            <div className="grid gap-5 xl:grid-cols-[.8fr_1.2fr]">
                <Card className="p-5">
                    ...
                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                        <StatusTile
                            label="L1 memory"
                            value={value?.topology.l1_enabled ? "On" : "Off"}
                            good={value?.topology.l1_enabled}
                        />
                        <StatusTile
                            label="Redis L2"
                            value={value?.topology.l2_enabled ? "On" : "Off"}
                            good={value?.topology.l2_enabled}
                        />
                        <StatusTile
                            label="Invalidation bus"
                            value={value?.topology.bus_enabled ? "On" : "Off"}
                            good={value?.topology.bus_enabled}
                        />
                ...
                <Card className="overflow-hidden">
                    <div className="border-b p-5">
                        <SectionTitle
                            ...
                            side={
                                <Pill tone={value?.evidence.fresh ? "good" : "warn"}>
                                    {value?.evidence.fresh ? "Evidence تازه" : "Evidence قدیمی/ناموجود"}
                                </Pill>
                            }
                        />
                    </div>
            ...
            <div className="grid gap-5 xl:grid-cols-2">
                <HistoryCard title="آخرین پاکسازی‌ها" empty="هنوز purge event ثبت نشده است.">
                    {(value?.recent_purges ?? []).map((item) => (
                        <div
                            key={item.public_id}
                            className="grid gap-2 border-b p-4 last:border-b-0 md:grid-cols-[1fr_auto_auto] md:items-center"
                        >
                            <div>
                            ...
                            <Pill
                                tone={
                                    item.blast_radius === "broad" ? "danger" : item.blast_radius === "medium" ? "warn" : "neutral"
                                }
                            >
                                {item.blast_radius}
                            </Pill>
                            <Pill tone={item.status === "succeeded" ? "good" : item.status === "failed" ? "danger" : "neutral"}>
                                {item.status}
                            </Pill>
                        </div>
                    ))}
...
function PoliciesPanel() {
    ...
    const selected = useMemo(
        () => (list.data ?? []).find((row) => row.public_id === selectedId) ?? null,
            ...
            <Card className="overflow-hidden">
                <div className="border-b p-5">
                    ...
                    <Input
                        ...
                        placeholder="جست‌وجوی نام، key یا route…"
                    />
                </div>
                    ...
                    {(list.data ?? []).map((row) => (
                        ...
                        >
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                <Pill tone={row.risk_tier === "critical" || row.risk_tier === "high" ? "danger" : "neutral"}>
                                    {row.risk_tier}
                                </Pill>
                            </div>
                            <div className="mt-2 truncate font-mono text-muted-foreground text-xs" dir="ltr">
                                {row.route_pattern}
                            </div>
                            <div className="mt-2 text-muted-foreground text-xs">
                                TTL {fa(row.ttl_seconds)}s · v{fa(row.version)}
                            </div>
                        </button>
                    ))}
                    {(list.data?.length ?? 0) === 0 ? (
                        <div className="p-5">
                            <Empty>Policy ثبت نشده است.</Empty>
                        </div>
                    ) : null}
                </div>
            </Card>
        ...
        <Card className="p-5">
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
                <Field label="Policy key">
                    <Input
                        ...
                        onChange={(event) => setForm({ ...form, policy_key: event.target.value })}
                        dir="ltr"
                    />
                </Field>
                <Field label="نام">
                ...
                <Field label="نوع">
                    <SimpleSelect
                        value={form.kind}
                        values={["api", "page", "asset", "query"]}
                        onChange={(value) => setForm({ ...form, kind: value as typeof form.kind })}
                    />
                </Field>
                <Field label="وضعیت">
                    <SimpleSelect
                        value={form.status}
                        values={["disabled", "enabled", "archived"]}
                        onChange={(value) => setForm({ ...form, status: value as typeof form.status })}
                    />
                </Field>
                <Field label="ریسک">
                    <SimpleSelect
                        ...
                        onChange={(value) => setForm({ ...form, risk_tier: value as typeof form.risk_tier })}
                    />
                </Field>
                <Field label="Route pattern">
                    <Input
                        value={form.route_pattern}
                        onChange={(event) => setForm({ ...form, route_pattern: event.target.value })}
                        dir="ltr"
                    />
                </Field>
                <Field label="TTL (s)">
                    <Input
                        ...
                        onChange={(event) => setForm({ ...form, ttl_seconds: event.target.value })}
                    />
                </Field>
                <Field label="Grace (s)">
                    <Input
                        ...
                        onChange={(event) => setForm({ ...form, grace_seconds: event.target.value })}
                    />
                </Field>
                <Field label="Stale-if-error (s)">
                    <Input
                        type="number"
                        value={form.stale_if_error_seconds}
                        onChange={(event) => setForm({ ...form, stale_if_error_seconds: event.target.value })}
                    />
                </Field>
                <Field label="Soft / Hard timeout (ms)">
                    <div className="grid grid-cols-2 gap-2">
                        <Input
                            type="number"
                            value={form.soft_timeout_ms}
                            onChange={(event) => setForm({ ...form, soft_timeout_ms: event.target.value })}
                        />
                        <Input
                            type="number"
                            value={form.hard_timeout_ms}
                            onChange={(event) => setForm({ ...form, hard_timeout_ms: event.target.value })}
                        />
                    </div>
                </Field>
                <Field label="Registered tags">
                    <Input value={form.tags} onChange={(event) => setForm({ ...form, tags: event.target.value })} dir="ltr" />
                </Field>
                <Field label="Vary dimensions">
                    <Input value={form.vary} onChange={(event) => setForm({ ...form, vary: event.target.value })} dir="ltr" />
                </Field>
            </div>
            <Field label="توضیح">
                <Textarea
                        ...
                        onClick={async () => {
                            ...
                            try {
                                const result = await validate.mutateAsync({
                                    path: `policies/${policy.public_id}/validate`,
                                    body: {},
                                });
                                setMessage(
                                    result && "valid" in result && result.valid
                                        ? "Validation موفق بود."
                                        : "Validation با finding برگشت.",
                                );
                            } catch (caught) {
            ...
            {policy && "errors" in policy.validation ? (
                <div className="mt-5 space-y-2">
                    {(policy.validation.errors ?? []).map((item) => (
                        <Notice key={item.code} tone="danger">
                            {item.code}: {item.message}
                        </Notice>
                    ))}
                    {(policy.validation.warnings ?? []).map((item) => (
                        <Notice key={item.code} tone="warn">
                            {item.code}: {item.message}
                        </Notice>
                    ))}
                </div>
    ...
    return (
        <div className="grid gap-5 xl:grid-cols-[.8fr_1.2fr]">
            <Card className="p-5">
                <SectionTitle
                    title="Purge planner"
                    description="هیچ Redis key یا wildcard دستی پذیرفته نمی‌شود؛ فقط scopeهای ثبت‌شده به CacheTags واقعی resolve می‌شوند."
                />
                <div className="mt-5 space-y-4">
                    ...
                    {selectedScope?.target_required ? (
                        <Field label="Target">
                            <Input
                                value={target}
                                onChange={(event) => setTarget(event.target.value)}
                                dir="ltr"
                                placeholder={scope === "settings_group" ? "general" : "123"}
                            />
                        </Field>
                    ) : null}
                    <Field label="دلیل">
                    ...
                    {selectedScope?.broad ? (
                        <Notice tone="danger">
                            Full-tenant purge دارای blast radius گسترده است، global tenant registry را پاک نمی‌کند و در API به
                            permission و identity step-up نیاز دارد.
                        </Notice>
                    ) : null}
                    {error ? <Notice tone="danger">{error}</Notice> : null}
                    <div className="flex flex-wrap gap-2">
                        <Button type="button" variant="outline" onClick={doPlan} disabled={plan.isPending}>
                            Dry run / Plan
                        </Button>
                        <Button
                            type="button"
                            variant={selectedScope?.broad ? "destructive" : "default"}
                            onClick={doExecute}
                ...
                {planned ? (
                    <div className="mt-5 space-y-3 rounded-2xl border p-4">
                        <div className="flex flex-wrap items-center gap-2">
                            <Pill>{planned.mode}</Pill>
                            <Pill
                                tone={
                                    planned.blast_radius === "broad"
                                        ? "danger"
                                        : planned.blast_radius === "medium"
                                          ? "warn"
                                          : "good"
                                }
                            >
                            ...
                            <Pill
                                tone={
                                    planned.status === "succeeded" ? "good" : planned.status === "failed" ? "danger" : "neutral"
                                }
                            >
                                {planned.status}
                            </Pill>
                        ...
                        <div className="max-h-48 overflow-auto rounded-xl bg-muted/30 p-3 font-mono text-xs leading-6" dir="ltr">
                            {planned.resolved_tags.join("\n")}
                        </div>
                    </div>
                ) : null}
            </Card>
            <HistoryTable
                title="Purge history"
                rows={history.data ?? []}
                columns={["Scope", "Blast", "Mode", "Status", "زمان"]}
                rowKey={(item: LiteCashPurgeEvent) => item.public_id}
                render={(item: LiteCashPurgeEvent) => [
                    item.scope,
                    item.blast_radius,
                    item.mode,
                    item.status,
...
function WarmPanel() {
    ...
    const [form, setForm] = useState({
        scope: "catalog",
        target_key: "catalog-main",
        strategy: "cold_fill",
        priority: "normal",
        concurrency: "2",
        plan: "{}",
        reason: "Warm cache plan",
    });
    const [error, setError] = useState<string | null>(null);
    ...
    return (
        <div className="grid gap-5 xl:grid-cols-[.8fr_1.2fr]">
            <Card className="p-5">
                <SectionTitle
                    title="Warm / preload plan"
                    description="lite cash URL دلخواه را fetch نمی‌کند؛ plan نسخه‌پذیر می‌سازد و worker مورداعتماد progress واقعی را observe می‌کند."
                />
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                    <Field label="Scope">
                        <SimpleSelect
                            value={form.scope}
                            values={["catalog", "taxonomy", "storefront", "reports", "custom_registered"]}
                    ...
                    <Field label="Strategy">
                        <SimpleSelect
                            value={form.strategy}
                            values={["cold_fill", "refresh", "verify"]}
                            onChange={(value) => setForm({ ...form, strategy: value })}
                        />
                    </Field>
                ...
                <Notice tone="neutral">
                    Progress از UI ساخته نمی‌شود. discovered/processed/success/failure فقط از observation مورداعتماد worker تغییر
                    می‌کند.
                </Notice>
                {error ? (
                    <div className="mt-3">
            ...
            <Card className="overflow-hidden">
                <div className="border-b p-5">
                    <SectionTitle title="Warm queue" description="صف برنامه‌ریزی‌شده و evidence واقعی اجرای worker." />
                </div>
                <div className="max-h-[680px] divide-y overflow-auto">
                    {(jobs.data ?? []).map((job) => {
                        const progress =
                            job.discovered_count > 0 ? Math.min(100, (job.processed_count / job.discovered_count) * 100) : null;
                        return (
                            <div key={job.public_id} className="p-4">
                                <div className="flex flex-wrap items-center gap-2">
                                    <span className="font-medium text-sm">
                                        {job.scope} · {job.target_key}
                                    </span>
                                    <Pill>{job.strategy}</Pill>
                                    <Pill
                                        tone={
                                            job.status === "succeeded" ? "good" : job.status === "failed" ? "danger" : "neutral"
                                        }
                                    >
                                        {job.status}
                                    </Pill>
                                ...
                                <div className="mt-3 grid gap-2 text-xs sm:grid-cols-4">
                                    <span>کشف: {fa(job.discovered_count)}</span>
                                    <span>پردازش: {fa(job.processed_count)}</span>
                                    <span>موفق: {fa(job.success_count)}</span>
                                    <span>خطا: {fa(job.failure_count)}</span>
                                </div>
                                ...
                                <div className="mt-3 flex items-center justify-between gap-3">
                                    ...
                                    {!["succeeded", "partial", "failed", "cancelled"].includes(job.status) ? (
                                        <Button
                                            ...
                                            onClick={() =>
                                                cancel.mutate({
                                                    path: `warm-jobs/${job.public_id}/cancel`,
                                                    body: { reason: "لغو توسط اپراتور" },
                                                })
                                            }
                                        >
                                            لغو
                    ...
                    {(jobs.data?.length ?? 0) === 0 ? (
                        <div className="p-5">
                            <Empty>Warm job ثبت نشده است.</Empty>
                        </div>
                    ) : null}
                </div>
            </Card>
...
function OptimizationPanel() {
    ...
    const [selectedId, setSelectedId] = useState<string | null>(null);
    const selected = useMemo(
        () => (profiles.data ?? []).find((row) => row.public_id === selectedId) ?? null,
            ...
            <Card className="overflow-hidden">
                <div className="border-b p-5">
                    <SectionTitle
                        ...
                        description="پروفایل‌ها artifact هستند؛ اعمال واقعی CSS/JS/image/edge باید توسط adapter مورداعتماد evidence بدهد."
                        side={
                            <Button type="button" variant="outline" onClick={() => setSelectedId(null)}>
                                پروفایل جدید
                            </Button>
                ...
                <div className="divide-y">
                    {(profiles.data ?? []).map((profile) => (
                        <button
                            ...
                            type="button"
                            onClick={() => setSelectedId(profile.public_id)}
                            className={cn(
                                "block w-full p-4 text-start hover:bg-muted/30",
                                selectedId === profile.public_id && "bg-primary/5",
                            )}
                        >
                            <div className="flex flex-wrap items-center gap-2">
                                <span className="font-medium text-sm">{profile.name}</span>
                                <Pill tone={profile.status === "active" ? "good" : "neutral"}>{profile.status}</Pill>
                                <Pill tone={profile.mode === "aggressive" ? "warn" : "neutral"}>{profile.mode}</Pill>
                            </div>
                            <div className="mt-2 text-muted-foreground text-xs">
                                v{fa(profile.version)} · {dateTime(profile.updated_at)}
                            </div>
                        </button>
                    ))}
                </div>
...
function ProfileEditor({ profile }: { profile: LiteCashProfile | null }) {
    ...
    return (
        <Card className="p-5">
            <SectionTitle
                title={profile ? profile.name : "پروفایل جدید"}
                description="حالت safe پیش‌فرض محافظه‌کار است؛ unused CSS، interaction-delay و edge features بدون adapter مورداعتماد فقط intent باقی می‌مانند."
                side={
                    profile ? (
                        <Pill tone={profile.status === "active" ? "good" : "neutral"}>{profile.status}</Pill>
                    ) : (
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-3">
                <Field label="Profile key">
                    <Input
                        ...
                        onChange={(event) => setForm({ ...form, profile_key: event.target.value })}
                        dir="ltr"
                    />
                </Field>
                <Field label="نام">
                ...
                <Field label="Mode">
                    <SimpleSelect
                        ...
                        onChange={(value) => setForm({ ...form, mode: value as typeof form.mode })}
                    />
                </Field>
            </div>
            {form.mode === "aggressive" ? (
                <div className="mt-4">
                    <Notice tone="warn">
                        Aggressive profile می‌تواند layout، interaction یا LCP را خراب کند؛ activation نیازمند identity step-up
                        است.
                    </Notice>
                </div>
            ) : null}
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
                ...
                <JsonField
                    ...
                    onChange={(value) => setForm({ ...form, javascript: value })}
                />
                ...
                <JsonField
                    ...
                    onChange={(value) => setForm({ ...form, navigation: value })}
                />
            ...
            <div className="mt-5 flex flex-wrap gap-2">
                    ...
                    onClick={async () => {
                        ...
                        try {
                            ...
                            if (profile) await update.mutateAsync({ path: `profiles/${profile.public_id}`, body: payload });
                                ...
                                await create.mutateAsync({
                                    path: "profiles",
                                    body: { ...payload, profile_key: form.profile_key },
                                });
                        } catch (caught) {
                ...
                {profile && profile.status !== "active" ? (
                    <Button
                        ...
                        onClick={() =>
                            activate.mutate({ path: `profiles/${profile.public_id}/activate`, body: { reason: form.reason } })
                        }
                    >
                        Activate با Step-up
                    </Button>
...
function EdgePanel() {
    ...
    return (
        <div className="space-y-5">
            <Card className="p-5">
                <SectionTitle
                    title="Edge & object cache topology"
                    description="این صفحه secret management نیست؛ فقط readiness و intentهای provider-neutral را نشان می‌دهد."
                />
                <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    <Metric label="Driver" value={topology.data?.driver ?? "—"} hint="runtime cache driver" />
                    <Metric
                        label="Purge registry"
                        value={fa(topology.data?.registered_purge_scopes)}
                        hint="scopeهای allow-listed"
                    />
                    <Metric
                        label="Default TTL"
                        value={`${fa(topology.data?.runtime_defaults.ttl_seconds)}s`}
                        hint="tenant policy baseline"
                    />
                    <Metric
                        label="Evidence"
                        value={dateTime(topology.data?.last_observation_at)}
                        hint="آخرین trusted observation"
                    />
                </div>
            </Card>
            <Card className="overflow-hidden">
                <div className="border-b p-4 font-semibold">Capability matrix</div>
                <div className="overflow-auto">
                    <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-background">
                            <tr className="border-b text-muted-foreground">
                                <th className="p-3 text-start font-medium">قابلیت</th>
                                <th className="p-3 text-start font-medium">وضعیت</th>
                                <th className="p-3 text-start font-medium">مرز</th>
                            </tr>
                        </thead>
                        <tbody>
                            {rows.map(([name, state, note]) => (
                                <tr key={name} className="border-b last:border-0">
                                    ...
                                    <td className="p-3">
                                        <Pill tone={state === "ممنوع" ? "danger" : "neutral"}>{state}</Pill>
                                    </td>
            ...
            <Notice tone="neutral">
                Cloudflare/QUIC/custom فقط provider metadata هستند. API token، zone secret، Redis password و origin credential نه
                در DB ذخیره می‌شوند و نه به browser برمی‌گردند.
            </Notice>
        </div>
    );
}
...
function DiagnosticsPanel() {
    ...
    const validateImport = useLiteCashMutation<{
        valid: boolean;
        ...
        <div className="space-y-5">
            <Card className="p-5">
                <SectionTitle
                    ...
                    description="فقط runtime factهای safe؛ هیچ credential، host داخلی یا token در گزارش نیست."
                    side={<Pill tone={topology.data?.secrets_exposed ? "danger" : "good"}>redacted</Pill>}
                />
                <div className="mt-5 grid gap-3 md:grid-cols-3">
                    <StatusTile label="Driver" value={topology.data?.driver ?? "—"} />
                    <StatusTile label="Namespace" value={topology.data?.tenant_namespace ?? "—"} good />
                    <StatusTile label="Last evidence" value={dateTime(topology.data?.last_observation_at)} />
                </div>
            </Card>
            <div className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]">
                <Card className="overflow-hidden">
                    <div className="border-b p-4 font-semibold">Observation log</div>
                    <div className="max-h-[520px] overflow-auto">
                        <table className="w-full text-xs">
                            <thead className="sticky top-0 bg-background">
                                <tr className="border-b text-muted-foreground">
                                    <th className="p-3 text-start">Source</th>
                                    <th className="p-3 text-start">Metric</th>
                            ...
                            <tbody>
                                {(observations.data ?? []).map((row) => (
                                    <tr key={row.id} className="border-b">
                                        ...
                                        <td className="p-3">{dateTime(row.observed_at)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                ...
                <Card className="p-5">
                    <SectionTitle
                        ...
                        description="Snapshotها immutable هستند و secret ندارند."
                        side={
                            <Button
                                type="button"
                                variant="outline"
                                ...
                                onClick={() =>
                                    snapshot.mutate({
                                        path: "snapshots",
                                        body: { snapshot_kind: "manual", reason: "Manual operator snapshot" },
                                    })
                                }
                            >
                                Snapshot
                    ...
                    <div className="mt-4 space-y-2">
                        {(snapshots.data ?? []).slice(0, 10).map((row) => (
                            <div key={row.public_id} className="rounded-xl border p-3">
                                <div className="flex items-center justify-between gap-2">
                                    <Pill>{row.snapshot_kind}</Pill>
                                    <span className="text-muted-foreground text-xs">{dateTime(row.created_at)}</span>
                                </div>
                        ...
                        {(snapshots.data?.length ?? 0) === 0 ? <Empty>Snapshot وجود ندارد.</Empty> : null}
                    </div>
                </Card>
            </div>
            <Card className="p-5">
                <SectionTitle
                    title="Import / Export"
                    description="فقط settings، policies و optimization profiles. purge history، observationها و secretها export نمی‌شوند."
                />
                <Textarea
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                    <Button
                        type="button"
                        variant="outline"
                        onClick={() => setDocument(JSON.stringify(exportQuery.data ?? {}, null, 2))}
                    >
                        بارگذاری Export فعلی
                    </Button>
                    <Button
                        type="button"
                        variant="outline"
                        onClick={async () => {
                            ...
                            try {
                                const data = await validateImport.mutateAsync({
                                    ...
                                    body: { document: parseJson(document), reason: "Import validation" },
                                });
                                setResult(
                                    data.valid ? `Import معتبر است · ${data.fingerprint}` : `${data.errors.length} خطا پیدا شد.`,
                                );
                            } catch (caught) {
                                setError(caught instanceof Error ? caught.message : "Validation ناموفق بود.");
                            }
                        ...
                        onClick={async () => {
                            ...
                            try {
                                await applyImport.mutateAsync({
                                    path: "import/apply",
                                    body: { document: parseJson(document), reason: "Apply configuration import" },
                                });
                                setResult("Import به‌صورت atomic اعمال شد و snapshot قبل از تغییر ثبت شد.");
                            } catch (caught) {
                                setError(caught instanceof Error ? caught.message : "Import ناموفق بود.");
                            }
                        }}
                    >
                        Apply با Step-up
...
function SettingsEditor({ settings }: { settings: LiteCashSettings }) {
    ...
    return (
        <Card className="p-5">
            <SectionTitle
                title="تنظیمات lite cash"
                description="تغییر هم‌مقدار no-op است و history/audit اضافی تولید نمی‌کند."
                side={<Pill tone={settings.enabled ? "good" : "danger"}>{settings.enabled ? "Enabled" : "Disabled"}</Pill>}
            />
            <div className="mt-5 grid gap-5 lg:grid-cols-2">
                <ToggleRow
                    label="Master enable"
                    description="خاموش‌کردن control plane policyها را حذف نمی‌کند."
                    checked={form.enabled}
                    onChange={(checked) => setForm({ ...form, enabled: checked })}
                />
                <ToggleRow
                    label="Broad purge step-up"
                    description="Full-tenant purge نیازمند احراز هویت تکمیلی بماند."
                    checked={form.broad_purge_requires_step_up}
                    onChange={(checked) => setForm({ ...form, broad_purge_requires_step_up: checked })}
                />
                <Field label="Default TTL (s)">
                    <Input
                        type="number"
                        value={form.default_ttl_seconds}
                        onChange={(event) => setForm({ ...form, default_ttl_seconds: event.target.value })}
                    />
                </Field>
                <Field label="Max policy TTL (s)">
                    <Input
                        type="number"
                        value={form.max_policy_ttl_seconds}
                        onChange={(event) => setForm({ ...form, max_policy_ttl_seconds: event.target.value })}
                    />
                </Field>
                <Field label="Default grace (s)">
                    <Input
                        type="number"
                        value={form.default_grace_seconds}
                        onChange={(event) => setForm({ ...form, default_grace_seconds: event.target.value })}
                    />
                </Field>
                <Field label="Stale-if-error (s)">
                    <Input
                        type="number"
                        value={form.default_stale_if_error_seconds}
                        onChange={(event) => setForm({ ...form, default_stale_if_error_seconds: event.target.value })}
                    />
                </Field>
                <Field label="Max warm concurrency">
                    <Input
                        type="number"
                        min={1}
                        max={32}
                        value={form.max_warm_concurrency}
                        onChange={(event) => setForm({ ...form, max_warm_concurrency: event.target.value })}
                    />
                </Field>
                <Field label="Default optimization">
                    <SimpleSelect
                        ...
                        values={["safe", "balanced", "aggressive", "custom"]}
                        onChange={(value) => setForm({ ...form, default_profile: value as typeof form.default_profile })}
                    />
                </Field>
                <Field label="Edge provider metadata">
                    <SimpleSelect
                        value={form.edge_provider}
                        values={["none", "cloudflare", "quic", "custom"]}
                        onChange={(value) => setForm({ ...form, edge_provider: value as typeof form.edge_provider })}
                    />
                </Field>
                <Field label="Debug برای چند دقیقه">
                    <Input
                        type="number"
                        min={0}
                        max={1440}
                        value={form.debug_minutes}
                        onChange={(event) => setForm({ ...form, debug_minutes: event.target.value })}
                    />
                </Field>
            </div>
            {debugActive ? (
                <div className="mt-4">
                    <Notice tone="warn">Debug mode تا {dateTime(settings.debug_until)} فعال است و بعد خودکار منقضی می‌شود.</Notice>
                </div>
            ) : null}
            {!form.broad_purge_requires_step_up ? (
                <div className="mt-4">
                    <Notice tone="danger">غیرفعال‌کردن step-up برای broad purge ریسک عملیاتی بالایی دارد.</Notice>
                </div>
            ) : null}
            <Field label="دلیل تغییر">
                <Input
                ...
                onClick={async () => {
                    await update.mutateAsync({
                        ...
                        body: {
                            ...
                            default_ttl_seconds: Number(form.default_ttl_seconds),
                            default_grace_seconds: Number(form.default_grace_seconds),
                            default_stale_if_error_seconds: Number(form.default_stale_if_error_seconds),
                            max_policy_ttl_seconds: Number(form.max_policy_ttl_seconds),
                            max_warm_concurrency: Number(form.max_warm_concurrency),
                            broad_purge_requires_step_up: form.broad_purge_requires_step_up,
                            debug_minutes: Number(form.debug_minutes),
                            default_profile: form.default_profile,
                            edge_provider: form.edge_provider,
                            reason: form.reason,
                        },
                    });
                    setMessage("تنظیمات ذخیره شد و snapshot تغییر ثبت شد.");
                }}
            >
                ذخیره تنظیمات
            </Button>
    ...
    return (
        <Card className="overflow-hidden">
            ...
            <div className="max-h-[680px] overflow-auto">
                ...
                {rows.length === 0 ? (
                    <div className="p-5">
                        <Empty>داده‌ای ثبت نشده است.</Empty>
                    </div>
                ) : null}
            </div>
        </Card>
