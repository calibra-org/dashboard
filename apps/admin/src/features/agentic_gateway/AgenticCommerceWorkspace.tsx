...
import { Input } from "#/components/ui/inpur";
...
function Section({ title, help, children, action }: { title: string; help: string; children: ReactNode; action?: ReactNode }) {
    return <Card className="space-y-4 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><InfoTitle help={help}>{title}</InfoTitle>{action}</div>{children}</Card>;
}
function Metric({ label, help, value, hint }: { label: string; help: string; value: ReactNode; hint?: string }) {
    return <Card className="p-4"><div className="flex items-center gap-2 text-sm text-muted-foreground"><span>{label}</span><HelperTooltip>{help}</HelperTooltip></div><div className="mt-3 text-2xl font-semibold tracking-tight">{value}</div>{hint ? <p className="mt-1 text-xs text-muted-foreground">{hint}</p> : null}</Card>;
}
...
export function AgenticCommerceWorkspace({ section }: { section: AgenticGatewaySection }) {
    ...
    if (section === "overview") {
        ...
        return <div className="space-y-5"><PageHeader title={t("title")} subtitle={t("subtitle")} />
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                ...
                <Metric label="کانال Live" help="فقط کانالی که conformance موفق داشته و صریحاً Live شده است." value={k.live_channels ?? "—"} />
                <Metric label="Agent فعال" help="principalهای Agent که scope و وضعیت فعال دارند؛ بدون نمایش credential خام." value={k.active_principals ?? "—"} />
                <Metric label="رویداد ۳۰ روز" help="فقط eventهای واقعی ثبت‌شده در ledger کانال؛ metric ساختگی تولید نمی‌شود." value={k.events_30d ?? "—"} />
            ...
            <Section title="مرز اجرایی Agentic Commerce" help="Core commerce protocol-neutral می‌ماند؛ adapterها فقط ترجمه قرارداد هستند.">
                <div className="grid gap-3 md:grid-cols-3"><div className="rounded-xl border border-border p-4"><Link2 className="size-5"/><h3 className="mt-3 font-medium">Protocol adapters</h3><p className="mt-1 text-sm text-muted-foreground">UCP / ACP / MCP / A2A فقط پس از conformance evidence فعال می‌شوند.</p></div><div className="rounded-xl border border-border p-4"><ChartNoAxesCombined className="size-5"/><h3 className="mt-3 font-medium">Canonical product graph</h3><p className="mt-1 text-sm text-muted-foreground">محصول، قیمت و موجودی از دامنه‌های فعلی خوانده می‌شوند؛ truth موازی ساخته نمی‌شود.</p></div><div className="rounded-xl border border-border p-4"><ShieldCheck className="size-5"/><h3 className="mt-3 font-medium">Merchant control</h3><p className="mt-1 text-sm text-muted-foreground">scope، rate limit، policy، audit و kill switch قبل از mutation.</p></div></div>
            </Section>
        </div>;
    }
    ...
    if (section === "channels") return <div className="space-y-5"><PageHeader title="کانال‌ها و قابلیت‌ها" subtitle="تعریف کانال protocol-neutral، نسخه قابلیت و مرز دسترسی Agent" />
        <Section title="ثبت یا ویرایش کانال" help="Live شدن کانال بدون conformance PASS در Backend رد می‌شود.">
            <div className="grid gap-4 md:grid-cols-2"><div><Label>کلید کانال</Label><Input value={channelKey} onChange={(e)=>setChannelKey(e.target.value)} placeholder="google-ai-mode" /></div><div><Label>نام نمایشی</Label><Input value={channelName} onChange={(e)=>setChannelName(e.target.value)} placeholder="Google AI Mode" /></div><div><Label>Adapter</Label><Select value={adapterKey} onValueChange={setAdapterKey}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{["native","ucp","acp","mcp","a2a","custom"].map(v=><SelectItem key={v} value={v}>{v.toUpperCase()}</SelectItem>)}</SelectContent></Select></div><div><Label>حالت</Label><Select value={mode} onValueChange={setMode}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="disabled">غیرفعال</SelectItem><SelectItem value="shadow">سایه</SelectItem><SelectItem value="read_only">فقط‌خواندنی</SelectItem><SelectItem value="live">Live</SelectItem></SelectContent></Select></div><div><Label>نسخه پروتکل</Label><Input value={protocolVersion} onChange={(e)=>setProtocolVersion(e.target.value)} placeholder="2026-04-08" /></div><div><Label>دلیل تغییر</Label><Input value={reason} onChange={(e)=>setReason(e.target.value)} placeholder="حداقل ۸ کاراکتر" /></div></div>
    ...
    if (section === "readiness") return <div className="space-y-5"><PageHeader title="آمادگی محصولات برای Agent" subtitle="امتیاز قابل توضیح، missing facts و freshness برای هر محصول" />
        ...
        <Section title="محصولات نیازمند تکمیل" help="کمترین readiness در بالا قرار می‌گیرد تا مسیر اصلاح روشن باشد."><State loading={readiness.isLoading} error={readiness.error} empty={!readiness.data?.length}><div className="space-y-3">{readiness.data?.map((row:any)=><div key={row.product_id} className="rounded-xl border border-border p-4"><div className="flex items-center justify-between gap-3"><div><div className="font-medium">{row.name ?? `محصول #${row.product_id}`}</div><div className="text-xs text-muted-foreground">SKU: {row.sku ?? "—"}</div></div><span className="font-mono text-sm">{(row.score_bp/100).toFixed(1)}٪</span></div><Progress className="mt-3" value={row.score_bp/100}/><div className="mt-3 flex flex-wrap gap-2">{(row.missing_facts ?? []).map((x:string)=><span key={x} className="rounded-full border border-border px-2 py-1 text-xs text-muted-foreground">{x}</span>)}</div></div>)}</div></State></Section>
    </div>;
