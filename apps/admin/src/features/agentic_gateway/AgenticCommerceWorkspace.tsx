...
function State({
    ...
    if (loading)
        return (
            <div className="rounded-lg border border-border p-8 text-center text-muted-foreground">در حال دریافت داده واقعی…</div>
        );
    if (error)
        return (
...
export function AgenticCommerceWorkspace({ section }: { section: AgenticGatewaySection }) {
            ...
            <div className="space-y-5">
                ...
                <State loading={overview.isLoading} error={overview.error}>
                    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                        ...
                        <Metric
                            label="کانال Live"
                            help="فقط کانال دارای conformance موفق و تازه."
                            value={k.live_channels ?? "—"}
                        />
                        <Metric
                            label="Agent فعال"
                            help="principalهای فعال با scope و rate limit واقعی."
                            value={k.active_principals ?? "—"}
                        />
                        <Metric
                            label="رویداد ۳۰ روز"
                            help="رویدادهای واقعی ledger، بدون KPI ساختگی."
                            value={k.events_30d ?? "—"}
                        />
                        <Metric
                            label="آمادگی محصول"
                            help="امتیاز facts-based با missing facts صریح."
                            value={k.avg_readiness_bp == null ? "—" : `${(k.avg_readiness_bp / 100).toFixed(1)}٪`}
                        />
                        <Metric
                            label="Policy block"
                            help="Actionهایی که policy یا Trust/Risk رد کرده است."
                            value={k.policy_blocks_30d ?? "—"}
                        />
                    </div>
                </State>
                <Section
                    title="مرز اجرایی Agentic Commerce"
                    help="هسته commerce منبع حقیقت می‌ماند و adapter فقط قرارداد را ترجمه می‌کند."
                >
                    <div className="grid gap-3 md:grid-cols-3">
                        <div className="rounded-xl border border-border p-4">
                            <Link2 className="size-5" />
                            <h3 className="mt-3 font-medium">Protocol adapters</h3>
                            <p className="mt-1 text-sm text-muted-foreground">فعال‌سازی فقط پس از conformance evidence.</p>
                        </div>
                        <div className="rounded-xl border border-border p-4">
                            <ChartNoAxesCombined className="size-5" />
                            <h3 className="mt-3 font-medium">Canonical graph</h3>
                            <p className="mt-1 text-sm text-muted-foreground">
                                Catalog، قیمت و موجودی از سرویس‌های موجود خوانده می‌شوند.
                            </p>
                        </div>
                        <div className="rounded-xl border border-border p-4">
                            <ShieldCheck className="size-5" />
                            <h3 className="mt-3 font-medium">Merchant control</h3>
                            <p className="mt-1 text-sm text-muted-foreground">
                                Scope، rate limit، Trust/Risk، audit و kill switch قبل از mutation.
                            </p>
                        </div>
                    </div>
                </Section>
    ...
    if (section === "channels") {
        return (
            <div className="space-y-5">
                <PageHeader
                    title="کانال‌ها، Agentها و قابلیت‌ها"
                    subtitle="کنترل protocol، principal، scope، rate limit، capability و action ledger"
                />
                <Section title="کانال" help="Live شدن کانال بدون conformance PASS تازه در Backend رد می‌شود.">
                    <div className="grid gap-4 md:grid-cols-2">
                        <div>
                            <Label>کلید کانال</Label>
                            <Input value={channelKey} onChange={(event) => setChannelKey(event.target.value)} />
                        </div>
                        <div>
                        ...
                        <div>
                            <Label>Adapter</Label>
                            <Select value={adapterKey} onValueChange={(value) => setAdapterKey(String(value))}>
                                <SelectTrigger>
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                    {["native", "ucp", "acp", "mcp", "a2a", "custom"].map((value) => (
                        ...
                        <div>
                            <Label>حالت</Label>
                            <Select value={mode} onValueChange={(value) => setMode(String(value))}>
                                <SelectTrigger>
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                    {["disabled", "shadow", "read_only", "live"].map((value) => (
                                        <SelectItem key={value} value={value}>
                                            {value}
                                        </SelectItem>
                                    ))}
                        ...
                        <div>
                            <Label>نسخه پروتکل</Label>
                            <Input value={protocolVersion} onChange={(event) => setProtocolVersion(event.target.value)} />
                        </div>
                        <div>
                    ...
                    <Button
                        disabled={!channelKey || !channelName || reason.length < 8 || saveChannel.isPending}
                        onClick={() =>
                            saveChannel.mutate({
                                path: "channels",
                                body: {
                ...
                <Section
                    ...
                    help="فقط SHA-256 fingerprint ذخیره می‌شود و secret خام در Admin یا DB نگهداری نمی‌شود."
                >
                    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                        <div>
                            <Label>Principal key</Label>
                            <Input value={principalKey} onChange={(event) => setPrincipalKey(event.target.value)} />
                        </div>
                        <div>
                            <Label>نام نمایشی</Label>
                        ...
                        <div>
                            <Label>وضعیت</Label>
                            <Select value={principalStatus} onValueChange={(value) => setPrincipalStatus(String(value))}>
                                <SelectTrigger>
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                    {["disabled", "shadow", "active", "revoked"].map((value) => (
                                        <SelectItem key={value} value={value}>
                                            {value}
                                        </SelectItem>
                                    ))}
                        ...
                        <div className="xl:col-span-2">
                            <Label>Scopeها</Label>
                            <Input value={principalScopes} onChange={(event) => setPrincipalScopes(event.target.value)} />
                        </div>
                        <div>
                            <Label>SHA-256 fingerprint</Label>
                            <Input
                                value={credentialFingerprint}
                                onChange={(event) => setCredentialFingerprint(event.target.value)}
                            />
                        </div>
                        <div>
                            ...
                            <Input
                                value={rateWindow}
                                onChange={(event) => setRateWindow(event.target.value)}
                                inputMode="numeric"
                            />
                        </div>
                        <div>
                            <Label>Max actions</Label>
                            <Input value={rateMax} onChange={(event) => setRateMax(event.target.value)} inputMode="numeric" />
                        </div>
                        <div>
                            <Label>دلیل تغییر</Label>
                            <Input value={principalReason} onChange={(event) => setPrincipalReason(event.target.value)} />
                        </div>
                    </div>
                    <Button
                        disabled={!principalKey || !principalName || principalReason.length < 8 || savePrincipal.isPending}
                        onClick={() =>
                            savePrincipal.mutate({
                                path: "principals",
                                body: {
                                    ...
                                    credential_fingerprint: credentialFingerprint
                                        ? `sha256:${credentialFingerprint.replace(/^sha256:/i, "").trim()}`
                                        : null,
                    ...
                    <State loading={channels.isLoading} error={channels.error} empty={!channels.data?.principals?.length}>
                        <div className="grid gap-3 lg:grid-cols-2">
                            {channels.data?.principals?.map((row: any) => {
                                ...
                                return (
                                    <div key={row.public_id} className="rounded-xl border border-border p-4">
                                        <div className="flex items-center justify-between gap-3">
                                            <div>
                                                <div className="font-medium">{row.display_name}</div>
                                                <div className="text-xs text-muted-foreground">{row.principal_key}</div>
                                            </div>
                                            <StatusBadge
                                                tone={
                                                    row.status === "active"
                                        ...
                                        <div className="mt-3 text-xs text-muted-foreground">
                                            Scope: {arrayValue(row.scopes).join("، ") || "—"}
                                        </div>
                                        <div className="mt-1 text-xs text-muted-foreground">
                                            Rate: {String(policy.max_actions ?? 0)} / {String(policy.window_seconds ?? 0)}s
                                        </div>
                                    </div>
                                );
                            })}
                ...
                <Section title="Capability version" help="Schema، scope، risk class، digest و signature برای هر نسخه ثبت می‌شود.">
                    <div className="grid gap-4 md:grid-cols-2">
                        <div>
                            <Label>Channel Public ID</Label>
                            <Input value={channelPublicId} onChange={(event) => setChannelPublicId(event.target.value)} />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <Select value={riskClass} onValueChange={(value) => setRiskClass(String(value))}>
                                <SelectTrigger>
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                    {["read_only", "low", "medium", "high", "critical"].map((value) => (
                                        <SelectItem key={value} value={value}>
                                            {value}
                                        </SelectItem>
                                    ))}
                    ...
                    <Button
                        disabled={!channelPublicId || !capability || createCapability.isPending}
                        onClick={() =>
                            createCapability.mutate({
                                ...
                                body: {
                                    ...
                                    reason: reason || "ثبت نسخه قابلیت Agentic",
                                },
                            })
                        }
                    >
                ...
                <Section title="کانال‌های ثبت‌شده" help="Mode واقعی Backend نمایش داده می‌شود.">
                    <State loading={channels.isLoading} error={channels.error} empty={!channels.data?.channels?.length}>
                        <div className="space-y-2">
                            {channels.data?.channels?.map((row: any) => (
                                <div
                                    key={row.public_id}
                                ...
                                >
                                    <div>
                                        ...
                                        <div className="text-xs text-muted-foreground">
                                            {row.channel_key} · {row.adapter_key}
                                        </div>
                                    </div>
                                    <StatusBadge
                                        tone={row.mode === "live" ? "success" : row.mode === "disabled" ? "neutral" : "warning"}
                                    >
                                        {row.mode}
                                    </StatusBadge>
                                </div>
                            ...
                            <TableBody>
                                {actions.data?.map((row: any) => (
                                    <TableRow key={row.public_id}>
                                        ...
                                        <TableCell>{row.principal_key ?? "—"}</TableCell>
                                        <TableCell>{row.capability_key}</TableCell>
                                        ...
                                        <TableCell>{String(objectValue(row.policy_result).reason ?? "—")}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
            ...
            <div className="space-y-5">
                ...
                <Section title="ارزیابی محصول" help="امتیاز فقط از facts واقعی catalog و inventory ساخته می‌شود.">
                    <div className="flex flex-wrap items-end gap-3">
                        ...
                        <Button
                            disabled={!productId || refreshReadiness.isPending}
                            onClick={() =>
                                refreshReadiness.mutate({
                                    path: "readiness/refresh",
                                    body: { product_id: Number(productId), locale: "fa" },
                                })
                            }
                        >
                ...
                <Section title="محصولات نیازمند تکمیل" help="کمترین readiness در بالا قرار می‌گیرد.">
                    <State loading={readiness.isLoading} error={readiness.error} empty={!readiness.data?.length}>
                        <div className="space-y-3">
                            {readiness.data?.map((row: any) => (
                                <div key={row.product_id} className="rounded-xl border border-border p-4">
                                    <div className="flex items-center justify-between gap-3">
                                        <div>
                                            <div className="font-medium">{row.name ?? `محصول #${row.product_id}`}</div>
                                            <div className="text-xs text-muted-foreground">SKU: {row.sku ?? "—"}</div>
                                        </div>
                                        <span className="font-mono text-sm">{(row.score_bp / 100).toFixed(1)}٪</span>
                                    </div>
                                    <Progress className="mt-3" value={row.score_bp / 100} />
                                    <div className="mt-3 flex flex-wrap gap-2">
                                        {(row.missing_facts ?? []).map((fact: string) => (
                                            <span
                                                key={fact}
    ...
    return (
        <div className="space-y-5">
            ...
            <Section
                title="اجرای Conformance"
                help="Schema، signature، protocol version و kill switch deterministic کنترل می‌شوند."
            >
                <div className="flex flex-wrap items-end gap-3">
                    ...
                    <Button
                        disabled={!channelPublicId || runConformance.isPending}
                        onClick={() =>
                            runConformance.mutate({
                                path: "conformance",
                                body: {
                                    channel_public_id: channelPublicId,
                                    reason: reason || "اجرای کنترل انطباق قبل از انتشار",
                                },
                            })
                        }
                    >
            ...
            <Section title="آخرین Runها" help="PASS، FAIL و BLOCKED مستقیماً از Backend می‌آیند.">
                <State loading={channels.isLoading} error={channels.error} empty={!channels.data?.conformance?.length}>
                    <Table>
                        ...
                        <TableBody>
                            {channels.data?.conformance?.map((row: any) => (
                                <TableRow key={row.public_id}>
                                    <TableCell>{row.adapter_key}</TableCell>
                                    <TableCell>{row.protocol_version ?? "—"}</TableCell>
                                    <TableCell>
                                        <StatusBadge
                                            tone={
                                                row.status === "pass" ? "success" : row.status === "fail" ? "danger" : "warning"
                                            }
                                        >
                                            {row.status}
                                        </StatusBadge>
                                    </TableCell>
                                    <TableCell>{row.ran_at ? new Date(row.ran_at).toLocaleString("fa-IR") : "—"}</TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
