            ...
            <section className="mx-auto max-w-[1600px] space-y-5">
                <header className="overflow-hidden rounded-[28px] border border-white/10 bg-slate-950 p-6 text-white shadow-2xl md:p-8">
                    <div className="flex flex-wrap items-end justify-between gap-5">
                        <div>
                            <p className="font-semibold text-info text-xs tracking-[.25em]">CALIBRA · PHASE 14</p>
                            <h1 className="mt-2 font-black text-3xl md:text-5xl">Procurement & Supplier Intelligence OS</h1>
                        ...
                        <div className="rounded-2xl border border-success/20 bg-success/10 px-4 py-3 text-success">
                            ...
                            <div className="font-black text-2xl">{d.kpis.average_supplier_score}%</div>
                        </div>
                    </div>
                </header>
                <div className="grid gap-3 md:grid-cols-5">
                    {[
                        ...
                        <div key={String(l)} className="rounded-3xl border bg-background/90 p-5 shadow-sm backdrop-blur">
                            ...
                            <div className="mt-2 font-black text-2xl tabular-nums">{v}</div>
                        </div>
                    ))}
                </div>
                <nav className="flex gap-2 overflow-auto rounded-2xl border bg-background/80 p-2 backdrop-blur">
                    {[
                        ...
                        <button
                            ...
                            className={`whitespace-nowrap rounded-xl px-4 py-2 font-bold text-sm transition ${tab === k ? "bg-slate-950 text-white shadow-lg" : "hover:bg-muted"}`}
                        >
                            {l}
                        </button>
                    ))}
                ...
                {tab === "command" && (
                    <div className="grid gap-4 xl:grid-cols-[1.25fr_.75fr]">
                        <section className="rounded-[28px] border bg-background/90 p-6 shadow-sm">
                            <h2 className="font-black text-lg">Supplier Score Landscape</h2>
                            <p className="text-muted-foreground text-xs">امتیاز مرکب قابل توضیح؛ نه یک عدد مبهم.</p>
                        ...
                        {d.suppliers.map((s) => (
                            <article key={s.id} className="rounded-[28px] border bg-background/90 p-5 shadow-sm">
                                <div className="flex items-start justify-between">
                                    ...
                                    <div className="rounded-xl bg-success/10 px-3 py-2 font-black text-success">
                                        {s.score?.composite}%
                                    </div>
                                </div>
                                ...
                                <tbody>
                                    {d.purchase_orders.map((po) => (
                                        <tr key={po.id} className="border-t">
                                            ...
                                            <td className="p-4">
                                                <span className="rounded-full bg-info/10 px-2 py-1 text-info text-xs">
                                                    {po.status}
                                                </span>
                                            </td>
                ...
                {tab === "recommendations" && (
                    <div className="grid gap-4 lg:grid-cols-2">
                        {recommendations.map((r) => (
                            <article key={String(r.id)} className="rounded-[28px] border bg-background/90 p-5 shadow-sm">
                                <div className="flex justify-between gap-4">
                                    ...
                                    <div className="text-left">
                                        <div className="font-black text-2xl">{r.proposed_quantity}</div>
