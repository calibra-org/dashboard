...
- **Same-origin admin proxy.** `src/app/api/admin/[...path]/route.ts` proxies GET/POST/PUT/PATCH/DELETE to `/api/v1/admin/<path>`. It reads the httpOnly `admin_session` cookie server-side, attaches `Authorization: Bearer …`, resolves and forwards the tenant header, and forwards `Accept-Language`. Browser mutations must use the proxy through `lib/queries/api-client.ts`; the proxy enforces the `admin_csrf` double-submit token and forwards `If-Match` and `Idempotency-Key` when supplied. The bearer never reaches client JavaScript. Upstream 401 clears both session and CSRF cookies; 403 preserves the authenticated session.
