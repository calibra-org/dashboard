...
test.group("Phase 11 Governance RLS isolation", (group) => {
    group.setup(async () => {
        ...
        await bootstrapRoles(su, { appUser: APP_USER, appPassword: APP_PASSWORD, adminUser: ADMIN_USER, adminPassword: ADMIN_PASSWORD });
        ...
        for (const [id, slug] of [[TENANT_A, "phase11-rls-a"], [TENANT_B, "phase11-rls-b"]] as const) {
            await su.table("tenants").insert({ id, slug, name: slug, status: "active", plan_id: Number(plan.id), db_tier: "shared", template_key: "default", currency_code: "IRR", primary_locale: "fa", created_at: now, updated_at: now }).onConflict("id").ignore();
        }
        const row = (tenantId: number, key: string) => ({ tenant_id: tenantId, policy_key: key, version: 1, name: key, action_pattern: "configuration.apply", effect: "deny", priority: 100, reason: "rls isolation evidence", content_hash: String(tenantId).padEnd(64, "1").slice(0, 64), created_at: now });
        await su.table("governance_policy_versions").insert([row(TENANT_A, "rls.a1"), row(TENANT_A, "rls.a2"), row(TENANT_B, "rls.b1")]);
        db.manager.add(APP_CONNECTION, {
            client: "pg",
            connection: { host: env.get("DB_HOST"), port: env.get("DB_PORT"), user: APP_USER, password: APP_PASSWORD, database: env.get("DB_DATABASE") },
        });
        return async () => {
    ...
    test("all Governance OS tenant tables FORCE RLS", async ({ assert }) => {
        const rows = await db.rawQuery("SELECT relname, relrowsecurity, relforcerowsecurity FROM pg_class WHERE relname = ANY(?) ORDER BY relname", [TABLES]);
    ...
    test("calibra_app fails closed without tenant context and sees only selected tenant", async ({ assert }) => {
        ...
        const countFor = (tenantId: number) => app.transaction(async (trx) => {
    ...
    test("WITH CHECK rejects a cross-tenant governance write", async ({ assert }) => {
        ...
        try {
            await app.transaction(async (trx) => {
                ...
                await trx.table("governance_policy_versions").insert({ tenant_id: TENANT_B, policy_key: "rls.must-fail", version: 1, name: "must fail", action_pattern: "configuration.apply", effect: "deny", priority: 100, reason: "wrong tenant", content_hash: "f".repeat(64) });
            });
        } catch {
            rejected = true;
        }
