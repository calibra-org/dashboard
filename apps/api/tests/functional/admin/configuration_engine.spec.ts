...
test.group("Phase 6 Configuration OS engine", (group) => {
    ...
    test("resolves Global → Tenant → Market → Channel → Environment → Temporary and reports origin", async ({
        client,
        assert,
    }) => {
        ...
        const item = response
            ...
            .data.definitions.find(
                (candidate: { definition: { key: string } }) => candidate.definition.key === "reading.feed_page_size",
            );
    ...
    test("secret-class values reject plaintext and blueprints expose references without secret material", async ({
        client,
        assert,
    }) => {
        const admin = await createAdmin();
    ...
    test("configuration tables are FORCE RLS protected and tax simulator fails closed on invalid input", async ({
        client,
        assert,
    }) => {
        const admin = await createAdmin();
