...
export default class extends BaseSchema {
    async up() {
        this.schema.createTable("suppliers", (t) => {
            ...
            t.string("legal_name", 220).notNullable();
            t.string("display_name", 220).notNullable();
            t.string("status", 24).notNullable().defaultTo("active");
            t.string("email", 254).nullable();
            t.string("phone", 64).nullable();
            t.string("currency", 3).notNullable().defaultTo("IRR");
            t.string("payment_terms", 120).nullable();
            t.integer("default_lead_time_days").nullable();
            t.decimal("lead_time_variance_days", 10, 3).nullable();
            t.decimal("fill_rate", 8, 6).nullable();
            t.decimal("on_time_rate", 8, 6).nullable();
            t.decimal("quality_rate", 8, 6).nullable();
            t.decimal("cost_score", 8, 6).nullable();
            t.decimal("responsiveness_score", 8, 6).nullable();
            t.decimal("dependency_risk", 8, 6).nullable();
            t.string("criticality", 16).notNullable().defaultTo("normal");
            t.jsonb("metadata").notNullable().defaultTo(this.raw("'{}'::jsonb"));
            t.timestamps(true, true);
            t.unique(["tenant_id", "code"]);
            t.index(["tenant_id", "status", "criticality"]);
        });
        this.schema.createTable("supplier_products", (t) => {
            t.bigIncrements("id");
            t.bigInteger("tenant_id").unsigned().notNullable().references("id").inTable("tenants").onDelete("CASCADE");
            t.bigInteger("supplier_id").unsigned().notNullable().references("id").inTable("suppliers").onDelete("CASCADE");
            t.bigInteger("product_id").unsigned().notNullable().references("id").inTable("products").onDelete("CASCADE");
            t.bigInteger("variation_id")
                .unsigned()
                .nullable()
                .references("id")
                .inTable("product_variations")
                .onDelete("SET NULL");
            t.string("supplier_sku", 190).nullable();
            t.decimal("unit_cost", 20, 4).nullable();
            t.string("currency", 3).notNullable().defaultTo("IRR");
            t.decimal("moq", 18, 4).notNullable().defaultTo(1);
            t.decimal("order_multiple", 18, 4).notNullable().defaultTo(1);
            t.integer("lead_time_days").nullable();
            t.boolean("preferred").notNullable().defaultTo(false);
            t.boolean("active").notNullable().defaultTo(true);
            t.timestamps(true, true);
            t.index(["tenant_id", "supplier_id", "active"]);
            t.index(["tenant_id", "product_id", "variation_id"]);
        });
        this.schema.createTable("purchase_orders", (t) => {
            ...
            t.string("number", 80).notNullable();
            ...
            t.bigInteger("fees_minor").notNullable().defaultTo(0);
            t.bigInteger("total_minor").notNullable().defaultTo(0);
            t.bigInteger("planning_recommendation_id")
                .unsigned()
                .nullable()
                .references("id")
                .inTable("planning_replenishment_recommendations")
                .onDelete("SET NULL");
            t.string("idempotency_key", 160).nullable();
            t.jsonb("impact_snapshot").notNullable().defaultTo(this.raw("'{}'::jsonb"));
            t.bigInteger("created_by_user_id").unsigned().nullable().references("id").inTable("users").onDelete("SET NULL");
            t.bigInteger("approved_by_user_id").unsigned().nullable().references("id").inTable("users").onDelete("SET NULL");
            t.timestamp("approved_at", { useTz: true }).nullable();
            t.timestamps(true, true);
            t.unique(["tenant_id", "number"]);
            t.unique(["tenant_id", "idempotency_key"]);
            t.index(["tenant_id", "status", "expected_date"]);
        });
        this.schema.createTable("purchase_order_lines", (t) => {
            ...
            t.bigInteger("purchase_order_id")
                ...
                .onDelete("CASCADE");
            t.bigInteger("product_id").unsigned().notNullable().references("id").inTable("products").onDelete("RESTRICT");
            t.bigInteger("variation_id")
                .unsigned()
                .nullable()
                .references("id")
                .inTable("product_variations")
                .onDelete("SET NULL");
            t.string("sku_snapshot", 190).nullable();
            t.string("name_snapshot", 255).notNullable();
            t.decimal("ordered_quantity", 18, 4).notNullable();
            t.decimal("received_quantity", 18, 4).notNullable().defaultTo(0);
            t.decimal("accepted_quantity", 18, 4).notNullable().defaultTo(0);
            t.decimal("rejected_quantity", 18, 4).notNullable().defaultTo(0);
            t.decimal("quarantine_quantity", 18, 4).notNullable().defaultTo(0);
            t.decimal("unit_cost", 20, 4).notNullable();
            t.bigInteger("line_total_minor").notNullable();
            t.date("expected_date").nullable();
            t.timestamps(true, true);
        ...
        this.schema.createTable("purchase_order_receipts", (t) => {
            ...
            t.bigInteger("received_by_user_id").unsigned().nullable().references("id").inTable("users").onDelete("SET NULL");
            t.timestamps(true, true);
            t.unique(["tenant_id", "number"]);
            t.unique(["tenant_id", "idempotency_key"]);
        });
        this.schema.createTable("purchase_order_receipt_lines", (t) => {
            ...
            t.bigInteger("receipt_id")
                ...
                .notNullable()
                .references("id")
                .inTable("purchase_order_receipts")
                .onDelete("CASCADE");
            t.bigInteger("purchase_order_line_id")
                .unsigned()
                .notNullable()
                .references("id")
                .inTable("purchase_order_lines")
                .onDelete("RESTRICT");
            t.decimal("received_quantity", 18, 4).notNullable();
            t.decimal("accepted_quantity", 18, 4).notNullable();
            t.decimal("rejected_quantity", 18, 4).notNullable().defaultTo(0);
            t.decimal("quarantine_quantity", 18, 4).notNullable().defaultTo(0);
            t.string("quality_reason", 240).nullable();
            t.string("lot_code", 120).nullable();
            t.string("batch_code", 120).nullable();
            t.jsonb("serials").notNullable().defaultTo(this.raw("'[]'::jsonb"));
        ...
        this.schema.createTable("supplier_incidents", (t) => {
            ...
            t.timestamps(true, true);
            t.index(["tenant_id", "supplier_id", "status"]);
        });
        for (const sql of [
