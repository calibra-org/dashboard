...
export default class extends BaseSchema {
    async up() {
        ...
        this.schema.raw(
            "ALTER TABLE content_scheduler_runs ALTER COLUMN tenant_id SET DEFAULT NULLIF(current_setting('app.current_tenant', true), '')::bigint",
        );
        ...
        this.schema.raw(
            "ALTER TABLE content_scheduler_runs ADD CONSTRAINT content_scheduler_runs_kind_check CHECK (job_kind IN ('publish_due','ingest_due'))",
        );
        this.schema.raw(
            "ALTER TABLE content_scheduler_runs ADD CONSTRAINT content_scheduler_runs_status_check CHECK (status IN ('running','completed','failed','skipped'))",
        );
        this.schema.raw(
            "ALTER TABLE content_scheduler_runs ADD CONSTRAINT content_scheduler_runs_count_check CHECK (processed_count >= 0)",
        );
    }
