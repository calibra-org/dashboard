...
export default class extends BaseSchema {
    async up() {
        ...
        this.schema.raw(
            "ALTER TABLE fulfillment_capacity_holds ADD CONSTRAINT fulfillment_capacity_hold_units_check CHECK (units > 0)",
        );
        this.schema.raw(
            "ALTER TABLE fulfillment_capacity_holds ADD CONSTRAINT fulfillment_capacity_hold_status_check CHECK (status IN ('held','committed','released','expired'))",
        );
        this.schema.raw(
            `ALTER TABLE fulfillment_capacity_holds ALTER COLUMN tenant_id SET DEFAULT NULLIF(current_setting('app.current_tenant', true), '')::bigint`,
        );
        ...
        this.schema.raw(
            `CREATE POLICY fulfillment_capacity_holds_tenant_isolation ON fulfillment_capacity_holds USING (${TENANT}) WITH CHECK (${TENANT})`,
        );
    }
