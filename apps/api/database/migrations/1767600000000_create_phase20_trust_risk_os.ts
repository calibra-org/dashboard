...
export default class extends BaseSchema {
    async up() {
        ...
        this.schema.createTable("fraud_risk_model_versions", (t) => {
            t.bigIncrements("id");
            t.bigInteger("tenant_id").unsigned().notNullable().references("id").inTable("tenants").onDelete("CASCADE");
            t.bigInteger("risk_model_id")
                .unsigned()
                .notNullable()
                .references("id")
                .inTable("fraud_risk_models")
                .onDelete("CASCADE");
        ...
        this.schema.createTable("fraud_signals", (t) => {
            ...
            t.string("subject_id", 160).notNullable();
            t.string("code", 120).notNullable();
            t.string("severity", 16).notNullable().defaultTo("medium");
            t.decimal("value", 12, 4).notNullable().defaultTo(1);
            t.jsonb("evidence").notNullable().defaultTo(this.raw("'{}'::jsonb"));
            t.string("dedupe_key", 180).nullable();
            t.timestamp("observed_at", { useTz: true }).notNullable().defaultTo(this.now());
            t.timestamp("expires_at", { useTz: true }).nullable();
            t.timestamps(true, true);
            t.unique(["tenant_id", "dedupe_key"]);
            t.index(["tenant_id", "subject_type", "subject_id", "observed_at"]);
            t.index(["tenant_id", "code", "severity", "observed_at"]);
        });
        this.schema.createTable("fraud_risk_scores", (t) => {
            ...
            t.string("subject_type", 40).notNullable();
            t.string("subject_id", 160).notNullable();
            t.bigInteger("model_version_id")
                .unsigned()
                .nullable()
                .references("id")
                .inTable("fraud_risk_model_versions")
                .onDelete("SET NULL");
            t.integer("score").notNullable();
            t.string("band", 16).notNullable();
            t.jsonb("reason_codes_json").notNullable().defaultTo(this.raw("'[]'::jsonb"));
            t.jsonb("evidence_summary").notNullable().defaultTo(this.raw("'{}'::jsonb"));
            t.string("idempotency_key", 180).nullable();
            t.timestamp("evaluated_at", { useTz: true }).notNullable().defaultTo(this.now());
            t.timestamps(true, true);
            t.unique(["tenant_id", "idempotency_key"]);
            t.index(["tenant_id", "subject_type", "subject_id", "evaluated_at"]);
            t.index(["tenant_id", "band", "evaluated_at"]);
        });
        this.schema.createTable("fraud_decisions", (t) => {
            t.bigIncrements("id");
            t.bigInteger("tenant_id").unsigned().notNullable().references("id").inTable("tenants").onDelete("CASCADE");
            t.bigInteger("risk_score_id")
                .unsigned()
                .notNullable()
                .references("id")
                .inTable("fraud_risk_scores")
                .onDelete("CASCADE");
            t.string("subject_type", 40).notNullable();
            t.string("subject_id", 160).notNullable();
            t.string("decision", 24).notNullable();
            t.string("policy_version", 80).notNullable().defaultTo("rule-v1");
            t.jsonb("reason_codes_json").notNullable().defaultTo(this.raw("'[]'::jsonb"));
            t.string("idempotency_key", 180).nullable();
            t.timestamp("expires_at", { useTz: true }).nullable();
            t.timestamps(true, true);
            t.unique(["tenant_id", "idempotency_key"]);
            t.index(["tenant_id", "subject_type", "subject_id", "created_at"]);
            t.index(["tenant_id", "decision", "created_at"]);
        });
        this.schema.createTable("fraud_action_executions", (t) => {
            ...
            t.timestamp("executed_at", { useTz: true }).notNullable().defaultTo(this.now());
            t.timestamps(true, true);
            t.unique(["tenant_id", "idempotency_key"]);
            t.index(["tenant_id", "action", "executed_at"]);
        });
        this.schema.createTable("fraud_cases", (t) => {
            ...
            t.bigInteger("assignee_user_id").unsigned().nullable().references("id").inTable("users").onDelete("SET NULL");
            t.text("summary").nullable();
            t.text("resolution").nullable();
            t.timestamp("opened_at", { useTz: true }).notNullable().defaultTo(this.now());
            t.timestamp("closed_at", { useTz: true }).nullable();
            t.timestamps(true, true);
            t.unique(["tenant_id", "case_number"]);
            t.index(["tenant_id", "status", "priority", "opened_at"]);
        ...
        this.schema.createTable("fraud_case_events", (t) => {
            ...
            t.index(["tenant_id", "case_id", "created_at"]);
        });
        this.schema.createTable("fraud_subject_controls", (t) => {
            ...
            t.string("subject_type", 40).notNullable();
            t.string("subject_id", 160).notNullable();
            t.string("control", 32).notNullable();
            ...
            t.timestamps(true, true);
            t.unique(["tenant_id", "idempotency_key"]);
            t.index(["tenant_id", "subject_type", "subject_id", "status"]);
        });
        for (const sql of [
