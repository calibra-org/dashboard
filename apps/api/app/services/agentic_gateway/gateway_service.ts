...
function parsePrincipalRateLimit(value: unknown) {
    ...
    if (!Number.isFinite(windowSeconds) || !Number.isFinite(maxActions) || windowSeconds < 0 || maxActions < 0) {
        throw Object.assign(new Error("Invalid principal rate limit policy"), {
            status: 422,
            code: "E_AGENTIC_RATE_LIMIT_POLICY_INVALID",
        });
    }
    if ((windowSeconds === 0) !== (maxActions === 0)) {
        throw Object.assign(new Error("Rate limit policy requires both window_seconds and max_actions"), {
            status: 422,
            code: "E_AGENTIC_RATE_LIMIT_POLICY_INCOMPLETE",
        });
    }
    if (windowSeconds > 86400 || maxActions > 1000000) {
        throw Object.assign(new Error("Principal rate limit policy exceeds supported bounds"), {
            status: 422,
            code: "E_AGENTIC_RATE_LIMIT_POLICY_BOUNDS",
        });
    }
...
export async function upsertChannel(input: {
    ...
    eligibleProductScope?: unknown;
    policyBoundary?: unknown;
    actorUserId: number;
}) {
    const tenantId = Number(currentTenantId());
    const trx = currentTrx();
    ...
    if (nextMode === "live") {
        ...
        if (!pass)
            throw Object.assign(
                new Error("Live mode requires a recent passing conformance run for the selected adapter/protocol"),
                { status: 422, code: "E_AGENTIC_CONFORMANCE_REQUIRED" },
            );
    }
    const payload = {
    ...
    if (existing) {
        await trx
            ...
            .update({ ...payload, version: Number(existing.version) + 1 });
        return trx.from("agentic_channels").where("id", existing.id).first();
    }
    const [row] = await trx
        .table("agentic_channels")
        .insert({ public_id: randomUUID(), tenant_id: tenantId, channel_key: input.channelKey, ...payload })
        .returning("*");
    return row;
}

export async function createCapabilityVersion(input: {
    ...
    const channel = await trx
        .from("agentic_channels")
        .where("tenant_id", tenantId)
        .where("public_id", input.channelPublicId)
        .first();
    if (!channel) throw Object.assign(new Error("Channel not found"), { status: 404, code: "E_AGENTIC_CHANNEL_NOT_FOUND" });
    if (isMutationCapability(input.capabilityKey) && input.riskClass === "read_only")
        throw Object.assign(new Error("Mutating capability cannot be classified read-only"), {
            status: 422,
            code: "E_AGENTIC_RISK_CLASS_INVALID",
        });
    ...
    const metadata = {
        ...
        version,
        protocol_version: input.protocolVersion ?? channel.protocol_version ?? null,
    ...
    const signature = encryption
        .getMessageVerifier()
        .sign(
            { metadataDigest, tenantId, channelId: channel.id, capabilityKey: input.capabilityKey, version },
            undefined,
            PURPOSE,
        );
    const [row] = await trx
        .table("agentic_capability_versions")
        .insert({
...
export async function runConformance(input: { channelPublicId: string; actorUserId: number }) {
    ...
    const [run] = await trx
        .table("agentic_conformance_runs")
        .insert({
            public_id: randomUUID(),
            tenant_id: tenantId,
            channel_id: channel.id,
            ...
            status,
            checks,
            artifacts: [{ kind: "capability_manifest", capability_count: capabilities.length }],
            failure_summary:
                status === "pass"
                    ? null
                    : checks
                          ...
                          .map((c) => c.key)
                          .join(", "),
            ran_by_user_id: input.actorUserId,
        })
        .returning("*");
    if (status === "pass")
        await trx
...
export async function gatewayOverview() {
    ...
    return {
        kpis: {
            ...
            live_channels: Number(live?.count ?? 0),
            active_principals: Number(principals?.count ?? 0),
            events_30d: Number(events?.count ?? 0),
            avg_readiness_bp:
                avgReadiness?.value === null || avgReadiness?.value === undefined ? null : Math.round(Number(avgReadiness.value)),
            policy_blocks_30d: Number(blocked?.count ?? 0),
        },
        freshness: {
            generated_at: new Date().toISOString(),
            sources: ["products", "product_translations", "product_variations", "inventory_items", "agentic_channel_events"],
        },
    };
}
...
export async function upsertPrincipal(input: {
    ...
    parsePrincipalRateLimit(input.rateLimitPolicy);
    const tenantId = Number(currentTenantId()),
        trx = currentTrx(),
        existing = await trx.from("agentic_principals").where({ tenant_id: tenantId, principal_key: input.principalKey }).first(),
        payload = {
            display_name: input.displayName,
    ...
    if (existing) {
        await trx.from("agentic_principals").where("id", existing.id).update(payload);
        return trx.from("agentic_principals").where("id", existing.id).first();
    }
    const [row] = await trx
        .table("agentic_principals")
        .insert({
            public_id: randomUUID(),
            tenant_id: tenantId,
...
export async function authorizeAgenticAction(input: {
    ...
    const prior = await trx
        ...
        .first();
    if (prior) {
        const sameRequest = String(prior.input_hash) === inputHash && String(prior.capability_key) === input.capabilityKey;
        if (!sameRequest) {
    ...
    const principal = await trx
        ...
        .where({ tenant_id: tenantId, public_id: input.principalPublicId, status: "active" })
        .first();
    if (!principal)
        throw Object.assign(new Error("Active principal not found"), { status: 404, code: "E_AGENTIC_PRINCIPAL_NOT_FOUND" });
    const cap = await trx
        .from("agentic_capability_versions")
        ...
        .first();
    if (!cap)
        throw Object.assign(new Error("Verified capability not found"), {
            status: 409,
            code: "E_AGENTIC_CAPABILITY_NOT_VERIFIED",
        });
    if (!verifyCapabilitySignature(cap, channel, tenantId)) {
        throw Object.assign(new Error("Capability metadata signature is invalid"), {
            status: 409,
            code: "E_AGENTIC_CAPABILITY_SIGNATURE_INVALID",
        });
    }
    ...
    if (channel.kill_switch || channel.mode === "disabled" || channel.mode === "shadow") {
        ...
        allowed = false;
        reason = "principal_rate_limit_exceeded";
        ...
        .insert({
            ...
            policy_result: {
                ...
                rate_limit: {
                    ...
                    max_actions: rateLimit.maxActions,
