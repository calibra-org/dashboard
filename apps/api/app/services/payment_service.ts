            ...
            .runImmediately(async () =>
                withTenantTransaction(async (trx) => {
                    ...
                    const order = await Order.query({ client: trx })
                        .where("id", Number(attempt.orderId))
                        .forUpdate()
                        .firstOrFail();
                    ...
                    if (ledger.replayed) {
                        if (ledger.payloadChanged)
                            Sentry.captureMessage("webhook_replay_payload_changed", {
                                level: "warning",
                                tags: {
                                    gateway: gatewayCode,
                                    order_id: String(order.id),
                                    attempt_id: String(attempt.id),
                                    event_kind: ledger.existing.eventKind,
                                },
                            });
                        const replayedRedirect =
                            ledger.existing.outcome === "verified" || ledger.existing.outcome === "verified_out_of_order"
                    ...
                    if (parsed.status === "cancelled" || parsed.status === "failed") {
                        if (attempt.status !== PaymentAttemptStatus.Verified) {
                            attempt.status =
                                parsed.status === "cancelled" ? PaymentAttemptStatus.Cancelled : PaymentAttemptStatus.Failed;
                    ...
                    if (!verifyResult.ok || amountMismatch) {
                        if (amountMismatch)
                            Sentry.captureMessage("payment_amount_mismatch", {
                                level: "error",
                                tags: { gateway: gatewayCode, order_id: String(order.id), attempt_id: String(attempt.id) },
                                extra: {
