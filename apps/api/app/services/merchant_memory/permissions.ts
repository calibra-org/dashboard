...
export async function requireApprovedAgentPrincipal(principalKey: string) {
    ...
    const classes = Array.isArray(row.data_access_classes) ? row.data_access_classes.map(String) : [];
    if (!classes.includes("merchant_memory") && !classes.includes("merchant_memory.read")) {
        throw new Exception("Agent principal does not have merchant-memory data access", {
            status: 403,
            code: "E_MERCHANT_MEMORY_AGENT_SCOPE_DENIED",
        });
    }
    return { id: Number(row.id), principal_key: String(row.principal_key), data_access_classes: classes };
}
