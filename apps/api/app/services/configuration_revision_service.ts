...
function identity(entry: SnapshotEntry): string {
    return entry.storage === "override" ? `${entry.key}@${entry.scope_type}:${entry.scope_key}` : `${entry.group}.${entry.key}`;
}
...
export default class ConfigurationRevisionService {
    ...
    private async restoreOverrides(scope: ConfigurationScope, snapshot: StoredSnapshot, actorUserId: number) {
        ...
        const currentMap = new Map(currentRows.map((row) => [`${row.definition_key}@${row.scope_type}:${row.scope_key}`, row]));
        let changed = false;
