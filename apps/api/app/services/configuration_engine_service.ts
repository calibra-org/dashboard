...
function rolloutAllows(row: OverrideRow, subjectKey?: string): boolean {
    ...
    const digest = createHash("sha256")
        .update(`${row.definition_key}:${row.scope_type}:${row.scope_key}:${subjectKey}`)
        .digest();
...
export default class ConfigurationEngineService {
    ...
    async group(group: ConfigurationGroup, context: ConfigurationResolutionContext = {}) {
        const definitions = [];
        for (const definition of configurationDefinitionsForGroup(group)) definitions.push(await this.resolve(definition, context));
        return {
            group,
            definitions,
    ...
    async apply(
        ...
        if (current) await trx.from("configuration_overrides").where("id", current.id).update(payload);
        else
    ...
    private async resolve(definition: ConfigurationSettingDefinition, context: ConfigurationResolutionContext) {
        ...
        return { definition: publicDefinition(definition), value: redact(definition, value), origin, resolution_chain: chain, mutable: true };
    }
    ...
    private requireDefinition(group: ConfigurationGroup, key: string): ConfigurationSettingDefinition {
        ...
        if (!definition || definition.group !== group) {
            throw new BusinessRuleException("Unknown configuration definition", "configuration.definition.unknown", { group, key });
        }
        return definition;
    }
    ...
    private assertMutable(definition: ConfigurationSettingDefinition) {
        if (definition.storage.kind !== "override") {
            throw new BusinessRuleException("Setting is owned by its canonical domain", "configuration.definition.linked_read_only", {
    ...
    private validateInput(definition: ConfigurationSettingDefinition, input: ConfigurationChangeInput) {
        ...
        if (!Number.isInteger(input.expected_version) || input.expected_version < 0) {
            throw new BusinessRuleException("expected_version must be a non-negative integer", "configuration.expected_version.invalid");
        }
        ...
        if (input.expires_at) {
            if (input.scope_type !== "temporary") {
                throw new BusinessRuleException("expires_at is only valid for temporary overrides", "configuration.expires_at.scope");
            }
    ...
    private assertValue(definition: ConfigurationSettingDefinition, value: unknown) {
        if (definition.secretClass === "reference") {
            if (!value || typeof value !== "object" || Array.isArray(value)) {
                throw new BusinessRuleException("Secret settings accept only env_ref objects", "configuration.secret.reference_required");
            }
            const envRef = String((value as { env_ref?: unknown }).env_ref ?? "");
        ...
        if (definition.type === "string" && typeof value !== "string") throw new BusinessRuleException("Expected string", "configuration.value.type");
        ...
        if (definition.type === "boolean" && typeof value !== "boolean") throw new BusinessRuleException("Expected boolean", "configuration.value.type");
        ...
        if (typeof value === "string" && definition.validation.pattern && !new RegExp(definition.validation.pattern).test(value)) {
    ...
    private assertVersion(expected: number, actual: number) {
        if (expected !== actual) {
            throw new BusinessRuleException("Configuration version conflict", "configuration.version_conflict", { expected, actual });
        }
    }
