...
export function calculateRiskDecision(signals: SignalInput[], control?: string | null) {
    if (control === "block")
        return { score: 1000, band: "critical" as RiskBand, decision: "block" as Decision, reasons: ["control.block"] };
    if (control === "allow_override")
        return { score: 0, band: "low" as RiskBand, decision: "allow" as Decision, reasons: ["control.allow_override"] };
...
class Phase20TrustRiskService {
    async overview() {
        const [bands, decisions, cases, signals, recentScores, recentCases] = await Promise.all([
            tenantDb()
                ...
                .select("band")
                .count("id as count")
                .where("evaluated_at", ">=", tenantDb().raw("NOW() - INTERVAL '30 days'"))
                .groupBy("band"),
            tenantDb()
                .from("fraud_decisions")
                .select("decision")
                .count("id as count")
                .where("created_at", ">=", tenantDb().raw("NOW() - INTERVAL '30 days'"))
                .groupBy("decision"),
            tenantDb().from("fraud_cases").whereNotIn("status", ["resolved", "closed"]).count("id as total").first(),
            tenantDb()
                .from("fraud_signals")
                .where("observed_at", ">=", tenantDb().raw("NOW() - INTERVAL '24 hours'"))
                .count("id as total")
                .first(),
            tenantDb()
                .from("fraud_risk_scores")
    ...
    async cases() {
        return { data: await tenantDb().from("fraud_cases").select("*").orderBy("opened_at", "desc").limit(200) };
    }
    async signals() {
        return {
            data: await tenantDb()
    ...
    async evaluate(input: {
        ...
        return withTenantTransaction(async (trx) => {
            ...
            const allSignals = [
                ...storedSignals.map((row: any) => ({ code: row.code, severity: row.severity, value: numberValue(row.value) })),
                ...incoming,
            ];
    ...
    async checkoutGuard(input: {
        ...
        if (input.customerId == null)
            signals.push({ code: "account.new", severity: "low", value: 0.5, evidence: { source: "guest_checkout" } });
        if (numberValue(input.amountMinor) >= 50_000_000)
            signals.push({ code: "velocity.checkout", severity: "medium", value: 0.5, evidence: { amount_bucket: "high" } });
        ...
        if (decision === "block")
            throw new Exception("Checkout blocked by trust policy", { status: 403, code: "E_TRUST_CHECKOUT_BLOCKED" });
        if (["review", "challenge", "hold"].includes(String(decision)))
            throw new Exception("Checkout requires trust review", { status: 409, code: "E_TRUST_CHECKOUT_REVIEW" });
        return result;
    }
    ...
    async updateCaseStatus(caseId: number, payload: any, actor: Actor) {
        ...
        const [row] = await tenantDb().from("fraud_cases").where("id", caseId).update(patch).returning("*");
        if (!row) throw new Exception("Fraud case not found", { status: 404, code: "E_TRUST_CASE" });
        await tenantDb()
            .table("fraud_case_events")
            .insert({
    ...
    async addCaseNote(caseId: number, note: string, actor: Actor) {
        if (!(await tenantDb().from("fraud_cases").where("id", caseId).first()))
            throw new Exception("Fraud case not found", { status: 404, code: "E_TRUST_CASE" });
        const [row] = await tenantDb()
            .table("fraud_case_events")
            .insert({ tenant_id: tenantId(), case_id: caseId, event_type: "case.note", actor_user_id: actor.id ?? null, note })
            .returning("*");
        return { data: row };
    }
    async createControl(payload: any, actor: Actor, idempotencyKey: string | null) {
        if (idempotencyKey) {
            const replay = await tenantDb().from("fraud_subject_controls").where("idempotency_key", idempotencyKey).first();
            if (replay) return { data: replay, replayed: true };
        }
        const [row] = await tenantDb()
    ...
    async releaseControl(controlId: number, actor: Actor, idempotencyKey: string | null) {
        if (idempotencyKey) {
            const replay = await tenantDb().from("fraud_action_executions").where("idempotency_key", idempotencyKey).first();
            if (replay) return { data: replay, replayed: true };
        }
        const [control] = await tenantDb()
            ...
            .returning("*");
        if (!control) throw new Exception("Subject control not found", { status: 404, code: "E_TRUST_CONTROL" });
        const [action] = await tenantDb()
            .table("fraud_action_executions")
            .insert({
                tenant_id: tenantId(),
    ...
    async health() {
        const result = await tenantDb().rawQuery(
