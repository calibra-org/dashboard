...
const SIGNAL_MULTIPLIERS: Record<string, number> = {
    "velocity.checkout": 1.0, "velocity.payment": 1.15, "account.new": 0.55, "device.untrusted": 0.7,
    "geo.impossible_travel": 1.45, "payment.mismatch": 1.25, "coupon.abuse": 1.05, "refund.burst": 1.25, "auth.ato_suspected": 1.7,
};
...
function sanitizeEvidence(value: unknown, depth = 0): unknown {
    ...
    return Object.fromEntries(Object.entries(value as Record<string, unknown>).slice(0, 40).map(([key, item]) => [key, SENSITIVE_KEY.test(key) ? "[redacted]" : sanitizeEvidence(item, depth + 1)]));
}

export function calculateRiskDecision(signals: SignalInput[], control?: string | null) {
    if (control === "block") return { score: 1000, band: "critical" as RiskBand, decision: "block" as Decision, reasons: ["control.block"] };
    if (control === "allow_override") return { score: 0, band: "low" as RiskBand, decision: "allow" as Decision, reasons: ["control.allow_override"] };
    const contributions = signals.map((signal) => {
        ...
        return { code: signal.code, score: Math.round(BASE_BY_SEVERITY[severity] * (SIGNAL_MULTIPLIERS[signal.code] ?? 0.8) * value) };
    });
    const score = clamp(contributions.reduce((sum, item) => sum + item.score, 0), 0, 1000);
    const band: RiskBand = score >= 750 ? "critical" : score >= 500 ? "high" : score >= 250 ? "medium" : "low";
    let decision: Decision = band === "critical" ? "block" : band === "high" ? "challenge" : band === "medium" ? "review" : "allow";
    if (control === "challenge") decision = "challenge";
    if (control === "review" && decision === "allow") decision = "review";
    return { score, band, decision, reasons: contributions.sort((a, b) => b.score - a.score).slice(0, 8).map((item) => item.code) };
}
...
class Phase20TrustRiskService {
    ...
    async evaluate(input: { subject_type: string; subject_id: string; signals?: SignalInput[]; idempotency_key?: string | null }) {
        return withTenantTransaction(async (trx) => {
            const key = input.idempotency_key?.trim().slice(0, 180) || null;
            if (key) {
                const existing = await trx.from("fraud_risk_scores").where("tenant_id", tenantId()).where("idempotency_key", key).first();
                if (existing) return { data: { score: existing, decision: await trx.from("fraud_decisions").where("risk_score_id", existing.id).first() }, replayed: true };
            }
            const activeControl = await trx.from("fraud_subject_controls").where("tenant_id", tenantId()).where("subject_type", input.subject_type).where("subject_id", input.subject_id).where("status", "active")
                .where((query) => query.whereNull("expires_at").orWhere("expires_at", ">", new Date())).orderBy("created_at", "desc").first();
            const storedSignals = await trx.from("fraud_signals").where("tenant_id", tenantId()).where("subject_type", input.subject_type).where("subject_id", input.subject_id)
                .where((query) => query.whereNull("expires_at").orWhere("expires_at", ">", new Date())).orderBy("observed_at", "desc").limit(100);
            const incoming = input.signals ?? [];
            for (const signal of incoming) {
                if (signal.dedupe_key && await trx.from("fraud_signals").where("tenant_id", tenantId()).where("dedupe_key", signal.dedupe_key.slice(0, 180)).first()) continue;
                await trx.table("fraud_signals").insert({ tenant_id: tenantId(), subject_type: input.subject_type, subject_id: input.subject_id, code: signal.code,
                    severity: signal.severity ?? "medium", value: clamp(Number(signal.value ?? 1), 0, 4), evidence: sanitizeEvidence(signal.evidence ?? {}), dedupe_key: signal.dedupe_key?.slice(0, 180) ?? null });
            }
            const allSignals = [...storedSignals.map((row: any) => ({ code: row.code, severity: row.severity, value: numberValue(row.value) })), ...incoming];
            ...
            if (result.decision !== "allow") {
                ...
                const [fraudCase] = await trx.table("fraud_cases").insert({ tenant_id: tenantId(), case_number: `FR-${new Date().getUTCFullYear()}-${score.id}`, subject_type: input.subject_type, subject_id: input.subject_id,
                    decision_id: decision.id, priority, summary: `Risk ${result.band}: ${result.reasons.join(", ") || "policy control"}` }).returning("*");
                await trx.table("fraud_case_events").insert({ tenant_id: tenantId(), case_id: fraudCase.id, event_type: "case.opened", metadata: { decision_id: decision.id, score: result.score, band: result.band } });
            }
    ...
    async checkoutGuard(input: { orderId: number | string; customerId?: number | string | null; idempotencyKey?: string | null; amountMinor?: number | null }) {
        ...
        const result = await this.evaluate({ subject_type: "order", subject_id: String(input.orderId), signals, idempotency_key: input.idempotencyKey ? `checkout:${input.idempotencyKey}:risk` : null });
        const decision = result.data.decision?.decision as Decision | undefined;
        if (decision === "block") throw new Exception("Checkout blocked by trust policy", { status: 403, code: "E_TRUST_CHECKOUT_BLOCKED" });
        if (["review", "challenge", "hold"].includes(String(decision))) throw new Exception("Checkout requires trust review", { status: 409, code: "E_TRUST_CHECKOUT_REVIEW" });
        return result;
    }
    ...
    async promoteChampion(versionId: number, actor: Actor, idempotencyKey: string | null) {
        return withTenantTransaction(async (trx) => {
            if (idempotencyKey) { const replay = await trx.from("fraud_action_executions").where("tenant_id", tenantId()).where("idempotency_key", idempotencyKey).first(); if (replay) return { data: replay, replayed: true }; }
            const version = await trx.from("fraud_risk_model_versions").where("id", versionId).forUpdate().first();
            if (!version) throw new Exception("Risk model version not found", { status: 404, code: "E_TRUST_MODEL_VERSION" });
            if (!version.validated_at) throw new Exception("Only validated versions can become champion", { status: 422, code: "E_TRUST_MODEL_NOT_VALIDATED" });
            await trx.from("fraud_risk_model_versions").where("risk_model_id", version.risk_model_id).where("deployment_state", "champion").update({ deployment_state: "retired" });
            await trx.from("fraud_risk_model_versions").where("id", versionId).update({ deployment_state: "champion" });
            const [action] = await trx.table("fraud_action_executions").insert({ tenant_id: tenantId(), action: "model.promote_champion", actor_user_id: actor.id ?? null, idempotency_key: idempotencyKey, metadata: { version_id: versionId, risk_model_id: version.risk_model_id } }).returning("*");
