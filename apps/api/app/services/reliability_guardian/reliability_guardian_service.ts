...
async function policyByPublicId(publicId: string) {
    ...
    if (!row)
        throw new Exception("Reliability remediation policy not found", { status: 404, code: "E_RELIABILITY_POLICY_NOT_FOUND" });
    return row;
}
async function latestOpenIncident(invariantId: number) {
    return currentTrx()
        ...
        .whereIn("status", ["open", "mitigating", "monitoring"])
...
async function fulfillmentPromiseAccuracy(invariant: Row, now: DateTime): Promise<Observation | null> {
    ...
    return {
        ...
        evidence: {
            source: "phase31_fulfillment_promise",
            sample_count: rows.length,
            on_time: onTime,
            late: rows.length - onTime,
            unit: "percent",
        },
    };
}
...
async function latestManualMetric(invariant: Row, now: DateTime): Promise<Observation | null> {
    ...
    if (!row) return null;
    return {
        value: numberValue(row.observed_value),
        evidenceRef: row.evidence_ref ? stringValue(row.evidence_ref) : null,
...
async function assertExecutionBudget(policy: Row, now: DateTime) {
    ...
    if (last) {
        ...
        if (lastAt.plus({ seconds: numberValue(policy.cooldown_seconds) }) > now) {
            throw new Exception("Remediation policy is cooling down", {
                status: 409,
                code: "E_RELIABILITY_REMEDIATION_COOLDOWN",
            });
        }
    }
...
async function executeAction(policy: Row, actorUserId: number | null) {
    ...
    if (policy.action_type === "rollback_configuration") {
        ...
        if (!scope || !revision)
            throw new Exception("Configuration rollback target is incomplete", {
                status: 422,
                code: "E_RELIABILITY_TARGET_INVALID",
            });
        ...
        if (!result)
            throw new Exception("Configuration rollback revision was not found", {
                status: 404,
                code: "E_RELIABILITY_ROLLBACK_REVISION_NOT_FOUND",
            });
        return {
            before: { scope, revision: before[0]?.revision ?? null },
            after: { scope, revision: result.revision.revision, rollback_of_revision: revision, changed: result.changed },
        };
    }
    if (policy.action_type === "pause_experiment") {
        const experimentId = numberValue(target.experiment_id);
        if (!experimentId)
            throw new Exception("Experiment target is incomplete", { status: 422, code: "E_RELIABILITY_TARGET_INVALID" });
        const trx = currentTrx();
        ...
        if (experiment.status === "paused")
            return {
                before: { id: experimentId, status: "paused", version: experiment.version },
                after: { id: experimentId, status: "paused", version: experiment.version },
            };
        if (!["running", "scheduled"].includes(String(experiment.status)))
            throw new Exception("Only running or scheduled experiments can be paused", {
                status: 409,
                code: "E_RELIABILITY_EXPERIMENT_STATE",
            });
    ...
    if (policy.action_type === "disable_policy") {
        ...
        if (!targetPolicy)
            throw new Exception("Reliability policy target not found", { status: 404, code: "E_RELIABILITY_POLICY_NOT_FOUND" });
        await trx
            .from("reliability_remediation_policies")
            .where("id", targetPolicyId)
            .update({ enabled: false, version: numberValue(targetPolicy.version) + 1, updated_at: new Date() });
        return {
            before: { id: targetPolicyId, enabled: Boolean(targetPolicy.enabled), version: targetPolicy.version },
            after: { id: targetPolicyId, enabled: false, version: numberValue(targetPolicy.version) + 1 },
        };
    }
...
export async function executeRemediation(incidentPublicId: string, actorUserId: number | null, explicitApproval = false) {
    ...
    if (!incident)
        throw new Exception("Reliability incident not found", { status: 404, code: "E_RELIABILITY_INCIDENT_NOT_FOUND" });
    if (!["open", "mitigating", "monitoring"].includes(String(incident.status)))
        throw new Exception("Incident is not actionable", { status: 409, code: "E_RELIABILITY_INCIDENT_STATE" });
    if (!incident.remediation_policy_id)
        throw new Exception("Incident has no remediation policy", { status: 409, code: "E_RELIABILITY_POLICY_REQUIRED" });
    const policy = await trx
        .from("reliability_remediation_policies")
        .where("id", incident.remediation_policy_id)
        .forUpdate()
        .first();
    if (!policy || !policy.enabled)
        throw new Exception("Remediation policy is unavailable", { status: 409, code: "E_RELIABILITY_POLICY_DISABLED" });
    ...
    if (requiresApproval && !explicitApproval) {
        ...
        await trx
            .table("reliability_remediation_runs")
            .insert({
    ...
    for (const run of runs) {
        if (passed) {
            await trx
                ...
                .update({
                    status: "succeeded",
                    verified_at: now.toJSDate(),
                    verification: { passed: true, invariant_key: invariant.invariant_key },
                    updated_at: now.toJSDate(),
                });
        } else {
            await trx
                ...
                .where("id", run.id)
                .update({
                    verification: { passed: false, invariant_key: invariant.invariant_key, observed_at: now.toISO() },
                    updated_at: now.toJSDate(),
                });
        }
    }
...
export async function runCycle(actorUserId: number | null = null) {
    ...
    for (const invariant of invariants) {
        ...
        results.push({
            ...
            observed_value: observation.value,
            threshold: numberValue(invariant.threshold),
            passed,
            incident_public_id: incident?.public_id ?? null,
        });
    }
...
async function createScorecard(windowStart: DateTime, windowEnd: DateTime) {
    const trx = currentTrx();
    const latest = await trx
        .from("reliability_evaluations as evaluation")
        .where("evaluation.evaluated_at", ">=", windowStart.toJSDate())
        .where("evaluation.evaluated_at", "<=", windowEnd.toJSDate())
        .select("evaluation.invariant_id")
        .max("evaluation.id as latest_id")
        .groupBy("evaluation.invariant_id");
    ...
    const auto = await trx
        .from("reliability_remediation_runs as run")
        .innerJoin("reliability_remediation_policies as policy", "policy.id", "run.policy_id")
        .where("run.created_at", ">=", windowStart.toJSDate())
        .where("run.created_at", "<=", windowEnd.toJSDate())
        .where("policy.auto_execute", true)
        .whereIn("run.status", ["verifying", "succeeded"])
        .count("run.id as count")
        .first();
    if (!evaluations.length) return null;
    const reliabilityBps = Math.round((passing / evaluations.length) * 10000);
    const [row] = await trx
        .table("reliability_scorecards")
        .insert({
...
export async function listInvariants() {
    return currentTrx()
        .from("reliability_invariants as invariant")
        .leftJoin("reliability_remediation_policies as policy", "policy.id", "invariant.remediation_policy_id")
        .select("invariant.*", "policy.public_id as remediation_policy_public_id", "policy.name as remediation_policy_name")
        .orderBy("invariant.severity", "desc")
        .orderBy("invariant.name", "asc");
}
export async function createInvariant(input: InvariantInput, actorUserId: number) {
    const trx = currentTrx();
    let policyId: number | null = null;
    if (input.remediation_policy_public_id)
        policyId = numberValue((await policyByPublicId(input.remediation_policy_public_id)).id);
    const payload = {
        ...input,
        remediation_policy_id: policyId,
...
export async function createPolicy(input: PolicyInput, actorUserId: number) {
    if (input.auto_execute && input.risk_level !== "low")
        throw new Exception("Only low-risk remediation may auto-execute", { status: 422, code: "E_RELIABILITY_AUTO_RISK" });
    const [row] = await currentTrx()
        .table("reliability_remediation_policies")
        .insert({ ...input, target: input.target ?? {}, created_by_user_id: actorUserId })
        .returning("*");
    return row;
}
export async function recordManualObservation(publicId: string, value: number, evidence: Record<string, unknown>) {
    ...
    if (invariant.source_kind !== "manual_metric")
        throw new Exception("Invariant does not accept manual metrics", { status: 422, code: "E_RELIABILITY_SOURCE_MISMATCH" });
    const now = DateTime.utc();
    const observation: Observation = {
        value,
        evidenceRef: `manual:${fingerprint({ value, evidence, at: now.toISO() })}`,
        evidence: { ...evidence, source: "manual_metric" },
    };
...
export async function listIncidents(limit = 100) {
    return currentTrx()
        .from("reliability_incidents as incident")
        .innerJoin("reliability_invariants as invariant", "invariant.id", "incident.invariant_id")
        .leftJoin("reliability_remediation_policies as policy", "policy.id", "incident.remediation_policy_id")
        .select(
            "incident.*",
            "invariant.invariant_key",
            "invariant.name as invariant_name",
            "policy.public_id as policy_public_id",
            "policy.name as policy_name",
        )
        .orderBy("incident.opened_at", "desc")
        .limit(Math.min(500, Math.max(1, limit)));
}
export async function listRemediations(limit = 100) {
    return currentTrx()
        .from("reliability_remediation_runs as run")
        .innerJoin("reliability_incidents as incident", "incident.id", "run.incident_id")
        .innerJoin("reliability_remediation_policies as policy", "policy.id", "run.policy_id")
        .select(
            "run.*",
            "incident.public_id as incident_public_id",
            "policy.public_id as policy_public_id",
            "policy.name as policy_name",
        )
...
export async function listScorecards(limit = 100) {
    return currentTrx()
        .from("reliability_scorecards")
        .orderBy("window_end_at", "desc")
        .limit(Math.min(500, Math.max(1, limit)));
}
...
export async function rollbackRemediation(publicId: string, actorUserId: number) {
    ...
    if (!["verifying", "succeeded"].includes(String(run.status)))
        throw new Exception("Remediation is not rollback-eligible", { status: 409, code: "E_RELIABILITY_REMEDIATION_STATE" });
    const policy = await trx.from("reliability_remediation_policies").where("id", run.policy_id).first();
    if (!policy || !policy.rollback_required)
        throw new Exception("Policy does not permit rollback", { status: 409, code: "E_RELIABILITY_ROLLBACK_DISABLED" });
    ...
    if (policy.action_type === "rollback_configuration") {
        ...
        if (!scope || !revision)
            throw new Exception("Rollback snapshot is incomplete", { status: 409, code: "E_RELIABILITY_ROLLBACK_SNAPSHOT" });
        const result = await new ConfigurationRevisionService().rollback(scope, revision, actorUserId);
        if (!result)
            throw new Exception("Rollback revision no longer exists", {
                status: 409,
                code: "E_RELIABILITY_ROLLBACK_REVISION_NOT_FOUND",
            });
        ...
        const experiment = await trx.from("experiments").where("id", experimentId).forUpdate().first();
        if (!experiment)
            throw new Exception("Experiment no longer exists", { status: 409, code: "E_RELIABILITY_EXPERIMENT_NOT_FOUND" });
        if (before.status === "running" && experiment.status === "paused") {
            await new Phase17ExperimentationService().transition(
                experimentId,
                { status: "running", expected_version: numberValue(experiment.version), reason: "Reliability Guardian rollback" },
                actorUserId,
            );
        }
        ...
        await trx
            .from("reliability_remediation_policies")
            .where("id", policyId)
            .update({ enabled: Boolean(before.enabled), version: trx.raw("version + 1"), updated_at: new Date() });
    }
    await trx
        .from("reliability_remediation_runs")
        .where("id", run.id)
        .update({ status: "rolled_back", rolled_back_at: new Date(), executed_by_user_id: actorUserId, updated_at: new Date() });
