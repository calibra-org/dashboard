...
function serialize(row: DbRow): DbRow {
    ...
    for (const key of ["id", "ticket_id", "message_id", "media_id", "owner_user_id", "user_id", "campaign_id", "source_ticket_id", "target_ticket_id", "version"]) {
...
async function ensureTicketVersion(ticketId: number, expectedVersion: number, lock = false): Promise<DbRow> {
    let query = currentTrx().from("support_tickets").where("id", ticketId);
...
function validateSavedViewQuery(query: Record<string, unknown>) {
    ...
    for (const key of Object.keys(query)) {
        if (!allowed.has(key)) throw new Exception("Unsupported saved-view filter", { status: 422, code: "E_TICKET_SAVED_VIEW_QUERY" });
    }
...
function validateRuleObject(input: Record<string, unknown>, kind: "conditions" | "actions") {
    const allowedConditionKeys = new Set(["status", "priority", "channel", "category", "assigned_user_id", "sla_breached", "tags_any"]);
    ...
    for (const key of Object.keys(input)) {
        if (!allowed.has(key)) throw new Exception(`Unsupported support ${kind} key`, { status: 422, code: "E_SUPPORT_RULE_SHAPE" });
    }
}
...
export class TicketOperationsService {
    async workflowStatuses() {
        ...
        const rows = await currentTrx().from("support_ticket_workflow_statuses").orderBy("sort_order", "asc").orderBy("id", "asc");
        return { data: rows.map((row) => serialize(row as DbRow)) };
    }

    async ensureDefaultWorkflowStatuses() {
        const defaults = [
            { code: "open", label_fa: "باز", label_en: "Open", semantic_group: "active", is_terminal: false, is_customer_waiting: false, sort_order: 10 },
            { code: "pending", label_fa: "در انتظار", label_en: "Pending", semantic_group: "waiting", is_terminal: false, is_customer_waiting: false, sort_order: 20 },
            { code: "waiting_customer", label_fa: "منتظر مشتری", label_en: "Waiting for customer", semantic_group: "waiting", is_terminal: false, is_customer_waiting: true, sort_order: 30 },
            { code: "resolved", label_fa: "حل‌شده", label_en: "Resolved", semantic_group: "resolved", is_terminal: true, is_customer_waiting: false, sort_order: 40 },
            { code: "closed", label_fa: "بسته", label_en: "Closed", semantic_group: "closed", is_terminal: true, is_customer_waiting: false, sort_order: 50 },
        ];
    ...
    async createSavedView(actorUserId: number, input: { name: string; query: Record<string, unknown>; is_shared?: boolean }) {
        ...
        const [row] = await currentTrx().table("support_ticket_saved_views").insert({
    ...
    async updateSavedView(id: number, actorUserId: number, input: { name?: string; query?: Record<string, unknown>; is_shared?: boolean }) {
        if (input.query) validateSavedViewQuery(input.query);
        const current = await currentTrx().from("support_ticket_saved_views").where("id", id).where("owner_user_id", actorUserId).first();
        ...
        const [row] = await currentTrx().from("support_ticket_saved_views").where("id", id).where("owner_user_id", actorUserId).update(patch).returning("*");
        return { data: serialize(row as DbRow) };
    }

    async deleteSavedView(id: number, actorUserId: number) {
        const deleted = await currentTrx().from("support_ticket_saved_views").where("id", id).where("owner_user_id", actorUserId).delete();
    ...
    async bulk(input: {
        ...
        if (new Set(ids).size !== ids.length) throw new Exception("Duplicate ticket ids are not allowed", { status: 422, code: "E_TICKET_BULK_DUPLICATE" });
        ...
        for (const item of input.tickets) {
            try {
                if (input.operation === "transition") {
                    if (!input.status) throw new Exception("Bulk transition requires status", { status: 422, code: "E_TICKET_BULK_PAYLOAD" });
                    const result = await supportTicketService.transition(item.id, input.status, item.expected_version, input.reason, actorUserId);
                ...
                if (input.operation === "priority") {
                    if (!input.priority) throw new Exception("Bulk priority requires priority", { status: 422, code: "E_TICKET_BULK_PAYLOAD" });
                ...
                results.push({ id: item.id, ok: false, code: candidate.code ?? "E_TICKET_BULK_ITEM", status: candidate.status ?? 422, message: candidate.message ?? "Ticket operation failed" });
            }
        }
        return { data: results, meta: { requested: input.tickets.length, succeeded: results.filter((item) => item.ok).length, failed: results.filter((item) => !item.ok).length } };
    }

    async addAttachment(ticketId: number, input: { media_id: number; message_id?: number | null; sha256?: string | null }, actorUserId: number) {
        ...
        if (input.message_id) {
            const message = await trx.from("support_ticket_messages").where("id", input.message_id).where("ticket_id", ticketId).first();
            if (!message) throw new Exception("Ticket message not found", { status: 422, code: "E_TICKET_ATTACHMENT_MESSAGE" });
        }
        const [row] = await trx.table("support_ticket_attachments").insert({
            ticket_id: ticketId,
        ...
        await event(ticketId, actorUserId, "ticket.attachment.added", { attachment_id: numberValue(row.id), media_id: input.media_id, scan_status: "pending" });
        return { data: serialize(row as DbRow) };
    }

    async updateAttachmentScan(id: number, input: { status: "clean" | "infected" | "error"; evidence?: string | null }, actorUserId: number) {
        ...
        const [row] = await currentTrx().from("support_ticket_attachments").where("id", id).update({
            scan_status: input.status,
            scan_evidence: input.evidence ?? null,
            scanned_at: new Date(),
        }).returning("*");
        await event(numberValue(row.ticket_id), actorUserId, "ticket.attachment.scanned", { attachment_id: id, status: input.status });
        return { data: serialize(row as DbRow) };
    }

    async attachments(ticketId: number) {
        const rows = await currentTrx().from("support_ticket_attachments").where("ticket_id", ticketId).orderBy("created_at", "asc");
        return { data: rows.map((row) => serialize(row as DbRow)) };
    }

    async merge(sourceTicketId: number, input: { target_ticket_id: number; expected_source_version: number; expected_target_version: number; reason?: string | null }, actorUserId: number) {
        if (sourceTicketId === input.target_ticket_id) throw new Exception("A ticket cannot be merged into itself", { status: 422, code: "E_TICKET_MERGE_SELF" });
        ...
        const rows = await currentTrx().from("support_tickets").whereIn("id", [firstId, secondId]).orderBy("id", "asc").forUpdate();
        ...
        if (numberValue(source.version) !== input.expected_source_version || numberValue(target.version) !== input.expected_target_version) {
            throw new Exception("Support ticket changed by another operator", { status: 409, code: "E_TICKET_VERSION_CONFLICT" });
        }
        const reverse = await currentTrx().from("support_ticket_merges").where("source_ticket_id", input.target_ticket_id).where("target_ticket_id", sourceTicketId).first();
        if (reverse) throw new Exception("Ticket merge would create a loop", { status: 409, code: "E_TICKET_MERGE_LOOP" });
        const [mergeRow] = await currentTrx().table("support_ticket_merges").insert({
            source_ticket_id: sourceTicketId,
            target_ticket_id: input.target_ticket_id,
            merged_by_user_id: actorUserId,
            reason: input.reason ?? null,
        }).returning("*");
        await currentTrx().from("support_tickets").where("id", sourceTicketId).where("version", input.expected_source_version).update({
            status: "closed",
            version: input.expected_source_version + 1,
            closed_at: new Date(),
            updated_at: new Date(),
        });
        await event(sourceTicketId, actorUserId, "ticket.merged", { target_ticket_id: input.target_ticket_id, merge_id: numberValue(mergeRow.id), reason: input.reason ?? null });
        await event(input.target_ticket_id, actorUserId, "ticket.merge.received", { source_ticket_id: sourceTicketId, merge_id: numberValue(mergeRow.id) });
    ...
    async heartbeat(userId: number, input: { state: "offline" | "available" | "busy" | "away"; capacity: number }) {
        const active = await currentTrx().from("support_tickets").where("assigned_user_id", userId).whereNotIn("status", ["resolved", "closed"]).count("id as total").first();
        const payload = {
    ...
    async channels() {
        ...
        return { data: rows.map((row) => ({ ...serialize(row as DbRow), credential_configured: Boolean(row.credential_env_ref && process.env[String(row.credential_env_ref)]), credential_value: undefined })) };
    }

    async configureChannel(input: { channel: string; enabled: boolean; credential_env_ref?: string | null; configuration?: Record<string, unknown> }) {
        ...
        const payload = {
            ...
            last_error: input.enabled && envRef && !credentialConfigured ? "Credential environment reference is not available at runtime" : null,
            last_verified_at: null,
            updated_at: new Date(),
        };
        await currentTrx().table("support_channel_integrations").insert(payload).onConflict(["tenant_id", "channel"]).merge(payload);
    ...
    async createRoutingRule(input: { name: string; priority?: number; enabled?: boolean; conditions: Record<string, unknown>; actions: Record<string, unknown> }) {
        validateRuleObject(input.conditions, "conditions");
        validateRuleObject(input.actions, "actions");
        const [row] = await currentTrx().table("support_routing_rules").insert({ ...input, conditions: JSON.stringify(input.conditions), actions: JSON.stringify(input.actions) }).returning("*");
        return { data: serialize(row as DbRow) };
    }

    async updateRoutingRule(id: number, input: { expected_version: number; name?: string; priority?: number; enabled?: boolean; conditions?: Record<string, unknown>; actions?: Record<string, unknown> }) {
        ...
        const [row] = await currentTrx().from("support_routing_rules").where("id", id).where("version", input.expected_version).update(patch).returning("*");
        if (!row) throw new Exception("Support routing rule changed or was not found", { status: 409, code: "E_SUPPORT_RULE_VERSION" });
        return { data: serialize(row as DbRow) };
    }
    ...
    async createAutomationRule(input: { name: string; trigger: string; enabled?: boolean; conditions: Record<string, unknown>; actions: Array<Record<string, unknown>> }) {
        validateRuleObject(input.conditions, "conditions");
        for (const action of input.actions) validateRuleObject(action, "actions");
        const [row] = await currentTrx().table("support_automation_rules").insert({ ...input, conditions: JSON.stringify(input.conditions), actions: JSON.stringify(input.actions) }).returning("*");
        return { data: serialize(row as DbRow) };
    }

    async updateAutomationRule(id: number, input: { expected_version: number; name?: string; enabled?: boolean; conditions?: Record<string, unknown>; actions?: Array<Record<string, unknown>> }) {
        if (input.conditions) validateRuleObject(input.conditions, "conditions");
        ...
        if (input.conditions) patch.conditions = JSON.stringify(input.conditions);
        if (input.actions) patch.actions = JSON.stringify(input.actions);
        const [row] = await currentTrx().from("support_automation_rules").where("id", id).where("version", input.expected_version).update(patch).returning("*");
        if (!row) throw new Exception("Support automation rule changed or was not found", { status: 409, code: "E_SUPPORT_AUTOMATION_VERSION" });
        return { data: serialize(row as DbRow) };
    }
    ...
    async createCampaign(input: { name: string; channel: string; template_body: string; quiet_hours?: Record<string, unknown>; estimated_cost_minor?: number; scheduled_at?: Date | null }) {
        const [row] = await currentTrx().table("support_campaigns").insert({
            name: input.name,
            channel: input.channel,
    ...
    async addCampaignRecipients(id: number, expectedVersion: number, recipients: string[]) {
        ...
        if (numberValue(campaign.version) !== expectedVersion) throw new Exception("Support campaign changed", { status: 409, code: "E_SUPPORT_CAMPAIGN_VERSION" });
        if (!['draft','scheduled','paused'].includes(String(campaign.status))) throw new Exception("Campaign recipients cannot be changed after delivery starts", { status: 409, code: "E_SUPPORT_CAMPAIGN_IMMUTABLE" });
        const unique = [...new Set(recipients.map((value) => value.trim().toLowerCase()).filter(Boolean))];
        for (const recipient of unique) {
            await currentTrx().table("support_campaign_recipients").insert({ campaign_id: id, recipient_key: recipient }).onConflict(["tenant_id", "campaign_id", "recipient_key"]).ignore();
        }
        await currentTrx().from("support_campaigns").where("id", id).where("version", expectedVersion).update({ version: expectedVersion + 1, updated_at: new Date() });
        const total = await currentTrx().from("support_campaign_recipients").where("campaign_id", id).count("id as total").first();
    ...
    async transitionCampaign(id: number, expectedVersion: number, status: "scheduled" | "paused" | "cancelled") {
        ...
        if (numberValue(row.version) !== expectedVersion) throw new Exception("Support campaign changed", { status: 409, code: "E_SUPPORT_CAMPAIGN_VERSION" });
        if (status === "scheduled") {
            if (!row.scheduled_at) throw new Exception("Campaign needs a schedule before scheduling", { status: 422, code: "E_SUPPORT_CAMPAIGN_SCHEDULE" });
            if (String(row.template_status) !== "approved") throw new Exception("Campaign template must be approved before scheduling", { status: 422, code: "E_SUPPORT_CAMPAIGN_TEMPLATE" });
            const channel = await currentTrx().from("support_channel_integrations").where("channel", row.channel).first();
            if (!channel || String(channel.status) !== "connected") throw new Exception("Campaign channel is not verified as connected", { status: 422, code: "E_SUPPORT_CHANNEL_NOT_CONNECTED" });
        }
        const [updated] = await currentTrx().from("support_campaigns").where("id", id).where("version", expectedVersion).update({ status, version: expectedVersion + 1, updated_at: new Date() }).returning("*");
    ...
    async reports() {
        ...
        const [backlog, sla, byAssignee, csat, reopened] = await Promise.all([
            trx.from("support_tickets").whereNotIn("status", ["resolved", "closed"]).select("priority").count("id as total").groupBy("priority"),
            trx.rawQuery(`SELECT
            ...
            trx.from("support_tickets as t").leftJoin("users as u", "u.id", "t.assigned_user_id").whereNotIn("t.status", ["resolved", "closed"]).select("t.assigned_user_id", "u.email").count("t.id as active").groupBy("t.assigned_user_id", "u.email").orderBy("active", "desc"),
            trx.from("support_csat_responses").avg("score as average").count("id as responses").first(),
            trx.rawQuery(`SELECT COUNT(DISTINCT ticket_id)::bigint AS total FROM support_ticket_events WHERE event_type = 'ticket.status_changed' AND payload->>'from' IN ('resolved','closed') AND payload->>'to' = 'open'`),
        ]);
        ...
        return {
            data: {
                ...
                assignees: byAssignee.map((row) => ({ assigned_user_id: nullableNumber(row.assigned_user_id), email: row.email ?? null, active: numberValue(row.active) })),
            },
        };
    }
}
