...
async function tokenRow(token: string, lock = false): Promise<DbRow> {
    ...
    if (!row || row.revoked_at || new Date(String(row.expires_at)).getTime() <= Date.now()) {
        throw new Exception("Support tracking token is invalid or expired", {
            status: 404,
            code: "E_SUPPORT_PUBLIC_TOKEN",
        });
    }
    return row;
}
...
async function publicTicket(ticketId: number) {
    ...
    return {
        ...
        attachments: attachments.map((row) => ({
            ...
            message_id: row.message_id === null ? null : numberValue(row.message_id),
...
export class PublicSupportService {
    async create(input: {
        ...
        const createdData = created.data as DbRow;
    ...
    async reply(token: string, input: { expected_version: number; body: string }) {
        ...
        if (numberValue(ticket.version) !== input.expected_version) {
            throw new Exception("Support ticket changed", {
                status: 409,
                code: "E_TICKET_VERSION_CONFLICT",
            });
        }
        if (String(ticket.status) === "closed") {
            throw new Exception("Closed support tickets cannot receive public replies", {
                status: 409,
                code: "E_SUPPORT_PUBLIC_CLOSED",
            });
        }
    ...
    async csat(token: string, input: { score: number; comment?: string | null }) {
        ...
        if (!["resolved", "closed"].includes(String(ticket.status))) {
            throw new Exception("CSAT is available after ticket resolution", {
                status: 409,
                code: "E_SUPPORT_CSAT_NOT_READY",
            });
        }
        const existing = await currentTrx().from("support_csat_responses").where("ticket_id", ticketId).first();
        if (existing) {
            throw new Exception("CSAT response already submitted", {
                status: 409,
                code: "E_SUPPORT_CSAT_EXISTS",
            });
        }

        const [row] = await currentTrx()
            .table("support_csat_responses")
            .insert({
                ticket_id: ticketId,
                score: input.score,
