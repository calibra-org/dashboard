...
function canonicalFromHtml(html: string, base: URL): string | null {
    const match = /<link\b[^>]*\brel=["'][^"']*canonical[^"']*["'][^>]*\bhref=["']([^"']+)["'][^>]*>|<link\b[^>]*\bhref=["']([^"']+)["'][^>]*\brel=["'][^"']*canonical[^"']*["'][^>]*>/i.exec(html);
...
async function resolvePublicTarget(value: string, baseUrl: string) {
    ...
    if (!["http:", "https:"].includes(url.protocol) || url.username || url.password) {
        throw new Exception("Crawl only allows credential-free HTTP(S) URLs", { status: 422, code: "E_SEO_CRAWL_URL" });
    }
    if (normalizeContentSourceHostname(url.hostname) !== normalizeContentSourceHostname(base.hostname)) {
        throw new Exception("Crawl URL must use the configured SEO site hostname", { status: 422, code: "E_SEO_CRAWL_SCOPE" });
    }
    ...
    if (["localhost", "localhost.localdomain"].includes(hostname) || hostname.endsWith(".localhost") || hostname.endsWith(".local") || hostname.endsWith(".internal")) {
        throw new Exception("Private crawl targets are blocked", { status: 422, code: "E_SEO_CRAWL_PRIVATE" });
    }
    const directFamily = isIP(hostname);
    let addresses: Array<{ address: string; family: number }>;
    try {
        addresses = directFamily ? [{ address: hostname, family: directFamily }] : await lookup(hostname, { all: true, verbatim: true });
    } catch {
        throw new Exception("Crawl hostname could not be resolved", { status: 422, code: "E_SEO_CRAWL_DNS" });
    }
    if (addresses.length === 0 || addresses.some((entry) => isPrivateContentSourceAddress(entry.address))) {
        throw new Exception("Private crawl targets are blocked", { status: 422, code: "E_SEO_CRAWL_PRIVATE" });
    }
    const selected = addresses[0];
    if (!selected) throw new Exception("Crawl hostname could not be resolved", { status: 422, code: "E_SEO_CRAWL_DNS" });
...
async function requestDocument(value: string, baseUrl: string, maxBytes = 2_000_000): Promise<SeoCrawlFetchResult> {
    ...
    return await new Promise<SeoCrawlFetchResult>((resolve, reject) => {
        ...
        const request = transport(target.url, {
            ...
            const contentType = response.headers["content-type"] ? String(response.headers["content-type"]).slice(0, 255) : null;
            ...
            response.on("end", () => {
                ...
                resolve({
                    ...
                    canonicalUrl,
                    robotsMeta,
                    indexable: isHtml ? statusCode >= 200 && statusCode < 300 && !/(^|[,\s])noindex([,\s]|$)/.test(directives) : null,
...
export async function prepareSeoCrawlTarget(targetId: number): Promise<PreparedSeoCrawlTarget | null> {
    ...
    if (!run) return null;
    await trx.from("seo_crawl_targets").where("id", targetId).update({ status: "processing", attempts: numberValue(target.attempts) + 1, claimed_at: new Date(), updated_at: new Date() });
    await trx.from("seo_crawl_runs").where("id", run.id).update({ status: "running", started_at: run.started_at ?? new Date(), updated_at: new Date() });
    return { targetId, runId: numberValue(run.id), url: String(target.url), baseUrl: String(run.base_url) };
}
...
async function finalizeRun(runId: number) {
    ...
    const queued = (map.get("queued") ?? 0) + (map.get("processing") ?? 0);
    const completed = map.get("completed") ?? 0;
    const failed = map.get("failed") ?? 0;
    const status = queued > 0 ? "running" : failed > 0 && completed > 0 ? "partial" : failed > 0 ? "failed" : "completed";
    await trx.from("seo_crawl_runs").where("id", runId).update({
        status,
        completed_count: completed,
        failed_count: failed,
        finished_at: queued === 0 ? new Date() : null,
        updated_at: new Date(),
    });
}
...
export async function completeSeoCrawlTarget(prepared: PreparedSeoCrawlTarget, fetched: SeoCrawlFetchResult) {
    const trx = currentTrx();
    await trx.table("seo_crawl_observations").insert({
        ...
        fetch_status: fetched.statusCode >= 200 && fetched.statusCode < 300 ? "success" : "http_error",
        error_evidence: fetched.statusCode >= 200 && fetched.statusCode < 300 ? null : `HTTP ${fetched.statusCode}`,
        fetched_at: new Date(),
    }).onConflict(["tenant_id", "crawl_run_id", "url"]).merge();
    await trx.from("seo_crawl_targets").where("id", prepared.targetId).update({ status: "completed", last_error: null, finished_at: new Date(), updated_at: new Date() });
...
export async function failSeoCrawlTarget(prepared: PreparedSeoCrawlTarget, error: unknown) {
    ...
    await trx.from("seo_crawl_targets").where("id", prepared.targetId).update({ status: "failed", last_error: message, finished_at: new Date(), updated_at: new Date() });
    await finalizeRun(prepared.runId);
}
