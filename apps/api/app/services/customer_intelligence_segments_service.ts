...
function validateDefinition(value: unknown): SegmentDefinition {
    ...
    for (const condition of candidate.conditions) {
        if (!condition || typeof condition !== "object" || !FEATURE_SQL[condition.feature] || !["eq", "neq", "gt", "gte", "lt", "lte", "in"].includes(condition.operator)) {
            throw new Exception("Segment definition contains an unsupported condition", {
                status: 422,
                code: "E_SEGMENT_DEFINITION",
            });
        }
        if (condition.operator === "in" && (!Array.isArray(condition.value) || condition.value.length === 0 || condition.value.length > 50)) {
            throw new Exception("Segment IN condition must contain between 1 and 50 values", {
                status: 422,
                code: "E_SEGMENT_DEFINITION",
            });
        }
...
function compileDefinition(definition: SegmentDefinition) {
    if (definition.conditions.length === 0) return { sql: "TRUE", bindings: [] as unknown[] };
...
export async function saveSegmentDefinition(input: {
    ...
    await currentTrx().rawQuery(
        ...
        [String(currentTenantId()), input.segmentId, input.kind, JSON.stringify(definition), input.refreshPolicy, version, now, now],
    );
...
export async function previewSegment(segmentId: number, userId: number) {
    await findOwnedSegment(segmentId, userId);
    const row = await currentTrx().from("customer_segment_definitions").where("segment_id", segmentId).first();
    if (!row) throw new Exception("This saved view has no dynamic segment definition", { status: 409, code: "E_SEGMENT_SAVED_VIEW" });
...
export async function evaluateSegment(segmentId: number, userId: number) {
    ...
    const now = DateTime.utc().toISO()!;
    await currentTrx().from("customer_segment_definitions").where("segment_id", segmentId).update({ status: "evaluating", updated_at: now });
    await currentTrx().from("customer_segment_memberships").where("segment_id", segmentId).delete();
    if (ids.length > 0) {
        await currentTrx().table("customer_segment_memberships").multiInsert(
...
export async function reconcileCustomerEventDrivenSegments(customerId: number): Promise<void> {
    const definitions = await currentTrx().from("customer_segment_definitions").where("refresh_policy", "event_driven").select("segment_id", "definition");
    for (const row of definitions) {
        ...
        if (matched) {
            ...
            await currentTrx().from("customer_segment_memberships").where("segment_id", segmentId).where("customer_id", customerId).delete();
        }
        const countRow = await currentTrx().from("customer_segment_memberships").where("segment_id", segmentId).count("* as total").first();
        await currentTrx().from("customer_segment_definitions").where("segment_id", segmentId).update({
            member_count: Number(countRow?.total ?? 0),
            last_evaluated_at: DateTime.utc().toISO(),
            status: "ready",
