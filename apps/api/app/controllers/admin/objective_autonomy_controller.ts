...
export default class ObjectiveAutonomyController {
    ...
    async createObjective(ctx: HttpContext) {
        ...
        await recordAudit({ ctx, actorUserId: Number(user.id), action: "objective_autonomy.objective.create", entityKind: "autonomy_objective", entityId: data.id, payload: { public_id: data.public_id, target_metric: data.target_metric, reason: payload.reason }, strict: true });
    ...
    async activate(ctx: HttpContext) {
        ...
        await recordAudit({ ctx, actorUserId: Number(user.id), action: "objective_autonomy.objective.activate", entityKind: "autonomy_objective", entityId: data.id, payload: { reason: payload.reason }, strict: true });
        return { data };
    }
    ...
    async halt(ctx: HttpContext) {
        ...
        await recordAudit({ ctx, actorUserId: Number(user.id), action: "objective_autonomy.objective.halt", entityKind: "autonomy_objective", entityId: data.id, payload: { reason: payload.reason }, strict: true });
        return { data };
    }
    ...
    async startCycle(ctx: HttpContext) {
        ...
        await recordAudit({ ctx, actorUserId: Number(user.id), action: "objective_autonomy.cycle.start", entityKind: "autonomy_cycle", entityId: data.id, payload: { objective_public_id: ctx.params.publicId, seed: payload.seed ?? null }, strict: true });
        return ctx.response.created({ data });
    }
    ...
    async executeStep(ctx: HttpContext) {
        ...
        const data = await autonomy.executeObjectiveStep({ objectivePublicId: ctx.params.publicId, cyclePublicId: ctx.params.cyclePublicId, stepPublicId: payload.step_public_id, idempotencyKey: payload.idempotency_key, dryRun: payload.dry_run, actor: user, stepUpSatisfied });
        await recordAudit({ ctx, actorUserId: Number(user.id), action: "objective_autonomy.step.execute", entityKind: "agent_tool_run", entityId: data?.id ?? null, payload: { objective_public_id: ctx.params.publicId, cycle_public_id: ctx.params.cyclePublicId, step_public_id: payload.step_public_id, dry_run: payload.dry_run }, strict: true });
        return { data };
    }
    ...
    async checkpoint(ctx: HttpContext) {
        ...
        await recordAudit({ ctx, actorUserId: Number(user.id), action: "objective_autonomy.checkpoint.record", entityKind: "autonomy_checkpoint", entityId: data.checkpoint.id, payload: { decision: data.control.decision, evidence_count: payload.evidence_refs.length, reason: payload.reason }, strict: true });
        return ctx.response.created({ data });
    }
    ...
    async postmortem(ctx: HttpContext) {
        ...
        await recordAudit({ ctx, actorUserId: Number(user.id), action: "objective_autonomy.postmortem.create", entityKind: "autonomy_postmortem", entityId: data.postmortem.id, payload: { memory_public_id: data.memory.public_id, evidence_count: payload.evidence_refs.length }, strict: true });
        return ctx.response.created({ data });
    }
    ...
    async accessPreset(ctx: HttpContext) {
        ...
        await recordAudit({ ctx, actorUserId: Number(user.id), action: "objective_autonomy.access.preset.apply", entityKind: "admin_user", entityId: payload.user_id, payload: { preset: payload.preset, reason: payload.reason }, strict: true });
        return { data };
    }
}
