...
import {
    ...
    refreshPlanningAccuracyValidator,
...
function optionalRequestId(value: unknown, code: string): number | null {
    return value === undefined || value === null || value === "" ? null : id(value, code);
}
...
export default class PlanningController {
    ...
    async forecast({ request }: HttpContext) {
        return phase13PlanningService.forecast(optionalRequestId(request.input("run_id"), "E_PLANNING_RUN_ID"));
    }
    ...
    async runForecast(ctx: HttpContext) {
        ...
        await recordAudit({
            ...
            payload: {
                history_days: payload.history_days ?? 84,
                horizon_days: payload.horizon_days ?? 28,
                review_period_days: payload.review_period_days ?? 7,
                default_lead_time_days: payload.default_lead_time_days ?? null,
                service_level_target: payload.service_level_target ?? 0.9,
            },
        });
        return result;
    }

    async categoryForecast({ request }: HttpContext) {
        return phase13PlanningService.categoryForecast(optionalRequestId(request.input("run_id"), "E_PLANNING_RUN_ID"));
    }

    async recommendations({ request }: HttpContext) {
        return phase13PlanningService.recommendations(optionalRequestId(request.input("run_id"), "E_PLANNING_RUN_ID"));
    }
    ...
    async refreshAccuracy(ctx: HttpContext) {
        const payload = await ctx.request.validateUsing(refreshPlanningAccuracyValidator);
        const result = await phase13PlanningService.refreshAccuracy(payload.run_id ?? null);
        await recordAudit({
            ctx,
            action: "planning.accuracy.refresh",
            entityKind: "planning_forecast_run",
            entityId: result.data.run_id,
            payload: { evaluated_points: result.data.evaluated_points },
        });
        return result;
    }
