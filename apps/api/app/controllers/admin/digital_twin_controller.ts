...
import { createDigitalTwinScenarioValidator, runDigitalTwinValidator, updateDigitalTwinScenarioValidator } from "#validators/admin/phase23_digital_twin_validator";

export default class DigitalTwinController {
    async overview({ response }: HttpContext) { return response.ok({ data: await twin.overview() }); }
    async scenarios({ response }: HttpContext) { return response.ok({ data: await twin.listScenarios() }); }
    ...
    async run({ params, response }: HttpContext) { return response.ok({ data: await twin.runDetail(params.publicId) }); }
    async compare({ request, response }: HttpContext) {
        const ids = String(request.input("runs", "")).split(",").map((v) => v.trim()).filter(Boolean);
        return response.ok({ data: await twin.compareRuns(ids) });
    }
    async sensitivity({ params, response }: HttpContext) { return response.ok({ data: await twin.sensitivity(params.publicId) }); }
    async brief({ params, response }: HttpContext) { return response.ok({ data: await twin.decisionBrief(params.publicId) }); }
}
