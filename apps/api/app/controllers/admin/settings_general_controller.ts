...
export default class AdminSettingsGeneralController {
    ...
    private assertCrossFieldRules(
        ...
        if (currency?.display !== undefined && !enabledCurrencyCodes.has(currency.display)) {
            throw new BusinessRuleException("Display currency is not enabled", "currency.display.disabled", { field: "currency.display" });
        }
        ...
        if (thousandSep === decimalSep) {
            throw new BusinessRuleException("Thousand and decimal separators must differ", "currency.separators.equal", { field: "currency.decimal_sep" });
        }
        const address = payload.store_address;
        if (address?.country !== undefined && address.country !== "" && !countryCodes.has(address.country)) {
            throw new BusinessRuleException("Unknown country", "store_address.country.unknown", { field: "store_address.country" });
        }
    ...
    private planWrites(payload: Awaited<ReturnType<typeof adminGeneralSettingsUpdateValidator.validate>>): PlannedWrite[] {
        ...
        if (addr) {
            if (addr.address_1 !== undefined) writes.push({ group: "general", key: "store_address_1", value: addr.address_1, type: "string" });
            if (addr.address_2 !== undefined) writes.push({ group: "general", key: "store_address_2", value: addr.address_2, type: "string" });
            if (addr.city !== undefined) writes.push({ group: "general", key: "store_city", value: addr.city, type: "string" });
            if (addr.state !== undefined) writes.push({ group: "general", key: "store_state", value: addr.state, type: "string" });
            if (addr.postcode !== undefined) writes.push({ group: "general", key: "store_postcode", value: addr.postcode, type: "string" });
            if (addr.country !== undefined) writes.push({ group: "general", key: "country_default", value: addr.country, type: "string" });
        }
        const opts = payload.general_options;
        if (opts) {
            if (opts.selling_locations !== undefined) writes.push({ group: "general", key: "selling_locations", value: opts.selling_locations, type: "string" });
            if (opts.selling_locations_specific !== undefined) writes.push({ group: "general", key: "selling_locations_specific", value: opts.selling_locations_specific, type: "json" });
            if (opts.selling_locations_excluded !== undefined) writes.push({ group: "general", key: "selling_locations_excluded", value: opts.selling_locations_excluded, type: "json" });
            if (opts.shipping_locations !== undefined) writes.push({ group: "general", key: "shipping_locations", value: opts.shipping_locations, type: "string" });
            if (opts.shipping_locations_specific !== undefined) writes.push({ group: "general", key: "shipping_locations_specific", value: opts.shipping_locations_specific, type: "json" });
            if (opts.default_customer_location !== undefined) writes.push({ group: "general", key: "default_customer_location", value: opts.default_customer_location, type: "string" });
        }
        const tc = payload.taxes_and_coupons;
        if (tc) {
            if (tc.taxes_enabled !== undefined) writes.push({ group: "tax", key: "enabled", value: tc.taxes_enabled, type: "boolean" });
            if (tc.coupons_enabled !== undefined) writes.push({ group: "tax", key: "coupons_enabled", value: tc.coupons_enabled, type: "boolean" });
            if (tc.calc_discounts_sequentially !== undefined) writes.push({ group: "tax", key: "calc_discounts_sequentially", value: tc.calc_discounts_sequentially, type: "boolean" });
        }
        const cur = payload.currency;
        if (cur) {
            if (cur.display !== undefined) writes.push({ group: "general", key: "currency_display_default", value: cur.display, type: "string", currency: true });
            if (cur.position !== undefined) writes.push({ group: "general", key: "currency_position", value: cur.position, type: "string", currency: true });
            if (cur.thousand_sep !== undefined) writes.push({ group: "general", key: "price_thousand_sep", value: cur.thousand_sep, type: "string", currency: true });
            if (cur.decimal_sep !== undefined) writes.push({ group: "general", key: "price_decimal_sep", value: cur.decimal_sep, type: "string", currency: true });
            if (cur.num_decimals !== undefined) writes.push({ group: "general", key: "price_num_decimals", value: cur.num_decimals, type: "number", currency: true });
        }
        return writes;
    }
}
