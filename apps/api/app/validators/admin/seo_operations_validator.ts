...
export const seoActionCreateValidator = vine.compile(vine.object({
...
export const seoActionReviewValidator = vine.compile(vine.object({
    decision: vine.enum(["approved", "rejected"] as const),
    note: vine.string().trim().maxLength(2000).optional().nullable(),
}));

export const seoMediaBulkAltValidator = vine.compile(vine.object({
...
export const seoCrawlCreateValidator = vine.compile(vine.object({
    urls: vine.array(vine.string().trim().url()).minLength(1).maxLength(100),
}));

export const seoExportCreateValidator = vine.compile(vine.object({
