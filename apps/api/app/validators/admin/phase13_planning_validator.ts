...
export const runPlanningForecastValidator = vine.compile(
    vine.object({
        history_days: vine.number().withoutDecimals().min(28).max(365).optional(),
        horizon_days: vine.number().withoutDecimals().min(1).max(90).optional(),
        review_period_days: vine.number().withoutDecimals().min(1).max(60).optional(),
        default_lead_time_days: vine.number().withoutDecimals().min(0).max(365).nullable().optional(),
        service_level_target: vine.number().min(0.9).max(0.9).optional(),
    }),
);

export const refreshPlanningAccuracyValidator = vine.compile(
    vine.object({
        run_id: vine.number().withoutDecimals().positive().optional(),
    }),
);
...
export const createPlanningScenarioValidator = vine.compile(
    vine.object({
        ...
        lead_time_days: vine.number().withoutDecimals().min(0).max(365).nullable().optional(),
        review_period_days: vine.number().withoutDecimals().min(1).max(60).optional(),
