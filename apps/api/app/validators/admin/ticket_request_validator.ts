...
export const adminTicketCreateValidator = vine.compile(vine.object({
...
export const adminTicketUpdateValidator = vine.compile(vine.object({
...
export const adminTicketTransitionValidator = vine.compile(vine.object({
    status: vine.enum(TICKET_STATUSES),
    expected_version: expectedVersion(),
    reason: nullableText(1000),
}));

export const adminTicketMessageValidator = vine.compile(vine.object({
    kind: vine.enum(["reply", "internal_note"] as const),
    body: vine.string().trim().minLength(1).maxLength(20_000),
    expected_version: expectedVersion(),
}));

export const adminTicketSettingsValidator = vine.compile(vine.object({
    reference_prefix: vine.string().trim().minLength(1).maxLength(12).regex(/^[A-Za-z0-9-]+$/).optional(),
...
export const adminTicketResourceValidator = vine.compile(vine.object({
