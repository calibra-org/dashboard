...
const returnItem = vine.object({
    ...
    refund_amount_minor: vine.number().min(0).optional().nullable(),
});
    ...
    vine.object({
        ...
        items: vine
            .array(
                vine.object({
                    order_line_item_id: vine.number().positive(),
                    approved_quantity: vine.number().min(0).max(10_000),
                }),
            )
            .minLength(1),
    }),
    ...
    vine.object({
        ...
        items: vine
            .array(
                vine.object({
                    order_line_item_id: vine.number().positive(),
                    received_quantity: vine.number().min(0).max(10_000),
                    damaged_quantity: vine.number().min(0).max(10_000),
                    restock_quantity: vine.number().min(0).max(10_000),
                }),
            )
            .minLength(1),
    }),
...
export const inventoryMovementListValidator = vine.compile(
    vine.object({
        inventory_item_id: vine.number().positive(),
...
export const inventoryAdjustmentValidator = vine.compile(
    vine.object({
        inventory_item_id: vine.number().positive(),
        quantity_delta: vine.number().min(-1_000_000).max(1_000_000),
...
export const shippingZoneMethodCreateValidator = vine.compile(
    vine.object({
        method_id: vine.number().positive(),
        title_override: vine.string().trim().maxLength(160).optional().nullable(),
        enabled: vine.boolean().optional(),
        ordering: vine.number().min(0).max(100_000).optional(),
...
export const taxRateCreateValidator = vine.compile(
    vine.object({
        tax_class_id: vine.number().positive(),
        ...
        priority: vine.number().min(0).max(1000).optional(),
...
export const taxRateUpdateValidator = vine.compile(
    vine.object({
        tax_class_id: vine.number().positive().optional(),
        country: vine.string().trim().maxLength(2).optional().nullable(),
        region_id: vine.number().positive().optional().nullable(),
        postcodes: vine.array(vine.string().trim().maxLength(32)).optional(),
        cities: vine.array(vine.string().trim().maxLength(120)).optional(),
