...
export const ticketSavedViewCreateValidator = vine.compile(vine.object({
    name: vine.string().trim().minLength(1).maxLength(120),
    query: jsonObject(),
    is_shared: vine.boolean().optional(),
}));

export const ticketSavedViewUpdateValidator = vine.compile(vine.object({
    name: vine.string().trim().minLength(1).maxLength(120).optional(),
    query: jsonObject().optional(),
    is_shared: vine.boolean().optional(),
}));

export const ticketBulkOperationValidator = vine.compile(vine.object({
    tickets: vine.array(vine.object({ id: positiveId(), expected_version: version() })).minLength(1).maxLength(100),
...
export const ticketAttachmentValidator = vine.compile(vine.object({
    media_id: positiveId(),
    message_id: positiveId().nullable().optional(),
    sha256: vine.string().trim().regex(/^[a-f0-9]{64}$/i).nullable().optional(),
}));

export const ticketAttachmentScanValidator = vine.compile(vine.object({
    status: vine.enum(["clean", "infected", "error"] as const),
    evidence: vine.string().trim().maxLength(512).nullable().optional(),
}));

export const ticketMergeValidator = vine.compile(vine.object({
...
export const ticketPresenceValidator = vine.compile(vine.object({
    state: vine.enum(["offline", "available", "busy", "away"] as const),
    capacity: vine.number().withoutDecimals().min(0).max(500),
}));

export const ticketChannelUpdateValidator = vine.compile(vine.object({
    channel: vine.enum(["web", "email", "phone", "api", "whatsapp", "telegram", "instagram", "rubika", "bale", "eitaa", "sms"] as const),
    enabled: vine.boolean(),
    credential_env_ref: vine.string().trim().regex(/^CALIBRA_SUPPORT_[A-Z0-9_]{1,120}$/).nullable().optional(),
    configuration: jsonObject().optional(),
}));

export const ticketRuleCreateValidator = vine.compile(vine.object({
...
export const ticketRuleUpdateValidator = vine.compile(vine.object({
...
export const ticketAutomationCreateValidator = vine.compile(vine.object({
    name: vine.string().trim().minLength(1).maxLength(140),
...
export const ticketAutomationUpdateValidator = vine.compile(vine.object({
    expected_version: version(),
    name: vine.string().trim().minLength(1).maxLength(140).optional(),
    enabled: vine.boolean().optional(),
    conditions: jsonObject().optional(),
...
export const ticketCampaignCreateValidator = vine.compile(vine.object({
    ...
    scheduled_at: vine.date({ formats: ["iso8601"] }).nullable().optional(),
}));

export const ticketCampaignRecipientsValidator = vine.compile(vine.object({
    expected_version: version(),
    recipients: vine.array(vine.string().trim().minLength(1).maxLength(254)).minLength(1).maxLength(5000),
}));

export const ticketCampaignTransitionValidator = vine.compile(vine.object({
    expected_version: version(),
    status: vine.enum(["scheduled", "paused", "cancelled"] as const),
}));

export const ticketWorkflowStatusValidator = vine.compile(vine.object({
    code: vine.string().trim().regex(/^[a-z][a-z0-9_]{1,47}$/),
