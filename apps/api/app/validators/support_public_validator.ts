...
export const publicTicketCreateValidator = vine.compile(vine.object({
...
export const publicTicketReplyValidator = vine.compile(vine.object({
    expected_version: vine.number().withoutDecimals().positive(),
    body: vine.string().trim().minLength(1).maxLength(20_000),
}));

export const publicTicketCsatValidator = vine.compile(vine.object({
