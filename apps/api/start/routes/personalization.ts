...
router
    .group(() => {
        ...
        router.post("/events", [PersonalizationController, "event"]).as("personalization.events.create").use(contentPublicLimiter);
        router.get("/consent", [PersonalizationController, "consent"]).as("personalization.consent.show");
        router.put("/consent", [PersonalizationController, "updateConsent"]).as("personalization.consent.update").use(contentPublicLimiter);
        router.get("/preferences", [PersonalizationController, "preferences"]).as("personalization.preferences.show");
        router.put("/preferences", [PersonalizationController, "updatePreferences"]).as("personalization.preferences.update").use(contentPublicLimiter);
...
router
    .group(() => {
        ...
        router.post("/events/batch", [PersonalizationController, "eventBatch"]).as("phase9.events.batch").use(contentPublicLimiter);
        router.post("/recommendations/serve", [PersonalizationController, "serve"]).as("phase9.recommendations.serve").use(contentPublicLimiter);
        router.post("/recommendations/serve-page", [PersonalizationController, "servePage"]).as("phase9.recommendations.servePage").use(contentPublicLimiter);
    })
    .prefix("/api/v1");
