...
router
    .group(() => {
        ...
        router.post("/scenarios", [DigitalTwinController, "createScenario"]).as("admin.digital_twin.scenarios.create").use(adminWriteLimiter);
        router.put("/scenarios/:publicId", [DigitalTwinController, "updateScenario"]).as("admin.digital_twin.scenarios.update").use(adminWriteLimiter);
        router.post("/scenarios/:publicId/run", [DigitalTwinController, "runScenario"]).as("admin.digital_twin.run.create").use(adminWriteLimiter);
        ...
        router.get("/runs/:publicId/brief", [DigitalTwinController, "brief"]).as("admin.digital_twin.brief");
    })
