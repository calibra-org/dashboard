...
router
    .group(() => {
        ...
        router.patch("/general", [AdminSettingsGeneralController, "update"]).as("admin.settings.general.update").use(adminWriteLimiter);
        router.get("/datetime", [AdminSettingsDatetimeController, "show"]).as("admin.settings.datetime.show");
        router.patch("/datetime", [AdminSettingsDatetimeController, "update"]).as("admin.settings.datetime.update").use(adminWriteLimiter);
        ...
        router.patch("/branding", [AdminSettingsBrandingController, "update"]).as("admin.settings.branding.update").use(adminWriteLimiter);

        router.get("/configuration/registry", [AdminConfigurationController, "registry"]).as("admin.settings.configuration.registry");
        router.get("/configuration/definitions", [AdminConfigurationController, "definitions"]).as("admin.settings.configuration.definitions");
        router.get("/configuration/groups/:group", [AdminConfigurationController, "group"]).as("admin.settings.configuration.group");
        router.post("/configuration/groups/:group/preview", [AdminConfigurationController, "preview"]).as("admin.settings.configuration.preview").use(adminWriteLimiter);
        router.post("/configuration/groups/:group/test", [AdminConfigurationController, "test"]).as("admin.settings.configuration.test").use(adminWriteLimiter);
        router.put("/configuration/groups/:group", [AdminConfigurationController, "update"]).as("admin.settings.configuration.update").use(adminWriteLimiter);
        router.get("/configuration/history", [AdminConfigurationController, "history"]).as("admin.settings.configuration.history");
        router.get("/configuration/history/:scope/:revision", [AdminConfigurationController, "show"]).as("admin.settings.configuration.history.show");
