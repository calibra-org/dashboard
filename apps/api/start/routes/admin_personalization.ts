...
router
    .group(() => {
        ...
        router.post("/campaigns", [Controller, "createCampaign"]).as("admin.personalization.campaigns.create").use(adminWriteLimiter);
        router.patch("/campaigns/:id", [Controller, "updateCampaign"]).as("admin.personalization.campaigns.update").use(adminWriteLimiter);
        router.post("/campaigns/:id/publish", [Controller, "publishCampaign"]).as("admin.personalization.campaigns.publish").use(adminWriteLimiter);
        router.post("/campaigns/:id/pause", [Controller, "pauseCampaign"]).as("admin.personalization.campaigns.pause").use(adminWriteLimiter);
        router.post("/campaigns/:id/transition/:target", [Controller, "transitionCampaign"]).as("admin.personalization.campaigns.transition").use(adminWriteLimiter);

        router.get("/settings", [Controller, "settings"]).as("admin.personalization.settings.show");
        router.patch("/settings", [Controller, "updateSettings"]).as("admin.personalization.settings.update").use(adminWriteLimiter);
        router.get("/placements", [Controller, "placements"]).as("admin.personalization.placements.index");
        router.patch("/placements/:placement", [Controller, "updatePlacement"]).as("admin.personalization.placements.update").use(adminWriteLimiter);
        ...
        router.post("/registry/:kind/:key/:version/activate", [Controller, "activateRegistry"]).as("admin.personalization.registry.activate").use(adminWriteLimiter);
        router.post("/registry/:kind/:key/rollback", [Controller, "rollbackRegistry"]).as("admin.personalization.registry.rollback").use(adminWriteLimiter);
