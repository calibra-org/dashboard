...
for (const token of ["incomplete", "realized", "forecast", "reversal_of_id", "Idempotency-Key", "pg_advisory_xact_lock", "unit_landed_cost_minor === null"]) {
  if (!service.includes(token) && !readFileSync(required[2], "utf8").includes(token)) throw new Error(`Phase 12 service invariant missing: ${token}`);
}
