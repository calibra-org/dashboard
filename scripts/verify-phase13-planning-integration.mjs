...
for (const table of [
    ...
    expect(migration.includes(`FORCE ROW LEVEL SECURITY`) && migration.includes(table), `${table} must participate in tenant RLS`);
}
expect(migration.includes("p10_quantity >= 0 AND p50_quantity >= p10_quantity AND p90_quantity >= p50_quantity"), "forecast quantile ordering must be database-constrained");
expect(migration.includes('table.bigInteger("location_id").unsigned().nullable()'), "planning must retain the current advisory location dimension");
...
expect(service.includes('"li.sku_snapshot"') && service.includes('"li.name_snapshot"'), "planning demand must use authoritative historical line snapshots");
...
expect(service.includes("product_category_links") && service.includes("same_versioned_forecast_points"), "category forecast must derive from the same versioned point truth");
...
expect(route.includes('middleware.auth({ guards: ["api"] })') && route.includes("middleware.admin()"), "planning routes must enforce API auth and admin RBAC");
...
expect(service.includes("actual_censored: actualCensored"), "accuracy refresh must persist censoring state instead of treating stockout zeros as full demand");
...
expect(workspace.includes("P10") && workspace.includes("P50") && workspace.includes("P90"), "control tower must visualize probabilistic forecast bands");
expect(workspace.includes("Phase 12") && workspace.includes("Phase 14"), "control tower must disclose hard dependency and execution boundaries");
