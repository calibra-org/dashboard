...
must(reviewedGuard >= 0 && scorerGuard > reviewedGuard && promiseGuard > scorerGuard && finalize > promiseGuard && consume > finalize, "Checkout ordering must be Trust -> Promise revalidation -> canonical finalization -> evidence consumption");
...
for (const boundary of ["does **not** become the fulfillment", "Stale/unknown stock returns no promise", "Phase 20 reviewed trust guard", "must not invent ETA confidence"]) must(posture.includes(boundary), `Phase31 posture missing ${boundary}`);
for (const operationId of ["adminFulfillmentPromiseOverview", "adminFulfillmentPromiseNodes", "adminFulfillmentPromiseAccuracy", "adminFulfillmentPromiseAccessPreset"]) must(adminOpenapi.includes(operationId), `Admin OpenAPI missing ${operationId}`);
for (const operationId of ["storefrontFulfillmentPromiseQuote", "storefrontFulfillmentPromiseSelect"]) must(storefrontOpenapi.includes(operationId), `Storefront OpenAPI missing ${operationId}`);
