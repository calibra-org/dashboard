...
for (const token of [
    ...
    if (!migration.includes(token)) throw new Error(`Phase20 RLS loop invariant missing ${token}`);
...
for (const token of ["calculateRiskDecision", "idempotency_key", "control.block", "model.promote_champion", "[redacted]"])
    if (!service.includes(token)) throw new Error(`Phase20 invariant missing ${token}`);
...
if (
    /#[0-9a-f]{3,8}\b|rgba?\(|\b(?:slate|gray|zinc|neutral|stone|red|orange|amber|yellow|lime|green|emerald|teal|cyan|sky|blue|indigo|violet|purple|fuchsia|pink|rose)-\d+/i.test(
        ui,
    )
)
    throw new Error("Phase20 TrustWorkspace contains non-token color");
