...
must(
    service.includes("misleading_memory_rate") && service.includes("source_linked_retrieval_rate"),
    "Phase 26 effectiveness KPIs incomplete",
);
must(
    read("apps/api/app/controllers/admin/merchant_memory_controller.ts").includes(
        "agentPrincipal = await requireApprovedAgentPrincipal",
    ),
    "Governance principal is not bound to retrieval logging",
);
