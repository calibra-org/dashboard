...
check(sidebar.indexOf("/products/amazing-deals") > sidebar.indexOf('href: "/products"'), "amazing deals must appear after Products");
...
const docsAdmin = read("docs/api/reference/openapi/admin.phase9.v1.yaml"), docsStore = read("docs/api/reference/openapi/storefront.phase9.v1.yaml");
for (const needle of ["/api/v1/admin/personalization/features", "/api/v1/admin/personalization/models", "/api/v1/admin/personalization/rollouts"])
    check(docsAdmin.includes(needle), `admin Phase9 OpenAPI missing ${needle}`);
for (const needle of ["/api/v1/events", "/api/v1/events/batch", "/api/v1/recommendations/serve", "/api/v1/recommendations/serve-page"])
