...
must(docsPackage.includes("pnpm build:json:admin-phase32 && pnpm build:json:admin-merge"), "Phase32 overlay is not in canonical admin build order");
must(mergeAdmin.includes('dist/admin.phase32.v1.json') && mergeAdmin.includes("Phase32ReliabilityGuardianOverlay"), "Phase32 overlay is not merged into canonical admin OpenAPI");
must(posture.includes("Missing samples produce `no_evidence`"), "Phase32 conformance posture must forbid invented reliability evidence");
