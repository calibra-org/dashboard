...
contains("apps/api/app/services/support/public_support_service.ts", "randomBytes(32)", "public support must issue opaque random tokens");
contains("apps/api/app/services/support/public_support_service.ts", "createHash(\"sha256\")", "public support must hash tracking tokens");
contains("apps/api/app/services/support/public_support_service.ts", '.whereIn("kind", ["requester_message", "reply"])', "public support must exclude internal notes");
contains("apps/api/app/services/support/public_support_service.ts", '.where("scan_status", "clean")', "public support must exclude unscanned attachments");
contains("apps/api/app/services/support/ticket_operations_service.ts", "E_TICKET_MERGE_LOOP", "ticket merge must reject loops");
contains("apps/api/app/services/support/ticket_operations_service.ts", "credential_env_ref", "channel secrets must use environment references");
contains("apps/api/app/services/support/ticket_operations_service.ts", 'String(channel.status) !== "connected"', "campaign scheduling must require a verified channel");
...
contains("apps/api/app/services/seo/operations_service.ts", 'String(action.status) !== "approved"', "SEO actions must require approval before apply");
contains("apps/api/app/services/seo/operations_service.ts", "E_SEO_ROLLBACK_CONFLICT", "SEO rollback must conflict on drift");
contains("apps/api/app/services/seo/operations_service.ts", "contentService.update", "SEO content refresh must use Content OS update/revision path");
contains("apps/api/app/services/seo/crawl_service.ts", "isPrivateContentSourceAddress", "SEO crawler must block private network addresses");
contains("apps/api/app/services/seo/crawl_service.ts", "pinnedLookup", "SEO crawler must pin resolved DNS address");
contains("apps/api/app/services/seo/crawl_service.ts", "crawl redirects are not followed", "SEO crawler must not follow redirects silently");
contains("apps/api/commands/seo_run_crawls.ts", "withJobTenantContext", "SEO crawl worker must re-enter tenant RLS context around DB phases");
...
contains("apps/api/app/services/content/news_service.ts", "contentService.publicDetail", "public News detail must use the public Content OS boundary");
