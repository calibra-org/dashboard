...
must(
    routes.includes('await import("./routes/fulfillment_promise_storefront.js")'),
    "Phase31 storefront routes are not materialized",
);
must(
    checkout.includes('from "#services/fulfillment_promise/promise_service"'),
    "Phase31 checkout promise guard import is missing",
);
...
must(
    checkout.includes("await fulfillmentCapacity.commitPromiseCapacity(selectedPromiseId"),
    "Phase31 capacity commit is missing",
);
must(
    checkout.includes("await fulfillmentPromise.commitOrderPromise(result.order, selectedPromiseId)"),
    "Phase31 promise evidence commit is missing",
);
...
must(
    ...
    "Phase31 admin OpenAPI merge is missing",
);
