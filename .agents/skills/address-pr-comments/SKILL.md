...
7. Commit each resolved comment.
8. Push the resulting commits to the PR branch.
...
3. Create exactly one commit for that resolved comment.
...
Once all resolved comments have been committed and the targeted checks (step 6) are green, push the new commits to the PR branch:

```bash
git push
```

Push regular (no force, no `--force-with-lease`) — appended commits are how reviewers track which feedback you addressed and in what order. If the push is rejected because the remote moved, fetch and rebase rather than force-pushing unless the user explicitly asks for a rewrite.

If a commit fails its checks at step 6, fix forward with another commit; do **not** amend or drop the failing one. Only push once every commit you intend to ship is in place — pushing mid-flight just to update the remote is fine, but the workflow should not exit until at least one push has succeeded so the PR reflects the addressed feedback.
...
# Repeat for each resolved comment, then:
git push
```
