...
def verifier_patch(s):
    s = s.replace('import { readFileSync, existsSync } from "node:fs";', 'import { existsSync, readFileSync } from "node:fs";')
    s = s.replace('import { existsSync, readFileSync } from "node:fs";\nconst must', 'import { existsSync, readFileSync } from "node:fs";\n\nconst must')
    return s
...
edit('scripts/verify-phase16-discovery-integration.mjs', verifier_patch)
