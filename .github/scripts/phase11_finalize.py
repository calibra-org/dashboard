...
marker = "type Row = Record<string, any>;\n\n"
helper = """type Row = Record<string, any>;
...
if source.count(marker) != 1:
    raise SystemExit("Row marker not found exactly once")
source = source.replace(marker, helper, 1)

old_append = """        const material = {
            tenantId,
            sequence,
            eventId,
...
if source.count(old_append) != 1:
    raise SystemExit("appendLedger hash block not found exactly once")
source = source.replace(old_append, new_append, 1)

old_verify = """            const material = {
                tenantId: Number(row.tenant_id),
                sequence: Number(row.sequence),
                eventId: String(row.event_id),
                actorType: String(row.actor_type),
...
"""
new_verify = """            const hash = sha256(ledgerHashMaterial(row));
"""
if source.count(old_verify) != 1:
    raise SystemExit("verifyLedger hash block not found exactly once")
source = source.replace(old_verify, new_verify, 1)
service_path.write_text(source)

test_path = Path("apps/api/tests/functional/admin/governance_os.spec.ts")
test_source = test_path.read_text()
diagnostic = "        assert.isTrue(verified.body().data.ok, JSON.stringify(verified.body().data));\n"
clean = "        assert.isTrue(verified.body().data.ok);\n"
if test_source.count(diagnostic) != 1:
    raise SystemExit("diagnostic assertion not found exactly once")
test_path.write_text(test_source.replace(diagnostic, clean, 1))

for temporary in [
    Path(".github/workflows/phase11-openapi-diagnose.yml"),
    Path(".github/workflows/phase11-ledger-finalize.yml"),
    Path(".github/scripts/phase11_finalize.py"),
]:
    if temporary.exists():
        temporary.unlink()
